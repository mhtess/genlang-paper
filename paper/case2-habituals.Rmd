...
We do this using a novel data-analytic approach that takes advantage of the mixture-model validated in Expt. 1b.
Then, we test our model on endorsements of habitual sentences, manipulating the frequency of a referent person doing an action (Expt. 2b).
...
We do this by measuring participants' predictions about future rates when causal forces enable or prevent the action (Expt. 2c). 
We separately measure endorsements under the same contexts (Expt. 2d) and use the prediction measurements (Expt. 2c) as the referent feature-probabilities $h$ that the endorsement model is trying to communicate.
...
In this experiment we elicit the feature-probability prior $P(h)$ for different events in order to generate model predictions for corresponding habitual statements (measured in Expt. 2b \& 2d).
...
We next explore the endorsements of habituals from the items whose feature-probability priors were measured the prior elicitation task (Expt. 2a). 
...
The particular intervals used (\{weeks, months, years\}) were selected by simulating predictions of the endorsement model (Eq. \ref{eq:S1}) with the goal of yielding a range of predicted endorsement rates.
...
#### Distinctiveness and referent frequency baseline
...
We saw tremendous flexibility in the frequencies that leads participants to endorse habituals. 
