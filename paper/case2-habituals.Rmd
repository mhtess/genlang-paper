...
Once individuated, an event can be categorized as a kind of a event.
For example, the event of Mary smoking at 2:30pm on January the 24th is an instance of *Mary smoking*. 
Generalizations can be said of this event, in the form of *habitual language*: "Mary smokes".
...
Habituals about people doing actions are interesting because a 
Learning about the behaviors of others is useful because it tells us about what that person is like more generally [e.g., @Repacholi1997; @Seiver2013].
When children describe their lives to other, a surprisingly large amount of the langauge produced concerns the actions of people close to them [e.g., "My brother works part-ti8me at the restaurant"; @McGuire1986].
...
First, we measure the prior distribution over the frequencies for different actions (e.g., how often different people smoke cigarttes; Expt. 2a).
...
By describing novel characters to participants, we are able to directly the manipulate the frequency (feature-probability) with which the referent does the action.
...
Our computational model predicts endorsement rates given a referent-category feature-probability $h$. 
Does $h$ represent the actual, past frequency of the event (e.g., how often Mary has actually smoked in the past month) or the predictive rate of the event in the near future (e.g., how often we should expect Mary to smoke next week)?
...
align = 'h', 
labels = c("A","B","C"),
hjust = -1, nrow =1 , rel_widths = c(1.2, 0.95, 1.6))
```
