...
<!-- of particular importance to social psychology because of their role in causal attribution [@Kelley1973] and intuitive theories of personality and behavior [@]. -->
...
<!-- We focus on present-tense habitual sentences of the form  -->
...
First, we measure the prevalence prior distribution for different actions (e.g., how often different people smoke cigarttes; Expt. 2a).
We then measure endorsements of generalizations about events (*habituals*), while manipulating the frequency with which the event occurs (e.g., how often Mary smokes; Expt. 2b), and use our computational model to predict habitual endorsements. 
...
By describing novel characters to participants, we are able to directly the manipulate the frequency (prevalence) with which the referent does the action.
...
Our computational model predicts endorsement rates given a referent-prevalence $h$. 
Does prevalence represent the actual, past frequency of the event (e.g., how often Mary has actually smoked in the past month) or the predictive rate of the event in the near future (e.g., how often we should expect Mary to smoke next week)?
We extend our theory by measuring participants' predictions about future frequencies and endorsements of habituals when causal forces intervene on the world (Expt. 2c).
...
In this experiment we elicit the prior distributions over frequencies for different events in order to generate model predictions for corresponding habitual statements (measured in Expt. 2b \& 2c).
For language about the behaviors of people, $P(h)$ represents a language user's background knowledge about the  frequency of doing the behavior; this can be conceived as a distribution over *different people*, each of whom do the behavior with different rates.
We designed our elicitation task to take advantage of the mixture-model representation of the prevalence prior used in Expt. 1 (namely: that the distribution over prevalence can be represented as a mixture of those with a stable causal mechanism that gives rise to the feature and those without such a mechanism).
...
For a full list the stimulus set, see \mht{Appendix}.
...
#figure.habituals.priors.scatter
```
...
Parallel to our analysis of generic language endorsements, we articulate a set of simple regression models and an alternative to our uncertain-threshold endorsement model to better understand the data and the contribution of our model.
...
This model predicts the same endorsement level for two actions done with the same frequency. 
Obvious counterexamples exist in our data set:
While participants are willing to endorse that a person "... climbs mountains" having done it 3 times in the past *year*, they are less willing to say that a person "... hikes" and not willing to say that a person "... runs"  (Figure \@ref(fig:habituals-endorsement-figure) A).
Overall, the referent-frequency (log-frequency of action) predicts only a fraction of the variability in responses ($r^2(`r r2.hab.n`) = `r r2.habituls.regression.freq`$; MSE=$`r mse.habituls.regression.freq`$). 
...
The prevalence baseline does appreciably worse in this data set in comparison to the generics data set (Case Study 1) because of the larger stimulus set and presence of multiple referent-frequencies per item.
...
This parameter can be viewed as a measure of the *distinctiveness* of the action, analagous to *cue validity* in the case study of generic langauge.
We again construct a regression model that treats endorsement as a linear combination of frequency and distinctiveness information. 
...
A fixed-threshold model commits the habitual to conveying that a person *does the action with some frequency*, and that frequency is common to all actions.
As in Case Study 1, we incorporate this model into a Bayesian joint-inference model to infer the fixed-threshold and simultaneously predict both the priors data and the endorsement data.
Because we observe no difference between the felicity judgments for habituals of male and female characters, we use a 50\% mixture of the inferred priors for each gender to construct a single frequency distribution $P(p_f)$, which we use for both fixed-threshold and uncertain-threshold models.
We assume the referent-prevalence $h'$ being conveyed by the endorsement model (Eq. \ref{eq:S1}) is the frequency provided to participants (e.g., 3 times in the past year).
Additionally, to account for statements that would be literally false under this model (frequencies that fall below the fixed threshold), we include an additional noise parameter, as we did in Case Study 1. 
...
The fixed-threshold model exhibits a small dynamic range of endorsements. 
...
The lowest frequency used in our data set was $0.6$ times per year (3 times every 5 years).
Thus, all of the utterances evaluated under this fixed-threshold model were literally true.
Therefore, the lowest endorsement this model can apply to an utterance is 0.5 (since both the habitual and silence are always true).

Differences in endorsement rate are due to the informativeness of the fixed-threshold model.
This varies by items, but not by frequency within an item, analogous to how "some" is equally true of 20\% or 50\%.
...
Again, this model has two fewer parameters than the fixed-threshold model (no threshold parameter and no extrinsic noise process).
...
Figure \@ref(fig:habituals-endorsement-figure)C provides insight into how the model is able to do this. 
...
The relationship is not linear (or log-linear), however; habitual endorsements vary in complex ways that reflect interpreters' prior knowledge about the event in the question. 
...
In Expt. 2c, we investigate this question by manipulating and measuring *predictive frequency* and habitual endorsement, while keeping past frequency fixed.
...
We observe in the baseline condition that future frequency perfectly tracks past frequency. 
...
This result implies that our model makes identical predictions for Expt. 2b whether the target is past frequency or expected future frequency (indicating, as expected, that we must look to the new data to distinguish these models).
...
To formalize the predictive frequency speaker model, we use the mean predictive frequency as the referent-prevalence $h'$ that the endorsement model (Eq. \ref{eq:S1}) conditions on. 
