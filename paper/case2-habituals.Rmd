...
We take the event analogue of prevalence to be the *rate* with which the event occurs (e.g., how often Mary smokes).^[
  Specifically, for the generalization "Mary smokes", the instances being generalized are *instances of Mary at different times*, and for an instance to have the feature, it means the instance of Mary is one in which she is smoking. \ndg{this footnote is confusing}
]
...
\ndg{describe the motivation and procedure more clearly. it sounds a little fishy this way, but actually it's great: we set out to test the predictions of the model by choosing the items predicted to have most variability....}
...
\ndg{don't you want to report the posterior predictive correlation with data for this model?}
...
\ndg{this part seems like discussion? also edit for grammar.}
Figure \@ref(fig:habituals-endorsement-figure)C provides insight into how the model achieves this. 
An interpreter who hears a generalization interprets it relative the prior distribution over frequencies, which results in different points at which the generalization is a good statement to assert. 
...
Figure \ref{fig:habituals-predictive-figure} (left) \ndg{check all subfig refs -- in many cases you say eg "left" when it shoulld be "a"..} shows the mean predicted future rate as a function of the past rate given to the participant and the type of causal information given. 
...
Critically, we observe the preventative information strongly decreases and the enabling information slightly increases predicted frequency (Figure \ref{fig:habituals-predictive-elicitation} \ndg{fix ref} left; blue and green dots).
