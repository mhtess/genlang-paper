...
Habitual language conveys generalizations about events and our model decides if a habitual sentence is a pragmatically useful way to describe the rate at which a person does an action, taking into account a naive interpreter's prior beliefs about the event (measured in Expt. 2a). 
...
In Expt. 2c, we further investigated the nature of the underlying prevalence scale by introducing enabling and preventative causal evidence.
We used the empirically-measured predicted future frequency as the object of communication for our endorsement model.
We found that the endorsement model that seeks to communicate its predictions (rather than its observations) is able to account for the influence of top-down moderators on habitual endorsement.
...
In these experiments, we introduced participants to novel actors and by doing so, were able to directly manipulated the frequency of a person doing the action.
The kinds of events we used were familiar to participants (e.g., *running*) and so we measured the prevalence priors for those events.
