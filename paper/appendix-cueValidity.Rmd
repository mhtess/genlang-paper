...
With respect to malaria-carrying mosquitos, @Leslie2007 notes that the mosquito is the only species to carry malaria. 
The distinctiveness of *carrying malaria* then perhaps why *Mosquitos carry malaria* is true.
...
The measures deviate in their characterization of the undiagnostic features. 
...
For example, in response to the question "There is a thing that is female. What are the odds that it is a robin?", a reasonable human response may be "I don't know". Participants may cope with the awkwardness of the question by placing the slider bar in the middle of the scale. 
...
We can further understand our measures of cue validity by comparing them to cue validity derived from our prevalence prior elicitation task (Expt. 1a).
Expt. 1a is not perfectly designed for this comparison, as we supplied participants with half of the animal categories that they rated. (Participants freely generated the other half). Thus, we do not have the best measurement of the prior probability of categories $P(k)$.
...
The prevalence prior derived cue validity is high associated with both measurements: $r_{direct}^2(`r n.dq`) = `r r2.dq`; MSE_{direct} = `r mse.dq`$ and $r_{free}^2(`r n.fp`) = `r r2.fp`; MSE_{free} = `r mse.fp`$.
Of primary interest is how the measurements behave for for desiderata items. 
...
Interestingly, the direct question measurement consistently overestimates cue validity in comparison to the prevalence prior derived cue validity. 


## Summary

*Cue validity* is often used as a measurement for understanding generic truth conditions. 
We articulated three *a priori* boundary conditions for a measurement of cue validity, because we observe different measurements being used in the literature. 
