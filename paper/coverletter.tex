...
But different semantic types can be described in generalization, including events (\emph{habitual language}; e.g., ~\emph{Mary swims after work}) and causal relationships (\emph{causal language}; e.g., ~\emph{Staring at the sun makes you go blind}).
...
The current manuscript differs from previous submissions a number of important ways.
...
Second, we have extended from generalizations about categories, or generic language, to investigate three case studies: generalizations about categories, events, and causes.
We are able to explain endorsements across these three domains without adopting domain-specific assumptions. 
Furthermore, each case study naturally lends methodological advantages to provide further tests of our model. (E.g. with causal generalizations, we are able to \emph{manipulate} (rather than merely measure) prior knowledge.)
...
Overall we feel that this paper represents a much improved, generalized and streamlined, study of generalization in language.
(We would be happy to provide detailed responses to previous reviewers, however we feel this would be of limited use given the major changes in content and presentation.)
