...
#                      10000, "_burn", 5000, "_lag", 20, "_chain", 4, ".csv", sep = ""))
# 
# m.samp <- m.samp %>%
#   filter(param != "score")
# 
# m.samp <- m.samp %>% mutate(chain = 0)
```
...
m.samp <- m.samp %>% filter(!(param == "prevalencePrior")) %>%
  mutate(param = factor(param, levels = c("isPresent", "prevalenceGivenPresent"),
                        labels = c("")))
