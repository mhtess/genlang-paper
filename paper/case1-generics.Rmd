...
We first investgate how the *underspecified threshold* endorsement model predicts actual human endorsements of generic statements.
...
We examine generics about animal categories in order to reliably measure the prior belief distribution over the prevalence of features $P(p_{fk})$.
The prevalence elicitation procedure (Expt. 1b) includes measurements of the referent-category feature-probabilities $p_{fk'}$  (e.g.,$P(x \text{ lays eggs} \mid x \text{ is a robin})$), allowing us to generate predictions for the endorsement model (Eq. \ref{eq:S1}) as well as for simpler, alternative models. 
...
indeterminate generics were agreed at a rate \emph{less} likely than chance ($`r uncertain_MAP`$ $[`r uncertain_low`, `r uncertain_high`])$) but substantially more than false generics  ($`r false_MAP`$ $[`r false_low`, `r false_high`])$).

In addition to these categorical differences between our thirty generic statements, we find a continuum of endorsement values (Figure \ \@ref(fig:generics-endorsement-figure) A). \ndg{the letters after figures are often with a space.. get rid of that?}
...
We next measure the prevalence prior distribution and use them to articulate a set of quantitative models that try to predict these endorsement data. 
...
\ndg{next two paragraphs are redundant with subsections below. delete or merge into subsections.}
...
We use the empirically estimated referent-category prevalence (the same as used in the baseline regression models) as the referent-category prevalence $p_{fk'}$ in the endorsement model.
The empirical measurments of prevalence from Expt. 1b are used to constrain the literal interpreter's prevalence prior $P(p_{fk})$.

We build a single Bayesian data analysis model of the referent-category prevalence $p_{fk'}$, prevalence priors $P(p_{fk})$ (both from Expt. 1b data), and the endorsement data (Expt. 1a). \ndg{recal the mixture of betas model, and resuting params?}
Modeling the data from both Expt. 1a and 1b by a single, joint-inference model simultaneously makes explicit our assumptions about how these data were generated and is the proper way to represent the uncertainty in our measurement of the prior elicitation data (see Appendix \ndg{which one?} and Figure\  \@ref(fig:genericsModelDiagram) for further model specification details).
The endorsement model in addition has optimality parameter $\lambda_1$ (in Eq. \ref{eq:S1}).
...
\ndg{point to a figure or give some stat to back up thi claim?}
...
\ndg{this is just hanging out in space. add a sentence of interpretation, or burry it among other results.}
...
This prior is also the listener $L_0$'s posterior on referent prevalence upon hearing the silent utterance.
The red distribution shows the $L_0$ posterior upon hearing the generic utterance with an uncertain threshold-semantics. 
The speaker model decides when the generic utterance conveys information that would bring the listener's distribution more in line with the speaker's own beliefs. 
Thus, it finds that "Leopards have spots", "Robins lay eggs", and "Mosquitos carry malaria" are useful things to say.
"Robins are female" is neither good nor bad, as the information-content of the utterance is very small relative to silence.
"Tigers don't eat people" is rather misleading, however; it doesn't bring the listener closer to the speaker's belief, despite the speaker believing that the prevalence is more than half. 
...
Heretofore, no formal model has been able to pass the most basic truth-functional tests of the semantics of generalizations: knowing when they are true and false.
...
The key theoretical insight is that the truth-functional semantics is *underspecified* but used precisiely in context by a process of probabiistic inference.
