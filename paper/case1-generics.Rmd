...
Generalizations about categories (i.e., *generic language*; e.g., "Dogs bark.") have received substantial interest in psychology for their role in concept and theory formation [e.g., @Gelman2004], stereotype propagation [@Rhodes2012], motivation [@Cimpian2007], and many other facets of everyday reasoning.
...
However, intuitions and empirical data argue that generics simply do not reduce to quantified statements [e.g., @Khemlani2012, @Prasada2013, @Cimpian2010].
...
```{r generic-endorsement-priors-figure, fig.width = 7, fig.height = 6, fig.cap="A: Empirically elicited prior distributions over prevalence for eight properties. B: Cumulative density plots reveal that a model of a mixture of two Beta distributions does substantially better at capturing the structure of the priors than a single Beta distribution. Distributions are the posterior predictive distributions for the models of the prio and the raw empirical distribution. A completely uniform distribution would be represented as the y = x line.", cache = F}
...
Cue validity is a metric of how diagnostic the feature is of the kind, and is given formally by the probability of a kind given the feature $P(k \mid f)$.
...
Cue validity can thus be seen as a *point estimate* of the prevalence prior distribution, a single metric that collapses across the full distribution. 
...
Though the cue validity of a particular category by a particular property can be derived from the prevalence prior distribution, previous empirical studies have estimated cue validity in various ways from empirical data about distinctiveness [@Cree2006; @Khemlani2012].
...
We found that these different ways of estimating cue validity diverge for a number of key cases, possibly due to an experimenter demand effect in the direct question paradigm.
We thus used the data from the free production paradigm ($n = 100$) to estimate cue validity. 
For a detailed analysis of the different cue validity measurements and comparison to *prevalence prior derived cue validity*, see Appendix B.
...
In explaining the variable endorsements of generics, we related the referent-category prevalence (e.g., the percentage of robins that lay eggs) to the endorsement of the generalization (e.g., "Robins lay eggs") via our communicative model with prior knowledge about the distribution of prevalence (e.g., the prevalence of laying eggs for different kinds of animals). 
...
We now seek to demonstrate how these quantities are causally related to endorsement, by manipulating referent prevalence (Case 2) and prevalence priors (Case 3).
