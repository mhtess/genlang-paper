...
Generalizations about categories (i.e., *generic language*; e.g., "Dogs bark.") have received substantial interest in psychology for their role in concept and theory formation [e.g., @Gelman2004], stereotype propagation [@Rhodes2012], and motivation [@Cimpian2007], as well as many other facets of everyday reasoning.
...
We examine generics about animal categories in order to reliably measure the prior belief distribution over the prevalence of features $P(p_{fk})$ (Expt. 1b).
This elicitation procedure includes measurements of the referent-category feature-probabilities $p_{fk'}$  (e.g.,$P(x \text{ lays eggs} \mid x \text{ is a robin})$), allowing us to both generate predictions for the endorsement model (Eq. \ref{eq:S1}) as well as for simpler, alternative models. 
...
The prevalence prior $P(p_{fk})$ in Eq. \ref{eq:L0} describes the belief distribution on the probability of a given feature (e.g., \textsc{lays eggs}) across relevant categories. 
...
This thought experiment decomposes the prevalence prior $P(p_{fk})$ into a prior distribution on kinds $P(k)$ and then a conditional probability of the prevalence given the kind $P(p_{fk} \mid k)$.
We will use this decomposition to measure the prevalence prior $P(p_{fk}) = \int_{k} P(p_{fk} \mid k) \diff k$ and perform Bayesian data analysis to accurately reconstruct the prevalence prior.
...
Our underspecified-threshold hypothesis models endorsement by considering how well the generalization would bring a naive interpreter's prior distribution on prevalence $P(p_{fk})$ (e.g., the prevalence of laying eggs among other animals) in line with the referent-category prevalence (e.g, the prevalence of laying eggs among robins).
There are several substantive components to this hypothesis: (a) the prior distribution $P(p_{fk})$ is relevant, (b) endorsement is a decision-theoretic process of uttering the generalization vs. not uttering it, and (c) the core semantics of a generalization is an uncertain threshold.
...
The endorsement model (Eq. \ref{eq:S1}) decides whether or not to assert the generalization to a naive interpreter (Eq. \ref{eq:L0}) conditioned on a particular referent-category prevalence $p_{fk'}$  (e.g.,  the percentage of robins that lay eggs).
We use the empirically estimated referent-category prevalence (the same as used in the baseline regression model) as the referent-category prevalence $p_{fk'}$ in the endorsement model.
The empirically measured priors from Expt. 1b are used as the literal interpreter's prevalence prior $P(p_{fk})$.

To incorporate the measurements of these model components into our endorsement model, we build a single, Bayesian joint-inference model of the referent-category prevalence $p_{fk'}$, prevalence priors $P(p_{fk})$ (both from Expt. 1b data), and the endorsement data (Expt. 1a). 
...
To learn about the credible values of the parameters of the model and resulting model predictions, we ran an incrementalized version of MCMC [@Ritchie2016] for 3 chains of 100,000 iterations, discarding the first 50,000 for burn-in.
...
Finally, a fixed-threshold model will have to accommodate responses that are inconsistent with the threshold (i.e., a participant endorsing the generalization when the referent prevalence is low); we thus outfit this model with an *additional* noise parameter, to allow for random guessing. 
Thus, our fixed-threshold alternative model says that participants make an information-theoretic decision, taking into account the interpreter's prior distribution $P(p_{fk})$, using a fixed-threshold semantics, where deviations from a pure information-theoretic decision are accounted for by noise.
...
To evaluate the fixed-threshold model, we examine the posterior distribution which represents what we should believe about the latent parameters of the model (referent prevalence, prevalence priors, the optimality, fixed-threshold, and noise parameters) given the observed data, as well as what predictions of the model given these parameter values.
...
The inferred optimality parameter in Eq. \ref{eq:S1} is `r round(uncertain.threshold.s1opt[[1,"MAP"]], 2)` [`r round(uncertain.threshold.s1opt[[1,"cred_lower"]], 2)`, `r round(uncertain.threshold.s1opt[[1,"cred_upper"]], 2)`].
...
The speaker model decides when the generic utterance conveys information that would bring the listener's distribution more in line with the speaker's own distribution. 
...
Generics have been studied extensively in the cognitive and developmental psychological literatures and shown to have deep implications for wide ranging phenomena from stereotype propagation [@Rhodes2012] to motivation [@Cimpian2007].
...
We now seek to demonstrate how these quantities are causally related to endorsement, by manipulating referent prevalence (Case 2) and feature-probability priors (Case 3).
