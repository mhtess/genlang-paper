...
```{r habituals-prior-fig, fig.width = 8.5, fig.height = 4, fig.cap="Reconstructed priors on frequency from the structured, prior elicitation task (Expt. 2a). Priors are reconstructed by bootstrapping and forward sampling using the structured model (data, blue) or using Bayesian data analysis on the structured model (model, red)."}
...
```{r habituals-simulation-model, eval = F}
l0.hab.model <- '
var probability = function(Dist, x) {
    return Math.exp(Dist.score(x));
}
...
}, stateBins(0, stateBins.length-1), stateBins.slice(1))
...
var priorParams = data.prior.params[0];
  ...
  {
    "action": "climbs mountains",
    "mixture_female_mean": 0.0949,
    "mixture_female_samplesize": 3.7153,
    "mixture_male_mean": 0.0566,
    "mixture_male_samplesize": 0.9652,
    "stableFreq_female_mean": 0.3953,
    "stableFreq_female_samplesize": 1.6992,
    "stableFreq_male_mean": 0.8339,
    "stableFreq_male_samplesize": 1.5171
  }
] 

var stable_female = beta({
  a: mixture_female_mean * mixture_female_samplesize,
  b: (1 - mixture_female_mean) * mixture_female_samplesize
})

var stable_male = beta({
  a: mixture_male_mean * mixture_male_samplesize,
  b: (1 - mixture_male_mean) * mixture_male_samplesize
})
  ...
  return gender == "female" ?
      ...
      categorical({
        ...
        ps: map(function(b) {
          return probability(Gaussian({
                mu: stableFreq_female_mean, 
                sigma: stableFreq_female_samplesize
          }), b) + Number.EPSILON
        }, stateBins )
      }) :
    _.min(stateBins) : 
    flip(stable_male) ? 
      categorical({
        ...
        ps: map(function(b) {
          return probability(Gaussian({
                mu: stableFreq_male_mean, 
                sigma: stableFreq_male_samplesize
          }), b) + Number.EPSILON
        }, stateBins )
      }) :
    _.min(stateBins)
...
```{r habituals-model-insets, fig.width = 4.75, fig.height = 1.5, eval = F}
...
```{r causals-conditions, fig.caption = "Four background distributions participants were exposed to in Expt. 4"}
...
```{r empirical-causal-prior-densities, fig.width = 5, fig.height = 3, fig.caption="Empirical densities from the prediction task in the prior manipulation experiment (Expt. 4a)."}
...
```{r causals-endorsement, fig.width = 4, fig.height = 3}
...
With reasonable assumptions about the prior (e.g., assuming international elephants are unlikely), upon the hearing "Elephants live in Africa", the model infers that most elephants live in Africa.
...
The current formulation of the model is relatively ambivalent the deviations of the listener's beliefs from the speaker's belief (i.e., the speaker doesn't mind that the prevalence can be exaggerated from the generic statement).
If states of the world (or, listener's beliefs about states of the world) correspond to different subjective utilities, speakers who take into the account the subjective utilities of the listener may produce language that deviates from what a pure informational transmission speaker might say.
