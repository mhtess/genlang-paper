...
Leslie starts from the same point as our account: Generics express generalization. 
...
Leslie describes the default generalization mechanism as having three components: the ability to (i) identify whether or not F is a *characteristic* property of K (e.g., one characteristic property for an animal would pertain to the mode of locomotion for the animal) (ii) segment *counterinstances* (Ks which do not have F) into positive and negative counterinstances (i.e., Ks which have some alternative positive property *alt F* and Ks which simply lack F, respectively)^[
  This is a rather technical consideration used to account for the difference between "Birds lay eggs" and "Birds are female". For the latter, counterinstances (male birds) are positive because they have an alternative property (being male). For the former, counterinstances are negative because they simply lack the property (i.e., male birds do not reproduce by some other method).
], and (iii) identify whether or not F is *striking* (e.g., can kill you; "Mosquitos carry the West Nile Virus"). 
On this account, generics are true if only some of Ks have F if all counterinstances are negative and the property is either characteristic or striking. 
...
But there are two other factors that are confounded with striking properties that make it difficult to make a claim about strikingness *per se* leading to generic endorsement.
...
Thus, the prevalence prior distributions of these features are hard to distinguish from properties that are just generally rare and distinctiveness (e.g., having extraordinarily beautiful feathers). 
...
This increase in endorsement for striking and distinctive properties suggests that the prevalence prior $P(p)$  distributions for the two are similar. 
Those experiments did not measure the prevalence prior distribution, but we have found in pilot work that the prior distribution over the prevalence changes to resemble that of a distinctive property when supplied information about the dangerousness of the feature.

The second factor confounded with striking properties, which Leslie herself uses as motivation for their distinction, is that striking properties are projected more strongly (i.e., generalized to more instances of the category) than neutral properties.
Interesting, this enhanced projectibility holds for both dangerous properties (e.g., people doing criminal actions) as well as neutral, statistically low-probability properties [e.g., people taller than 6'5"; @Rothbart1978, Expts. 2 & 3].
...
The notion of *characteristic* properties is the same as @Prasada2006's notion of a *k-property*, which is a property that bears a *principled connection* to the kind. 
Both @Prasada2006 and @Leslie2008 surmise that a property may qualify as characteristic if there exists an overhypothesis about that property for a relavant superordinate category [e.g., each kind of animal has a means for self locomotion; @Goodman1955; @Shipley1993].


\mht{from above:
Beliefs about prevalence will often be the result of conceptual knowledge about kinds and properties [@Leslie2007; @Leslie2008; @Prasada2013].
Such knowledge is not necessary, however: Relevant background knowledge may be merely correlational in nature.
The prevalence prior is a common representation that provides the glue between conceptual and statistical knowledge and natural language semantics.
The truth conditions of generalizations depend only on prevalence, but prevalence depends upon knowledge.
}

The relation of our account to the conceptual accounts of @Leslie2008 and @Prasada2012 has been described above.
\ndg{actually, i think it's better to have a specific section for these conceptual theories here. can leave a but of high-level discussion of prevalence priors in relation to bayesian intuitive theories above, and then talk about details of leslie and prasada here...}
...
We believe this separation provides a more unifying view of the phenomena of generics, giving a single metric as the basic currency for the language of generalizations: the metric of probability. 

In addition, our model provides a path towards unifying conceptual and statistical approaches to generics. 
Our semantic representation is inherently statistical: Natural language semantics only gets to operate on prevalence. 
