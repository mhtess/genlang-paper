...
Similar context-sensitivity can be observed with habituals: Say Mary climbed mountains three times last year, the habitual "Mary climbs mountain" seems well-supported. 
Yet, if John went for a run three times in the last year, we probably should not say that "John runs".
...
Instead, generic statements are thought of semantically as a direct, linguistic manifestation of abstract relations between kinds and properties [@Leslie2008; @Prasada2000; @Prasada2012]. 
...
A semantics based on probability will not be enough, however.
...
For example, the literal meaning of the sentence "Some dogs have four legs" is that there is a non-zero chance that an individual dog will have four legs: $\denote{some}(p) := p > 0$.
"Most dogs have four legs" can also be described as a threshold on prevalence (e.g., the chance that a dog will have four legs is greater than 50%): $\denote{most}(p) := p > 0.5$.^[
  We are describing the standard semantic truth conditions of quantifiers, not their pragmatic interpretations (e.g., that "some" often implies "not all").
]
...
The model in Eq \ref{eq:L0} describes a general mechanism of belief-updating via an uncertain threshold semantics and, thus, naturally extends to generalizations of different semantic types.
For example, if we take underlying semantic scale (formally, the support of $P(p)$) to be that of the temporal frequency of an event (e.g., how often we expect an event to occur), then Eq. \ref{eq:L0} describes a model of habitual interpretation (e.g., "John smokes").
In this case, $p$ is naturally thought of as a rate (e.g., the frequency of an event within a certain time window).
