...
Generalizations in language are often studied via *generic language* (or, generics): simple statements that convey generalizations about categories [e.g., "Dogs are friendly"; @Carlson1977; @Cohen1999; @Leslie2007; @Nickel2008].
Generics are ubiquitous in everyday conversation as well as in child-directed speech [@Gelman2008]. 
In contrast to *particular statements* (e.g., "Rufus is friendly") which describe individual entities or situations and can be verified through observation, generic statements refer to inherently unobservable categories (e.g., the category of \textsc{dogs}) and convey information that extends beyond the present context, a fact which even 2-and-a-half-year-olds appreciate [@Cimpian2008].
...
Generics are just one case of generalization in language, however; events can be described in generalizations (*habitual language*; e.g., "Mary swims after work.") as well as causal relationships (*causal language*; e.g., "Staring at the sun makes you go blind.").
...
These observations have led to proposals that the prevalence or probability of a member of the category having the property (e.g., $P(x \text{ barks} \mid x \text{ is a dog})$) is only tangentially related to generic meaning. 
...
Generic statements have diverse theoretical underpinnings, and the conceptual view highlights this diversity [e.g., @Prasada2013].
...
Formalizing such a theory is scientifically useful as well to make explicit the assumptions the theory, which in turn, elucidates the conditions under which the theory may be meaningfully tested. 
...
To test the hypothesis that probability can serve as the core semantic theory, we build a computational model of endorsement around this semantic theory and compare our model to human judgments.
...
The properties correspondingly also vary widely (e.g., having fur, causing jitters, typing). 
Yet the mathematical description of the inductive inference is always given by the conditional feature-probability $h$.

How does one come to learn $h$, the uncertainty that underlies an inductive inference about a member of a category having a property?
One way is through observation: One can observe instances of the category $k$ and note how many of them have $f$.
But the inferences derived from observation will depend upon on one's background knowledge about the property [@Gelman2003; @Keil1992].
For example, if you observe an instance of a novel creature category that \emph{has two legs}, you may infer that all of that category has two legs.
If in addition you observe that it has broken wings, the corresponding prediction of another instance having broken wings is not nearly as strong [@Nisbett1983].
This kind of knowledge can be represented formally as a probability distribution over the feature-probabilities $h$: $P(h)$ [@Kemp2008], and observational data $d$ can update one's beliefs via Bayes' Rule: $P(h \mid d) \propto P(d \mid h) \cdot P(h)$
...
If generalization from observations can be described as a predictive probability, then it seems natural to consider that same metric to be central to the semantic content of an utterance that conveys a generalization.
To connect to the semantic tools of *truth values* [@Montague1973], we must relate the scalar quantity of probability to a Boolean truth value.
...
For generalizations, then, the threshold would apply to the *feature-probability* $h$:^[
We emphasize that our account is based on an agent's subjective probability $h$. The literature on generic language has often examined *prevalence*, or the proportion of members of a category with the property, as the primary quantitative metric that could relate to generic meaning (in addition to *cue validity*, described below). A Bayesian agent, however, would only use prevalence as a way to update her prior knowldege $P(h)$ to form a posterior probability for future use. Thus, we consider the semantic content of a generalization to be related to probability, not necessarily prevalence.
]
$$\denote{u_{gen}}(h, \theta) = \{h>\theta\}$$

### Resolving underspecification

What threshold $\theta$ should be used for the generalization?
We posit that the language of generalization is *underspecfied* with respect to the actual truth-functional threshold $\theta$.
That is, we posit that $\theta$ is contextually-determined, and specifically that it is determined by Bayesian reasoning using relevant prior knowledge.

\begin{eqnarray}
Lit(h, \theta \mid u) &\propto& {\delta_{\denote{u}(h, \theta)} \cdot P(h) \cdot P(\theta)} \label{eq:L0}
\end{eqnarray}

Equation \ref{eq:L0} is a model of a "literal" listener $Lit$ who updates her beliefs about the feature-probability $h$ according to the meaning of an utterance $u$. 
We formalize the underspecification of $\theta$ by a uninformed prior distribution over $\theta$ [i.e., *a priori* all thresholds are equally likely; cf., @Lassiter2013].
The semantic content of an utterance is represented by the Kronecker delta function  $\delta_{\denote{u}(h, \theta)}$, which here returns probabilities proportional to $1$ when the utterance is true (i.e., when $h > \theta$) and $0$ otherwise.
...
The above model determines how $\theta$ should be resolved in context.
Now, how do we decide when a statement conveying a generalization is true or not?
To answer this question, we draw upon recently developed tools in the field of probabilistic pragmatics [@Franke2009; @Frank2012; @Franke2015; @Goodman2016].
The felicity or endorsement of a statement can be modeled by a speaker model who has in his repetoire only two possible utterances: convey the statement to a naive listener or stay silent [@Franke2014cogsci; @Degen2014]. 

\begin{equation} 
E(u \mid h = h') \propto \exp{(\lambda \cdot U(u; h') )}
\label{eq:S1}
\end{equation}
...
The utility of an utterance is defined by how well it conveys the speaker's beliefs ($h'$) to a naive listener $Lit$, after integrating out her residual uncertainty about $\theta$.^[A more general version of this model can relax the assumption that the endorser has access to a specific feature-probability $h'$ that he is trying to communicate.
Rather, the endorser may have a belief distribution over $h'$ for the category $k$, corresponding to the endorser's beliefs about the feature-probability of the property in the category. 
In this situation, we would define the endorsement utility to be the expected value of the informativity, which integrates over the endorser's belief distribution:
$S(u \mid k) \propto \exp{(\lambda \cdot {\mathbb E}_{h\sim P_{k}} \ln{ \int_{\theta} L(h, \theta \mid u)} \diff \theta )}$
]
The literal semantics for the generalization is the threshold function described above; the literal semantics for the option of staying silent carries no informational content:  $\denote{u_{silence}(h, \theta)} \propto \text{True}$.^[
This "silent" alternative can be realized in at least two other ways: the endorser could say the negation of the generalization (i.e., "It is not the case that Ks F") or the negative generalization (i.e., "Ks do not have F").
...
Thus, Eq. \ref{eq:S1} provides a mapping from a *referent-category feature-probability* $h'$  (e.g., $P(x \text{ lays eggs} \mid x \text{ is a robin})$) to an endorsement probability for the corresponding generalization (e.g., "Robins lay eggs"). 
It does this assuming a threshold semantics that deals only with *feature-probabilities* but without us having to specify the threshold (note that there is not $\theta$ is Eq. \ref{eq:S1}).
...
<!-- In Bayesian statistics, when there is uncertainty about latent parameter, one specifies a prior distribution over the parameter and reasons backward about the parameter's likely value given any data. -->
...
<!-- Though it is possible that different $P(\theta)$s could be learned for different contexts, we assume this prior is identical across contexts and is completely uninformed by the context *a priori*: $P(\theta) = \text{Uniform}(0, 1)$. -->
...
### The feature-probability prior and model simulations
...
The endorsement model defined in the previous section predicts that the endorsement of a generalization depends upon two factors: (i) the feature-probability $h$ in the referent-category (e.g., the \% of robins that lay eggs) and (ii) shared background knowledge about what feature-probabilities are likely *a priori*.
We use classic examples from the literature on generics to elucidate the meaning of these two components and demonstrate why the endorsement model makes the predictions that it does. 
We implemented the models in the probabilisitic programming language WebPPL [@dippl].
...
var endorser = function(featureProb){
  Infer({model: function(){
    ...
    var L0 = marginalize(listener0(endorsement), "state_Posterior")
...
exampleParameters <- list(
  robinsAreFemale =  list(params = data.frame(a = 10, b = 10), mix = 1, featureProb = 0.5), 
  robinsLayEggs =  list(params = data.frame(a = 10, b = 10), mix = 0.2, featureProb = 0.5), 
  sharksDontEatPeople =  list(params = data.frame(a = 50, b = 1), mix = 1, featureProb = 0.8), 
  dogsBark =  list(params = data.frame(a = 5, b = 1), mix = 0.4, featureProb = 0.9), 
  kangaroosHaveSpots =  list(params = data.frame(a = 5, b = 1), mix = 0.7, featureProb = 0.1), 
  mosquitosCarryMalaria =  list(params = data.frame(a = 1, b = 30), mix = 0.1, featureProb = 0.1)
)

s1.simulations <- data.frame()
for (p in exampleGenerics){

    s1.rs <- webppl(s1.model, data = exampleParameters[[p]], data_var = "data")
...
s1.simulations.relabeled <- s1.simulations %>%
  mutate(example = factor(example, 
                          levels = c("dogsBark", "kangaroosHaveSpots", "sharksDontEatPeople", "robinsLayEggs", "robinsAreFemale", "mosquitosCarryMalaria"),
                            labels = c("Dogs bark", "Kangaroos have spots", "Sharks don't eat people", "Robins lay eggs", "Robins are female", "Mosquitos carry malaria")))
...
```{r simulations, fig.cap = "Model behavior. Prior distributions over feature-probabilities (Left; grey) and listener model posterior distributions upon hearing an utterance with an uncertain threshold (Middle; red) and a fixed threshold (Right; blue), for five different schematic prior distributions. The uncertain threshold (Middle; red) is hypothesized to correspond to utterances that convey generalizations, while the fixed threshold (Right; blue) is a control model. Shapes of the priors were chosen to intuitivly correspond to properties labeling the distributions.", fig.width = 14, fig.height=6, cache=F}
