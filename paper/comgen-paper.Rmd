...
### Information-theoretic endorsement models

The model we propose for endorsing generalizations is a model of a speaker that decides whether or not to say the generalization to a naive listener.
Unlike the baseline regression models based on point-estimates of referent-category feature-probability and cue validity, this cognitive model has access to the full prior distribution on feature-probabilities $P(h)$.
This model has two other main architectural features: an information-theoretic decision between producing the generic vs. an information-less alternative (formalized in the speaker model) and uncertainty about the semantic threshold $P(\theta)$ that corresponds to the theoretical, core meaning of a generic statement.
We construct a strong, alternative model by removing the uncertainty about the semantic threshold $\theta$. 
We describe that model below in more detail; before, we describe our data analytic approach for this pair of models. 

For both fixed-threshold and uncertain-threshold models, the endorsement model $E$ (Eq. \ref{eq:S1}) decides whether or not to assert the generalization conditioned on a particular referent-category feature-probability $h'$^[
  ...
  For these data, the predictions of these two models are nearly identical.
]
(e.g., given the percentage of robins that lay eggs, does it want to say "Robins lay eggs"?).
We use the empirically estimated referent-category feature-probability (the same as used in the baseline regression model) as the $h'$ in the endorser $E$ model.
The empirically measured priors from Expt. 1b are used as the literal interpreter's feature-probability prior $P(h)$.
The $E$ endorsement model is then fully specified (i.e., can generate predictions) after setting the optimality parameter $\lambda_1$ (in Eq. \ref{eq:S1}).
To fit this pair of endorsement models, we incorporate them into the Bayesian data analytic model of the feature-probability prior data (Expt. 1b) to create a single, joint-inference model where the optimality parameter $\lambda_1$ is inferred jointly with all the other latent parameters of the full model (the referent feature-probability $h'_{k, f}$ for each category $k$ and property $f$ and the parameters of the feature-probability priors $P_f(h)$ for each property $f$) using data from Expt. 1a \& b (see Appendix and Figure \ref{fig:genericsModelDiagram}).
...
This means the theoretically-interesting predictionso of this model --- predictions of generic endorsement --- are based on intuitively meaningful model components (i.e., the shapes of the feature-probability distributions). 
...
(Again the posterior predictive distribution marginalizes over the inferred parameter values to produce predictions about what the data \emph{should look like} given the endorsement model and the observed data.)
