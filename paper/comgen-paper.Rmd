...
As a formal theory of generalizations in language, we aim to unify significant swaths of language that are, on their surface, quite distinct from one another: "Birds lay eggs", "Sally smokes", "Herb X makes animals sleepy".
<!-- What these expressions have in common is *genericity*, the fact that these statements convey generalizations.  -->
We have formalized a unified, core meaning using the underlying scale of predictive probability or *prevalence*.
We showed that all three kinds of generalizations exhibit the same kind of sensitivity to context (Expt. 1, 2a \& 2b, 3).
Our framework naturally accounts for a number of logically puzzling category generalizations, or *generics* (Expt. 1).
We provide an avenue for top-down moderators on language use, by elucidating the semantic scale as defined by a subjective, predictive probability, not mere frequency (Expt. 2c).
...
Our model formalizes a simple semantics for generalizations and investigates the structure of world knowledge that could give rise to the context-sensitivities observed in the data.
That is, we provide a route for precise quantitative influence of conceptual structure on generic language by way of shared background knowledge between speakers and listeners [c.f., @Leslie2008].
...
Our theory assumes that listeners and speakers share relevant background knowledge (in the form of a *prevalence prior*) against which generalizations are interpreted and that the language interfaces directly with *prevalence*. 
...
Indeed, we agree that generics are about kinds, but argue that the reason why particular generics are licensed in a context is about the information conveyed to a listener relative to their background knowledge.
This background knowledge can very well presuppose beliefs about natural kinds and assume richly structured relations between categories and the properties.
In other situations, relevant background knowledge may be merely correlational in nature.
The important commonality among such different possible domain-specific background theories is that they each have a statistical layer to them (i.e., they each give rise to beliefs about *prevalence*), which language gets to interface with.
Thus, our theory provides a clean separation between background knowledge and the semantics of generalizations; the interface between the two is predictive probability or *prevalence*. 
...
We do not assume a particular view of natural kinds in our theory, but note that viewing natural kinds as richly structured categories with clusters of non-obvious properties is compatible with our information-theoretic communicative theory. 
Future work should investigate formalizations of attested differences in background knowledge about properties [e.g., @Prasada2013's *principled*, *statistical*, and *causal* connections] and their corresponding influence on generic endorsement. 

In our modeling work, we found that empirical data that underlie the prevalence prior distributions are best modeled by a mixture-model with two mixture components.
Such a prior distribution draws qualitative distinctions (with respect to the feature) between different kinds of categories by virtue of the model representing multiple distinct components. 
It is likely that the actual conceptual knowledge against which generalizations are interpreted is far-richer than simply a two-component mixture-model.
We note, however, that a richer model of the prior is not necessary to account for the context-sensitive endorsement patterns we observe in classic philosophical cases as well as in our empirical endorsement data. 

Rational models of communication, of the kind we have developed in this paper, make explicit a speaker's assumptions about the *goals of communication*, sometimes referred to as the Question Under Discussion or QUD [@roberts1996qud]. 
Our simple endorsement model assumes that the goal of communication is the prevalence.
That is, the endorsement decision is about the *prevalence* information conveyed to the listener.
This idealization will surely be elaborated in 
Richer communicative contexts will surely need to elaborate the QUD to fit more appropriately with the informational goals of the context.
We note, however, that the endorsement model makes almost identical predictions if the QUD is assumed to be about the more abstract parameters of the background knowledge (e.g., which mixture component the referent is part of).
This finding is promising for the future goal of integrating communicative goals about kinds or conceptual structure into the information-theoretic communicative framework presented here.
...
Predictions about the presence of a feature can be the result of internal causes of the feature [e.g., in an essentialist view of natural kinds; @Gelman2003] or merely correlational happenstance.
What matters for their influence on generic endorsement, we posit, is their influence on subjective, predictive probability or *prevalence*.
...
Communicating probabilities might seem contrary to the long attested phenomena of failures in reasoning about probability [@Tversky1974].
...
Indeed, young children are actually quite good about reasoning about probabilities, but in ways that are not explicit or tied to the actual language of probability [@Gweon2010; @Dewar2010].
...
Our model of an interpreter, or naive listener, has access to implicit statistics of the event or property in question, represented by the prior belief distribution over the prevalence $P(h)$.
The prevalence priors for a property, event, or cause are constructed by considering *other possible* categories having the property, people doing the action, or causes producing the effect.
Collectively, these other kinds, people, or causes form *comparison classes* against which the referent-category is evaluated.
Thus, the prevalence prior $P(h)$ is constructed with respect some comparison class $C$: $P_C(h)$.
...
However, like the semantics of generalizations, the other entities or categories that comprise the comparison class are underspecified.

Uncertainty about the comparison class adds yet more flexibility to generalizations.
Even if interlocutors coordinate on a particular comparison class (e.g., agreeing that they are talking *relative to other animals*), they may disagree about the prevalences for other *other categories* in the class.
That is, our model makes the prediction that the acceptance or rejection of the statement "Humans cause global warming" or "Muslims are terrorists" depends upon a speaker's beliefs about how other forces (e.g., plate tectonics) influence the climate or how often other categories (e.g., neo-nazis) commit acts of terrorism.
In Expt. 3, we demonstrated the influence of other categories on endorsement of a generalization.
...
Interlocturs may not only fail to coordinate on what categories are in the comparison class, the prevalences that may be assumed for the constituent categories of the comparison class, but *how* the comparison class should be constructed.
The comparison classes used in our case studies were with respect to the target *category* (*other animals*, *other people*, *other possible causes*); thus, our model addresses the implicit Question Under Discussion [QUD; @roberts1996qud] of "*what* has feature?".
Were the QUD to be reversed (e.g., "what features does category have?"), the comparison classes would need to be constructed with respect to the *feature* (e.g., *other features*, *other actions*, *other effects*).
The same sentence can be used to address multiple QUDs [@KrifkaGenericBookFocus], giving rise to multiple distinct comparison classes.
...
Perhaps the most surprising aspect of the language of generalization is how difficult it is to formalize, given how common they are in child-directed and child-produced speech [@Gelman1998; @GelmanEtAl2004].
...
This asymmetry between the simplicity of the formal theory and the age acquisition has led some to conclude that the normal tools for describing the semantics of quantified utterances (i.e., a truth-functional threshold) is inapprorpriate for generic language [@Leslie2008].

Rejecting the tools of truth-functional semantics for generalizations would be throwing the baby out with the bathwater.
The acquisition of a threshold-based truth-functional meaning $\denote{u}(h, \theta) = \{h :h > \theta\}$ requires learning at least three aspects of meaning: 
...
However, for generalizations, no fixed threshold is required; the language of generalization is acquired once aspects (1) and (2) are learned.
<!-- In our model, $\theta$ is left underspecified in the semantics and is inferred in context. -->
This informal analysis predicts that a child would first acquire generalizations, and from that, carve out more precise meanings for quantified language, consistent with the developmental data.

There is a secondary argument for why the semantics we posit presents an easy learning problem.
...
Uniform uncertainty over values in the unit interval $[0, 1]$ for threshold $\theta$ in a threshold-function (as we have assumed in our model) is mathematically equivalent to a *soft semantics* wherein the degree to which the utterance is true is proportional to the degree, in this prevalence $h$ itself: $\int_{0}^{1} \delta_{h > \theta} \diff \theta =  h$
...
We aimed to balance two competing goals in designing our experimental stimuli: (1)  including familiar, philosophically puzzling examples that have been discussed in the vast literature on generic language and (2) introducing novel stimuli that provide targeted tests of our model.
...
Sentence (1) is still thought to be true despite zero actual instances of the event.
We think (1) is a boundary case of generalizations expressing predictive probabilities, which we elucidated in Expt. 2c.
...
Sentence (2) is still considered false, every though the property holds in 100\% of the category [@Cohen1999].
...
Then, given that the all professions would have roughly the same probability of having employees with even social security numbers, Sentence (2) would be similar to "Birds are female". 
...
In fact, with prior knowledge weighting against the existence of *international elephants* and without any further semantic assumptions, our model infers that some elephants live in Africa and others live in Asia (and they are different elephants).^[
...
We hypothesize that the strangeness of these examples has to do an inflexible relationship between the comparison class and the referent-prevalence.
...
Then, given that "books" is the superordinate category of "kinds of books", the referent-category prevalence cannot be different from the mean of the prevalence prior.
Finally, given this prevalence prior and the referent-prevalence, our endorsement model would predict "Books are paperbacks" is neither true nor untrue, in the a way analogous to "Robins are female". 
...
The current formulation of the model is relatively ambivalent about the deviations of the listener's beliefs from the speaker's belief (i.e., the speaker doesn't mind that the prevalence that she believes to be true will be exaggerated in the mind of the listener).
...
These theories fall into two broad camps: Those that appeal to the statistics of the world (e.g., how many Ks have F) and those that appeal to structured, conceptual representations (e.g., *there is something about being K which causes it to F*).
...
The relation of our account to the conceptual accounts of @Leslie2008 and @Prasada2012 was described above.
...
We believe this provides a more unifying view of generics, giving a single metric as the basic currency for the language of generalizations: probability. 
...
Disagreements are also predicted when interlocutors fail to share background assumptions, which could be differences in the prior distributions on prevalence or the comparison class.
