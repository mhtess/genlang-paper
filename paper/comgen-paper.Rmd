...
The prevalence prior reflects a listener's beliefs about the prevalence of the feature in an unknown or unfamiliar category and be conceptualized as a distribution over the prevalence among alternative categories (Figure\ \@ref(fig:simulations)).
...
For example, the *lay eggs* prevalence prior should be bimodal, with substantial probability mass near 0 (since many animal categories do not have egg-layers) and a secondary component peaked around 0.5 (because among the animals with egg-layers, only the female members of the category have the property).
...
worked.examples.tab <- xtable::xtable(worked.examples, 
                       caption = "Example generalizations that a theory of generics should correctly predict. See details of each example in corresponding section.", 
...
The endorsement model, which believes that very few kangaroos have spots, then would rather not endorse the statement because it is too strong a statement and would mislead the listener.
...
Given this background knowledge, the generic interpretation model returns roughly the same interpretation distribution for each hypothetical utterance: In each case, the model believes 50% have the property.
However, only in the case of "Robins lay eggs" does the endorsement model actually assert the generic; it does so because the listener interpretation would be more aligned with the referent prevalence than if the endorsement model produced the null utterance, which would instead return the prior.
In contrast, "Robins are female" does not carry any information (the listener posterior is not different from the prior); here, the generic would not be misleading but uninformative, and hence the model predicts an endorsement probability of 0.5 (i.e., *neither true nor false*).
Indeed, previous studies on generic endorsements have found "Robins are female" to receive an intermediate endorsement level [@Prasada2013].

**5. Mosquitos carry malaria.**

Finally, the statement "Mosquitos carry malaria" is intuitively true in spite of the fact that the vast majority of mosquitos are actually malaria-free.
...
When the prevalence of the feature is very high, it is also not necessary for the model to endorse the generic.
"Sharks don't eat people" is predicted to be a somewhat strange utterance, despite the fact that the vast majority of sharks do not eat people.^[
 We create this example as the converse of the classic example "Sharks eat people", predicted to be *true* despite low prevalence [@Leslie2007]. We reverse this example to show how the model deals with a high-prevalence feature that is predicted to be false or infelicitous.  
] 
<!-- Yet, intuitively, the generic would be misleading (similar in intuition to "Kangaroos have spots"). -->
Because for almost all animal categories, the prevalence of *not eating people* is almost 100% (very few things eat people), interpreting the statement "Sharks don't eat people" would lead one to believe that no sharks eat people, which is too strong.
...
We will test this quantitative prediction in Case Study 1 on generics.
...
\mht{do we want an example of a habitual or a minimal pair of habituals (e.g., sally hikes vs. climbs mountains)?}
...
"Mary handles the mail from Antarctica" is still thought to be intuitively true despite zero actual instances of the event.
This highlights an important ambiguity in the theoretical commitments of the uncertain threshold model: The endorsement model aims to communicate some referent prevalence $p$, but does the prevalence represent the actual, objective frequency in the world (e.g., the number of times Mary has handled mail in the past) or a subjective, predictive degree of belief in the head (e.g., our prediction that were the appropriate situation to arise, Mary would be handling the mail from Antarctica)?
...
Thus the predictive probability that Mary will handle Antarctican mail, if there is any, is high.
...
We explore this question experimentally in Case Study 2 on habituals. 

**8. Supreme Court Justices have even social security numbers.**

Imagine that every current and past Supreme Court Justice has had a social security number which was an *even number*. 
...
That is, we predict observers strongly believe there is no causal relation between the evenness of one's social security number and selection for the Supreme Court, and thus would assign a roughly 50\% subjective probability that the *next* justice would have an even social security number.
...
@Nickel2008 suggests that "Elephants live in Africa and Asia" is semantically equivalent to: "Elephants live in Africa and elephants live in Asia".
Statistical accounts of generics have trouble with this kind of conjunction because they historically have appealed to fixed, truth-conditional cutoffs for generics [e.g., generic means *more than half*; @Cohen1999].
...
Interestingly, an incremental parsing of the sentence  (i.e., upon hearing only that "Elephants live in Africa") leads our model to believe that *most* elephants live in Africa (Figure\ \@ref(fig:elephants)B).
...
We leave for future work the testing of quantitative predictions for this kind of example.
