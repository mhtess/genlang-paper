...
These introductory sections are followed by three empirical case studies: generalizations about categories (*generic language*), events (*habitual language*), and causes (*causal language*). 
...
We first describe a model for generics, before noting how the basic ideas generalize to habituals and causals.
...
The prevalence prior $P(p)$ is a distribution over probabilities $p$, which represents the output of a generative model of particular domain.
For properties of objects, the generative model is naturally a Bernoulli model (e.g., flipping a coin) and thus the support of $P(p)$ is $[0, 1]$.
...
The prevalence prior reflects a listener's beliefs about the prevalence of the feature in an unknown or unfamiliar category and be conceptualized in a theory-neutral way as a distribution over the prevalence among alternative categories (Figure\ \@ref(fig:simulations)).^[
  Alternatively, the prevalence prior can be conceptualized as one aspect of an intuitive theory of a domain. 
]
...
worked.examples <- data.frame(
  ...
  `potential issue` = c(
    ...
    "Impossible for most to live in Africa and most to live in Asia"
  )
) %>%
  rename("Referent prevalence" = referent.prevalence,
         "Intuitive truth value" = intuitive.truth.value,
         "Issue" = potential.issue)

worked.examples.tab <- xtable::xtable(worked.examples, 
                       caption = "Example sentences that a theory of generalizations should correctly predict. See details of each example in corresponding section.", 
...
It is this real-world knowledge (presently formalized in terms of a prevalence prior) where issues about domain specific knowledge, interpretation of properties [@Prasada2006], over-hypotheses [@Leslie2008], and essentialism [@Gelman2003] can all play a role.
...
We have pilot data suggested these constructs are indeed altered as a result of introducing information about the dangerousness of features to participants [e.g., as in @Cimpian2010].
