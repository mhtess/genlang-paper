...
We provide an avenue for top-down moderators on language use, by elucidating the scale as defined by a subjective, predictive probability, not mere frequency (Expt. 2c).
...
The formal Bayesian model we articulate provides a clean separation of the semantics of a generalization (operationalized in terms of the likelihood function) from background, world knowledge (the prior).
...
This idealization will surely be elaborated in richer communicative contexts. 
It is noteworthy, however, when using a structured model of the prior (e.g., a mixture model), as we have used throughout this paper, that the endorsement model makes almost identical predictions if the QUD is assumed the more abstract parameters of the background knowledge (e.g., which mixture component the referent is part of).
...
Prasada has argued for a distinction between *characteristic* properties (e.g., "Diapers are absorbent") and *statistical* properties (e.g., "Diapers are white").
...
We found that empirical prevalence distributions are best modeled as a mixture-model. 
...
It is likely that conceptual knowledge is far-richer than our simple two-component mixture-model, though a richer model of the prior is not necessary to account for the context-sensitive endorsement patterns we observe in our data. 
<!-- influences these distributions, such as higher-order conceptual knowledge about the nature of properties and categories [@Gelman2003; @Keil1992]. -->
...
We make the distinction that what is communicated is probability not frequency.
...
Communicating probabilities might seem contrary to a basic tenet in cognitive psychology, that human cognition falls short when reasoning about probability [@Tversky1974].
