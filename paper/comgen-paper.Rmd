...
The language of generalization is ubiquitous in everyday conversation and conveys rich meanings, impacting motivation [@Cimpian2007], transmitting stereotyped beliefs about social categories [@Rhodes2012], and making meaning from experience [@Orvell2017].
...
Abstract, potentially domain-specific, beliefs about the projectibility of different properties can be represented by a probability distribution over the prevalence $P(p)$ [@Kemp2007a].
...
For simplicity, we take the null alternative to be an information-less "silent" utterance which is always true: 
...
This endorsement model (called $S$ for speaker) does so by reasoning about the interpreter model $Lit$: 
\begin{equation}
...
We go beyond previous semantic models of generics [e.g., @Cohen1999] by embedding our semantic hypothesis (a simple but vague threshold) in a cognitive model $Lit$ in which relevant contextual knowledge can be formalized.
...
Cue validity operationalizes an alternative hypothesis about the interpretation of generic statements that the reading of statements like "Mosquitos carry malaria" is as "It is mosquitos that carry malaria", and empirically-elicited cue validity has been shown to be highly correlated with endorsements of generics [@Khemlani2012].
...
The human species is remarkable not only because we can extract useful generalizations from observations of the world, but because we can convey these generalizations to eachother succinctly in language.
Generalizations expressed in language (e.g., "John runs.", "Dogs are friendly.", "Staring at the sun makes you go blind.") are a premier example of how simple statements---statements understood by even the youngest language users---can convey subtle meanings and display complex sensitivities to context.
We have argued that the core meaning of such linguistic expressions can be understood by a vague threshold function that operates over a speaker's inductive beliefs, formalized in terms of predictive probability.
...
-->
We tested this theory by exploring its implications for truth judgments, operationalized as a speaker's decision about whether it would be useful to utter the generalization. 
...
In each case our model provided strong quantitative fit to human judgements.
...
By precisely articulating the conditions under which generalizations are true or false, we are able to describe how an agent should update its beliefs based on that sentence. 
Thus, in addition to providing a quantitative understanding of the phenomena of the language of generalization, formalizing this language opens up the possibility of building models that learn abstract, generalizable knowledge from language.
xx
...
These predictive probabilities are fundementally mental representations and not observable frequencies, the object of other statistical approaches to genericity.
...
Our theory suggests that the problems with understanding probabilities observed in classic, cognitive psychology paradigms are a problem in understanding the numerical language used to convey explicit probabilities, a historically quite recent innovation [cf., @Levinson1995]. 
...
<!-- \ndg{this section is pretty critical, and needs work. or posisbly much of it belongs in the discussion of others sematic theoris?} -->
...
Actual conceptual knowledge against which generalizations are interpreted is most likely richer than our simple two-component prevalence distributions.
...
The important commonality among different domain-specific background theories is that they each have a statistical layer to them (i.e., they each support prediction via beliefs about *prevalence*), which language interfaces with via the tools of truth-functional semantics.
...
Our model distinguishes the semantic contribution of an utterance from a speaker's utility in producing that utterance. 
For simplicity, the mode presented here assumes these two are the same: they are both about prevalence. 
However, the model makes nearly identical predictions if the speaker's utility comes from communicating the abstract parameters of the prevalence distributions (i.e., if the communicative goal is less about prevalence and more about the kind of thing that the referent is).
This behavior is promising for constructing models of generalizations that teach about the abstract relations present in intuitive theories of kinds and properties.
...
Adult judgments of the felicity of gradable adjectives like *tall* or *dark* depend upon fine-grained details of the statistics of the comparison class [@Qing2014; @Schmidt2009; @Solt2012] in ways directly analogous to the context-sensitivity of the language of generalizations highlighted in this paper.
...
In this paper, we do not include such a mechanism in our model, instead we made the comparison class clear to participants when possible in order to study the remaning context-sensitivity with generics.
<!-- \ndg{say something about how the comparison class might be set, cite your cogsci paper, say why this paper is ok without this: we made comparison class clear to Pp in order to study remaining issues with generics.} -->
...
Finally, the comparison classes used in our case studies were with always respect to the category (*other animals*, *other people*, *other possible causes*).
Thus, our endorsement model assumes the goal of communication, sometimes referred to as the *Question Under Discussion* or QUD [@roberts1996qud], was to address the question of "*what* has feature?".
...
The statement "Lawyers care about the law." can be interpreted with a category-wise comparison class (i.e., "Lawyers *(as opposed to doctors, firefighters, ...)* care about the law."; imagine a pedagogical context) or a feature-wise comparison class (i.e., "Lawyers care about the law *(as opposed to justice, doing what is morally right, ...)*" e.g., when said sardonically).
Prosodic focus easily gates between these interpretations (e.g., "[Lawyers] care about the law" vs. "Lawyers care about [the law]").
\ndg{mention that prosodic focus easily switches between these.}
...
This asymmetry between the simplicity of the formal theory and the age of acquisition has led some to conclude that the normal tools for describing the semantics of quantified utterances (i.e., a truth-functional threshold) are inappropriate for generalizations [@Leslie2008].

Rejecting the tools of truth-functional semantics for generalizations would be throwing the baby out with the bathwater.
With some reasonable assumptions about the hypothesis space of semantic meanings, the acquisition of a threshold-based truth-functional meaning $\denote{u}(p, \theta) = \{p :p > \theta\}$ requires learning at least three distinct aspects of meaning: 
(1) the *dimension* being described (i.e., probability $p$),
...
Thus, under our framework, generalizations should not only be learned before quantifiers but also faciliate the acquisition of quantifiers.
...
A number of other theories of genericity have been proposed in formal semantics and in psychology, which  posit different constructs or psychological mechanisms to account for the flexible truth conditions of generics.
Thse accounts fall into two broad camps: Those that appeal to the quantification or the statistics of the world (e.g., how many Ks have F) and those that appeal to conceptual representations or abstract beliefs (e.g., *there is something about being K which causes it to F*).
...
We believe this separation between conceptual structure (which exists in the head) and prevalence (a quantitative metric for natural language semantics) provides a more unifying view of the phenomena of generics, giving a single metric as the basic currency for the language of generalizations: the metric of probability. 
By formalizing how background knowledge influences generic understanding, our model provides a path towards unifying conceptual approaches to generics with inherently statistical natural language semantics.
...
\mht{unsure about this paragraph...}
<!-- Not all theories are created equal, however.  -->
Even the most influential of extant theories generally makes one or a few predictions about "different kinds" of generics, and further leave it to intuition to decide for a particular generic statement, what kind of generic it is.
...
As we describe below, it relates to extant theories in meaningful ways. 
...
Leslie takes the same starting point as our account: Generics express generalization. 
...
Leslie describes the default generalization mechanism as having three components: the ability to (i) identify whether or not F is a *characteristic* property of K (e.g., one characteristic property for an animal would pertain to the mode of locomotion for the animal), (ii) identify whether or not F is *striking* (e.g., can kill you; "Mosquitos carry the West Nile Virus"), and (iii) segment *counterinstances* (Ks which do not have F) into positive and negative counterinstances (i.e., Ks which have some alternative positive property *alt F* and Ks which simply lack F, respectively).^[
  This positive vs. negative counterinstance distinction is a rather technical consideration [though not without psychological foundation, see @Leslie2009 p.36] used to account for the difference between "Birds lay eggs" and "Birds are female". 
  For the latter, counterinstances (male birds) are positive because they have an alternative property (being male). 
  For the former, counterinstances are negative because they simply lack the property (i.e., male birds do not reproduce by some other method).
  We will not discuss this consideration any further other than to note that we explain the difference between "Birds lay eggs" and "Birds are female" by a different mechanism but that the notion of alternative features may come into play with feature-wise comparison classes (described above).
]
On this account, generics are true if only some Ks have F if all counterinstances are negative and the property is either characteristic or striking. 
...
The evolutionary argument for accepting striking generics is straightforward: These are relevant properties to know about if you want to survive. 
But there are two other factors that are confounded with striking properties that make it difficult to make a claim about strikingness *per se* (as opposed to beliefs about prevalence) leading to generic endorsement.
...
Thus, the prevalence prior distributions of these features are hard to distinguish from properties that are just generally rare and distinctiveness (e.g., having extraordinarily beautiful feathers; see also our Expt. 2a, e.g., the distributions for *steals cars* vs. *climbs mountains*). 
@Cimpian2010 (Expt. 4) can be seen as evidence for this point: In that experiment, participants show a willingness to endorse both generics about striking properties (e.g., "Lorches have dangerous feathers") and distinctive properties (e.g., "Lorches have distinctive feathers") about equally as often each other and more so than neutral properties (e.g., "Lorches have purple feathers"). 
...
Those experiments did not measure the prevalence prior distribution, but we have found in pilot work that the prior distribution over the prevalence changes to resemble that of a distinctive property when participants are supplied information about the dangerousness of a property.

The second factor confounded with striking properties Leslie herself cites as motivation for their distinction.
This factor is that striking properties are projected more strongly (i.e., generalized to more instances of the category) than neutral properties.
...
This provides evidence for the influence of strikingness on *predictive prevalence*, which we posit is the second intermediate representation between conceptual knowledge and generic endorsements.
Thus, strikingness may influence endorsement by either (i) altering the prevalence priors or (ii) influencing speaker's predictions about future prevalence; we do not know of any evidence that shows an effect of strikingness above and beyond these mediating factors.
...
"Ks, by virtue of being the kind of things that they are, F" (e.g., "Dogs, by virture of being the kind of things that they are, are four-legged"); if such a sentence is not endorsed (e.g., "Barns, by virture of being the kind of things that they are, are red") while the simple bare plural is endorsed (e.g., "Barns are red"), then the property is thought to bear a *statistical connection* to the kind (a so-called *t-property*, for emphasizing that it is the *tokens* having the property which matters for generic endorsement).
@Prasada2013 showed that different kinds of generic statements (e.g., characteristic properties displayed by a minority of the kind, striking properties, etc., see our Expt. 1) support different kinds of inferences (e.g., *by virtue of*, *normative*: "Dogs are supposed to have four legs", *aspect*: "Having four legs is one aspect of being a dog"), and there is some evidence that the impact of principled connections on interpretations is separate from their influence on beliefs about prevalence [@Prasada2006]. 
...
Overhypotheses are naturally formalized as hierarchical Bayesian models, wherein a learner acquires knowledge at multiple levels of abstraction [e.g., learning from the same event about a particular dog, dogs in general, and animals in general; @Kemp2007a].
Incorporating a hierarchical model of kinds and properties into our model of generic language is an intruiging direction towards understanding the computational underpinnings of principled connections. 
Furthermore, we find it an exciting possibility that different conceptual structures give rise to roughly the same distributions on prevalence, while nonetheless listeners draw different inferences from generic statements [a la the differences observed for "prevalence-matched items" in @Prasada2006].
Such a result would not contradict our model, but which does require further elaboration of how those different conceptual structures relate to beliefs about prevalence. 
