...
Generics are just one case of generalization in language, however;  events can be described in generalizations (*habitual language*; e.g., "Mary swims after work.") as well as causality (*causal language*; e.g., "Staring at the sun makes you go blind.").
...
The language of generalization, however, does not make reference to entities in the world or concrete situations, but rather refer to unobservable concepts in the head.
...
Perhaps the simplest, quantitative semantic theory is a *threshold semantics*: an utterance is true if some quantity or *degree* is above a threshold.
...
fig.habituals.endorsement.vs.freq <- 
  ...
  geom_text_repel(data = d.hab.endorsement.bayes %>% 
