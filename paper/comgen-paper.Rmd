...
The informational content of the generalization is the prevalence in the referent category $p$.
...
In the next section, we work through different examples from the linguistics literature, describing how the prevalence prior and other relevant model components for each.
...
First, we examine the model behavior assuming a uniform prevalence prior $P(p)$, representing a purely abstract property for which an interpreter has no substantive prior knowledge (Figure\ \@ref(fig:uniformPriorSimulation))
The literal interpretation model (Eq. \ref{eq:L0}) computes a posterior distribution on prevalence by considering different possible thresholds $\theta$.
...
Roughly speaking, the uncertain-threshold interpretation model averages over these possibilities, resulting in a posterior interpretation distribution that favors higher prevalence levels because they are consistent with more thresholds (Figure\ \@ref(fig:uniformPriorSimulation)B).
The endorsement model then inverts the interpretation model and predicts higher rates of endorsement as the speaker's belief about the prevalence (referent-prevalence) increases (Figure\ \@ref(fig:uniformPriorSimulation)C). 
...
The uncertain-threshold model's interpretations are highly sensitive to background knowledge.
...
We are now ready to examine the endorsement model's predictions for different examples.
These examples have historically proved challenging for prevalence-based approaches. 
We describe how the uncertain-threshold model treats each in turn.

**1. Dogs bark.**

Often the first observation with generics is that they appear like *universal* quantifiers (e.g., "All") but permit exceptions. 
...
The endorsement model then predicts this is a rather good generic sentence (Figure\ \@ref(fig:simulations), bottom row) even though not all dogs bark (shown with arrow in Figure\ \@ref(fig:simulations) row 2).

**2. Kangaroos have spots.**

A second observation regarding generics is that they cannot be so lenient so to always convey *existential* quantification (e.g., "Some").
...
The reason is that, given the interpretation model's background knowledge about the property, the statement "Kangaroos have spots" is like "Dogs bark": it means *almost all* kangaroos have spots. 
...
A similar logic can be applied to this example, which is the converse of the affirmative generalization "Sharks eat people", initially introduced by @Leslie2007. 
Here, however, the prevalence of the feature for the referent category---the number of sharks that don't eat people---is very high: Almost all sharks don't eat people.
...
It is interesting to note that the model is less certain of its decision, predicting an endorsement probability around 40%.
...
Interestingly, the generic interpretation model returns roughly the same interpretation distribution for each hypothetical utterance: In each case, the model believes 50% have the property.^[
  This behavior might suggest a future area for model revision. 
...
Finally, the statement "Mosquitos carry malaria" is intuitively true in spite of the fact that the vast majority of mosquitos are malaria-free.
...
We return to this relationship in the General Discussion, and provide a precise mathematical derivation of the relationship between the prevalence prior and cue validity in Appendix A.
