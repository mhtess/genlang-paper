...
In fact, with prior knowledge suggesting against the existence of international elephants, our model interprets "Elephants live in Africa and Asia" as meaning that some elephants live in Africa and that different ones live in Asia.^[
...
Interestingly, an incremental parsing of the sentence  (i.e., upon hearing only the first part of the sentence that "Elephants live in Africa") leads our model to believe that *most* elephants live in Africa.
...
The present paper presents the first formal theory of the language of generalization that generates quantitative predictions about human behavioral data. 
Our work builds on and relates to a number of extant theories of generics in both the formal semantics and psychological literatures.
The main set of approaches from formal semantics try to describe unified, objective criteria by which to establish if a generic and other generalizations are true or false (i.e., generic *truth conditions*).
...
Many of these theories rely upon mechanisms outside of the truth conditions to make sense of the extreme flexibility exhibited by generics; unfortunately, these extra-semantic mechanisms are rarely described in any detail and never to the extent to make precise predictions.
Other theorists, primarily in philosophy and psychology, have taken the extreme flexibility of generics to argue against a quantitative theory based on prevalence, instead arguing that abstract, mental representations are directly tied to semantics of generics (e.g., *there is something about being a K which causes it to F*).
...
The most influential account in psychology comes from @Leslie2008, which has same starting point as our analysis: Generics express generalization. 
This analysis draws upon insights from the psychological literature on infant generalization to argue that generics tap into an innate, default mechanism that signals the child (or adult) to generalize the property to the kind.
<!-- ^[ -->
...
The conceptual view of generics argues it is not desirable to define generics in the same terms as quantifiers (e.g., a truth-functional threshold on prevalence), because generics are more fundamental than quantifiers [@Leslie2008; @Gelman2009].
For example, young children have a nearly adult-like understanding of generics far earlier than they do with quantified language [@Gelman2015].
...
The uncertain threshold model can be viewed as a way of formalizing what constitutes a default generalization.
For this model, the default generalization is a Bayesian belief updating mechanism via a threshold function whose threshold value is uncertain. 
...
This similar increase in endorsement for striking and distinctive properties suggests that the prevalence prior $P(p)$ distributions for the two are similar, and distinct from that of the neutral properties.
...
The second mechanism by which strikingness could influence generic enodrsement is that striking properties may be projected more strongly (i.e., generalized to more instances of the category) than neutral properties.^[
...
Interestingly, this enhanced projectibility holds for both dangerous properties (e.g., people doing criminal actions) as well as neutral but distinctive properties [e.g., people taller than 6'5"; @Rothbart1978, Expts. 2 & 3].
...
This is an intruiguing direction for future work.
...
Cohen's and other statistical theories employ upon an additional mechanism (not currently required in our account) to explain the context-sensitivity of generics: contextually restricting the entities that go into the computation of prevalence (i.e., which robins do we look at to compute the probability of laying eggs among robins?).
This mechanism is called *domain restriction* and is a common feature in many theories of generics. 
...
The mechanism of domain restriction may be implicit in our own theory: The prior distribution over the prevalence of *lays eggs* relies upon aspects of an intuitive theory of reproduction (i.e., that only females lay eggs).
...
To understand the properties of generics, this analysis draws analogy to other, inherently context-sensitive linguistic expressions: *indexicals* (e.g., "this" or "I").
...
The idea that our beliefs about what is normal in the world influences our judgments about generalizations has intuitive appeal for rejecting accidentally true generalizations (e.g., "Supreme Court justices have even social security numbers") and resisting stereotyped language (e.g., "Boys are good at math”).
...
Cue validity is believed to play a role in understanding generic language [@Leslie2007; @Khemlani2012; @Prasada2013], but the details remain obscure.
...
There are no correspondingly simple ways to lesion the other two components (property knowledge or speaker decision) while still producing a model that makes quantitative, context-sensitive predictions. 
...
\mht{should this be moved to the intro before other theories?}
