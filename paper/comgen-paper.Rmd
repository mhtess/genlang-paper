...
By sharing generalizations we flourish collectively without individually needing to taste each potentially-poisonous plant or witness enough lightning strikes.
...
In contrast to *particular statements* which describe individual entities or situations (e.g., "Rufus is friendly") and can be verified through observation, generic statements refer to inherently unobservable categories (e.g., the category of \textsc{dogs}) and convey information that extends beyond the present context, a fact which two-and-a-half-year-olds recognize [@Cimpian2008].
...
Generalizations are much than just generics, however; they can be found in statements about individuals (*habitual language*; e.g., "Mary swims after work.") as well as causality (*causal language*; e.g., "Staring at the sun makes you go blind.").
...
We do so by first describing the much more well-studied form of generalization: generalization from observations. 
The Bayesian probability calculus describes inductive generalizations in terms of probability, and this is a good model of human generalizations [@Shepard1987; @Tenenbaum2001].
...
The language of generalizations, however, does not make reference to entities in the world or concrete situations, but instead refers to concepts in the head.
...
Our model makes predictions about how endorsements should vary according to (i) the listener's world knowledge prior and (ii) the speaker's state of knowledge (or, belief distribution) that she seeks to communicate. 
In Section 3, we describe experiments the measure both quantities and relate them via our model to the endorsements of generalizations about categories, or *generic language*. 
In Section 4, we describe experiments that manipulate the speaker's state of knowledge in communicating generalizations about events, or *habitual language*.
In Section 5, we describe experiments that manipulate both the listener's relevant prior knowledge and the speaker's state of knowledge in communicating generalizations about causes, or *causal language*. 
...
Generalizations are used to make predictions about properties instances that an agent has yet to experience [@HumeTHN]. 
...
As adults we readily predict that a new dog will have fur, drinking a new cup of coffee will cause jitters, and a new day will find me typing.
...
Observing some number of instances of $k$ with $f$ will update a prior belief distribution into a posterior belief distribution.
As a concrete example, consider the feature \textsc{is friendly} ($f$) for the category \textsc{dog} ($k$): $h = P(x \in \{\text{is friendly}\} \mid x \in \{\text{is a dog}\})$.
If an agent were to encounter some number of individual dogs and note how many of them were friendly ($d$), using Bayes' rule, she could form a posterior belief distribution about the probability that the next dog she encounters will be friendly (in other words, a belief about the friendliness of dogs).
...
This subtlety can be modeled using heirarchical models to provide structure to the prior distribution in a Bayesian model [@Kemp2008].
Observations that are generated by an intentional agent can lead to similarly subtle inferences, like unconfounding otherwise ambiguous causal evidence [@Goodman2009].
...
Relying upon observations would be particularly problematic to learn, for example, about properties that are difficult to costly to observe (e.g., *starting at the sun makes you go blind*) or events that are statistically rare (e.g., *lightning strikes tall objects*).
...
One very simple semantic theory that has been used for describing quantified statements (e.g., "some", "all") is a *threshold semantics*.
...
$\delta_{\denote{u}(h, \theta)}$ is the Kronecker delta function that returns probabilities proportional to $1$ for prevalence values $h$ greater than threshold $\theta$, and $0$ otherwise. 
...
Though it is possible different $P(\theta)$ could be learned for different contexts, we assume this prior is identical across contexts and is completely uninformed by the context: $P(\theta) = \text{Uniform}(0, 1)$.

Given the likelihood "meaning" function defined in Eq. \ref{eq:delta} and the prior distribution over thresholds $\theta$, we can formalize a **listener model** by specifying a prior distribution over the prevalence $P(h)$:

\begin{eqnarray}
L(h, \theta \mid u) &\propto& {\delta_{\denote{u}(h, \theta)} \cdot P(h) \cdot P(\theta)} \label{eq:L0}
\end{eqnarray}

Equation \ref{eq:L0} is a model of a Bayesian listener $L$ who updates her beliefs about the prevalence $h$ according the meaning of an utterance $u$.
...
This component of the model provides a potential bridge to conceptual account of generics [e.g., @Leslie2007; @Prasada2006; @Prasada2013].
Critically in our model, this structure is in listener's knowledge of the world $P(h)$ and not in the semantic content of the utterance $u$. 
...
Following the Rational Speech Act (RSA) framework [@Goodman2016], we describe an **endorsement model** as a Bayesian speaker's decision of whether or not to say the generalization to the listener $L$ (Eq. \ref{eq:L0}).^[A more general version of this model can relax the assumption that the speaker has access to a specific value of prevalence $h$ that he is trying to communicate.
...
Additionaly, following RSA, the speaker model is assumed to be an approximately rational Bayesian agent with degree with rationality governed by parameter $\lambda$, which acts as a soft-max function.
...
These are not of primary theoretical interest to this paper, as *any* formal model that explains the very flexible endorsement patterns of generalizations in language will be a substantial advancement.
...
The RSA model defined in the previous section predicts that the endorsement of a generalization depends upon the probability of that utterance vs. silence accurately conveying to a naive listener a prevalence $h$ in a referent-category (e.g., the \% of robins that lay eggs).
...
In Figure \ref{fig:simulations} (row 2), we show a schematic prior distribution that is bimodal with peaks near 0\% and some high prevalence values; this sort of prior could be over the prevalence of the feature "barks" because many kinds of creatures do not bark (i.e., have $h \sim 0$; e.g., cats) and *for those that do have members that bark*, many of them bark (i.e., have some high $h$).
...
For being female, no matter want animal category a person might think of, the feature is prevalent in the exact same proprotion (i.e., \%50 of X are female; Figure \ref{fig:simulations} bottom row).
...
In other words, "Robins are female" has little very informational content as the listener's the posterior is nearly identical with her prior.
...
Here, we see that listener's posterior upon hearing the generalization (the uncertain threshold) is similar to what she learns about hearing "Some mosquitos carry malaria" (the fixed threshold; Figure \ref{fig:simluations} row 4). 
...
Adjusting 1 to $1- \epsilon$ leads to improper inferences for this model, as $1 - \epsilon$ is only likely under a highly right-skewed distribution; this would disproportionately influence the shape of $\mathcal{D}_{stable}$. 
