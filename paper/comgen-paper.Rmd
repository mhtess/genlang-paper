...
People readily predict that a new dog will have fur, drinking another cup of coffee will cause jitters, and a new day will find people that we know doing what they habitually do. 
...
The targets of our predictions can vary widely: They may be objects (a dog), events (a coffee drink), or more fine grained types (a person on a particular day of the week).
...
How do we come to know things? How does one acquire inductive beliefs?
...
Learning, then, can proceed via Bayes' Rule: $P(p_{fk} \mid d) \propto P(d \mid p_{fk}) \cdot P(p_{fk})$ given one's observational data $d$ and their beliefs about the generative process that resulted in the observed data given some prevelance (the likelihood: $P(d \mid p_{fk})$).

We do not always have access to observational data $d$.
...
Verbs like *causes* or *makes* also seem to convey generalization, in this case, about an intance of a causal event (*causal language*; e.g., "Staring at the sun makes you go blind"). ^[
  ...
  Still, however, the primary "generic" investigated in the literature are generalizations about categories ("Ks F").
]
Psychologists, linguists, and philosphers have long studied the language of generalization, as it appears very simple (e.g., syntactically) and yet challenging to formalize. 
...
Attempts of this kind have been made before in formal semantics [e.g., by positing that $\theta = 0.5$; @Cohen1999], but heretofore no model has been made to make precise, quantitative predictions about adult judgments. 
In addition to articulating an explicit semantic hypothesis, we present a Bayesian cognitive model that uses this semantic hypothesis in order to update its beliefs about the world. 
...
The simplest semantics for a generalization is a threshold function: $\denote{gen}(p_{fk}, \theta) = \{p_{fk} >\theta\}$, but what is the semantic variable $\theta$?
Quantifier statements can be described using particular values of $\theta$: $\denote{some}(p_{fk}) = \{p_{fk} > 0\}; \denote{all} = \{p_{fk} = 1\}$.
The extreme flexiblity of generalizations (e.g., "Mosquitos carry malaria"; "Birds lay eggs" vs. "Birds are female") suggests that no threshold would suffice. 
Indeed, no particular threshold would suffice: The semantics of the language of generalizations is *underspecified*.
...
We can formalize this underspecified semantics in a Bayesian cognitive model, where the underspecifed threshold $\theta$ is sampled from a uniform prior distribution over possible thresholds $P(\theta)$ [cf., @Lassiter2013; @Lassiter2015].
This model also has prior knowledge about the property $P(p_f)$ and so literal interpretation is given by updating these prior beliefs according the (underspecified) truth-functional meaning of utterance $u$:
...
Given the above model of literal interpretation described above, how do we decide when a statement conveying a generalization is true or not?
...
$S$ does not know $\theta$ and integrates over the interpreter posterior beliefs about $\theta$.
We assume the decision to endorse is approximately rational, with degree of rationality given by $\lambda$.
...
The uncertain, truth-conditional threshold $\theta$ is resolved by the literal interpretation model $Lit$ (Eq. \ref{eq:L0}) by integrating the model's prior beliefs about the feature in question---the *prevalence prior* $P(p_fk)$---with the truth-functional semantics.
...
However, the interpretation model's *a priori* beliefs $P(p_fk)$ must be considered before the endorsement model can say whether or not it would endorse the statement. 
...
The literal interpretation model computes a posterior distribution on prevalence by considering different possible thresholds $\theta$ (Figure\ \@ref(fig:simulations)B).
Low thresholds (e.g., $\theta = 0.1$) rule out very few possibilities (only $p_{fk} \leq 0.1$), and thus are quite likely given that its consistent with much of the prevalence prior.
The flip-side of this is that the endorsement model is less likely to produce an utterance with a low-threshold in mind, because the end result would be similar to staying silent (a posterior not very different from the prior).
...
The prevalence-based interpretations of generalizations of novel categories are shown in Figure\ \@ref(fig:simulations)C.
These interpretations depend heavily on the prevalence priors (different rows).
Hearing "Ks bark" tells the literal interpretor that *almost all* Ks bark, though there is a good deal of uncertainty in the precise quantity (top row; pink).
"Ks lay eggs" has a strikingly different interpretation: It means "Half of Ks lay eggs" (4th row; green).
The literal interpreter can also derive very weak interpretation, as shwon in the  \textsc{carries malaria} distribution (bottom row, purple).

The endorsement model (Eq. \ref{eq:S1}) predicts endorsement probabilities based on the literal interpretation of the utterance and the referent-category prevalence (e.g., how many robins lay eggs).
Predictions for a few classic examples from the generics literature are shown in (Figure\ \@ref(fig:simulations)D).
...
To understand this prediction, examine the interpretation distribution for the \textsc{are female} distribution: It is identical with the prior (Figure\ \@ref(fig:simulations) A vs. C; blue distributions).
...
These simulations show how the model predicted endorsement depends upon the background knowledge about the property $P(p_f)$ as well as the prevalence $p_{fk}$ communicated. 
We chose schematic values for each of these, and saw that the endorsement model recapitulates standard intuitions about the truth or falsity of classic examples from the generics literature.
