...
It is the premier case study of generalizations in language and has received the most attention from philosophers, linguists, and psychologists.
...
We do this by measuring the prior belief distribution over the relevant probabilities $P(h)$ used in Eq. \ref{eq:L0} as well as the target-category prevalence $h$ that the speaker model seeks to convey in Eq. \ref{eq:S1} (Expt. 1a).
...
In particular, we hypothesize that different kind--property relations [e.g., *principled* v. *accidental relations*^[This is sometimes referred to as "statistical" relations.]; @Prasda2013] can be observed in the prevalence prior. 
If, in fact, the prevalence prior is one manifestation of deeper, conceptual knowledge, we would expect the prevalence prior data to follow a multi-modal, mixture distribution where there exist qualitatively modes or components of the prior, the results of the different types of kinds with respect to the property.
This is important because it is one way in which our formal theory connects to other, informal psychological accounts of generic language [cf., @Prasada2013; @Gelman2003; @Leslie2007].
