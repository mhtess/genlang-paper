...
```{r habituals-predictive-model-run, cache = T}
...
```{r habituals-predictive-forwardSample}
...
```{r habituals-predictive-bda-figure}
...
Figure \ref{fig:habituals-predictive-figure} (left) shows the Maximum A-Posteriori predicted future frequency as a function of the past frequency given to the participant and the type of causal information given. 
...
# for (i in seq(1, n_chains)){
#   mi <- fread(paste(project.path,  "models/habituals/results/",
#                     model_prefix, "targetH_predictiveMean-",
#                     n_samples, "_burn", burn, "_lag", lg, "_chain", i, ".csv", sep = ""))
...
load(file = paste(project.path,  "models/habituals/results/", model_prefix, "predictive-",
                    n_samples, "_burn", burn, "_lag", lg, "_",n_chains , "chains.RData", sep = ""))


load(file = paste(project.path,  "models/habituals/results/", model_prefix, "past-",
                    n_samples, "_burn", burn, "_lag", lg, "_",n_chains , "chains.RData", sep = ""))
...
```{r habituals-predictive-figure, fig.cap="A: Results of Expt 3a. B: Results of Expt. 3b as they relate to the predicted frequencies from Expt. 3a, C & D: Endorsement models based on past frequency (C) and predictive frequency (D).", fig.width = 9, fig.height = 9}
...
To formalize the predictive frequency speaker model, we use the predictive frequency data (Expt. 2c) as the $h$ that the speaker model $S$ (Eq. \ref{eq:S1}) is aiming to communicate. 
...
We analyze this model in the same Bayesian data analysis regime as for our previous models, integrating the simple Bayesian analysis of the predictive frequency data (Expt. 2c; for the predictive model only) with the joint model of the priors (Expt. 2a) and the new endorsement data (Expt. 2d).
We use the same priors over the parameters as before, and learn about the posterior distribution by collecting three independent MCMC chains of N iterations (removing the first M for burn-in). 
Figure \ref{fig:habituals-predictive-figure} C \& D shows the resulting model predictions for the past frequency and the predictive frequency speaker models. 
...
In Experiments 2c \& 2d, we further investigated the nature of the underlying “propensity” scale by introducing enabling and disabling causal evidence, measuring the predicted future frequency (Expt. 2c) and using that, with the model, to predict the felicity of habitual sentences (Expt. 2c). 
...
In Experiment 3a, we manipulate participants' beliefs about a causal system and measure their updated beliefs about system, serving a manipulation check.
