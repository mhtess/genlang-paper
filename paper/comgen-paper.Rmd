...
abstract: > 
    Generalizations are central to human understanding and language provides simple ways to convey generalization (e.g., "Dogs bark").
    ...
    The major issue in formalizing generalizations in language is in determining what which such statements are true or false.
    ...
    Across these diverse domains, we find that our model explains human endorsements of generalizations in language, while simpler models do not.
    ...
    The results suggest that central phenomena of genericity emerge from the interaction of diverse prior beliefs about properties with general communicative principles. 
...
Knowing that an object tends to have a property, an entity tends to exhibit a behavior, or a cause tends to produce an effect can be necessary to thrive in our open-ended world.
...
It thus is crucial that language allow us to communicate such *generalizations* to each other.
By sharing generalizations, we flourish collectively without individually needing to taste  potentially-poisonous plants or personally witness enough lightning strikes.
...
Generics are just one case of generalizations in language, however;  individuals can be described in generalizations (*habitual language*; e.g., "Mary swims after work.") as well as causality (*causal language*; e.g., "Staring at the sun makes you go blind.").
...
Despite their ubiquity and relative simplicity, there have been few formal models of generic language and none that have made precise, quantitative predictions about observable behavior.
A formal model in this domain is useful to make explicit the relevant assumptions of a psychological or linguistic hypothesis for predicting human behavior; this, in turn, makes the theory more falsifiable. 
The major challenge in formalizing generalizations in language is determining what makes such statements true or false.
...
These observations have led to proposals that the statistics of the feature (e.g., the prevalence or probability of a member of category having a property) is only incidentally related to generic meaning. 
...
From this perspective, the statement "Bishops move diagonally" is true not because most bishops move diagonally, but rather because those are the rules of chess.
With this perspective, @Leslie2007 proposes that generics express a cognitively primitive, default generalization. 
The *generics as default* view argues that certain principles explain how generics become true or false.
"Mosquitos carry malaria" conveys a dangerous or *striking property*, which is useful to know if you are to survive in a hostile world.
This theory correctly predicted that generics that convey striking properties would be true even if the property was relatively uncommon [e.g., "Sharks attack swimmers"; @Cimpian2010; @Prasada2013].
"Robins lay eggs" can be thought of as conveying a *characteristic property*, which permits its felicity despite only 50\% having the feature [@Prasada2006; @Prasada2013].

Yet, generics are not limited to conveying rich, conceptual relations. 
Arbitrary relationships can also be expressed with generics (e.g., "Barns are red"; "Ravens are bigger than toasters") which seem only to be true only because of the statistics of the world [@Nickel2008; @Nickel2016; @Sterken2015].
The *generics as default* view must then be supplemented with the principle that, in certain situations, statistics influence generic endorsement.
The diversity of generic statements highlighted by the conceptual accounts is interesting, but insofar as generics are a single, unified phenomenon, there should be a general, unified principle that explains what makes the statements true or false.
...
We propose a formal, unified account of generalizations in language that can make precise, quantitative predictions about these simple and ubiquituous utterances.
We do so by first reviewing the much more well-studied form of generalization: generalization from observations. 
The Bayesian probability calculus describes inductive generalizations in terms of probability, and this is a good model of human generalization [@Shepard1987; @Tenenbaum2001].
We then formalize how these probabilities are communicated, using a rational model of communication --- the Rational Speech Act theory [@Frank2012; @Goodman2013; @Franke2015; @Goodman2016].
...
Heretofore, the fragments of language formalized in such models have been limited to *particular statements* that convey information about a specific entity or situation [e.g., in hyperbole: "My watch cost a million dollars"; @Kao2014, or vagueness: "John is tall"; @Lassiter2013].
The language of generalizations, however, does not make reference to entities in the world or concrete situations, but rather refer to unobservable concepts in the head.

Specifically, we explore the hypothesis that the core meaning of a generic statement and other language for conveying generalizations concerns subjective probability in a *simple but underspecified* way.
We formalize this hypothesis in a probabilistic model of language understanding, and derive predictions for the endorsements of such statements by considering whether or not a speaker would say the generalization to a listener.
We show that this decision depends in systematic ways on the prior knowledge of the listener.
...
In Section 2, we formalize the hypothesis that the core of meaning of a generalization is *simple but underspecified* and incorporate this *uncertain semantics* into a framework for pragmatic language understanding -- the Rational Speech Act framework.
...
In Section 3, we describe experiments that measure both quantities and relate them via our model to the endorsements of generalizations about categories (*generic language*). 
In Section 4, we describe experiments that manipulate the speaker's state of knowledge in communicating generalizations about events (*habitual language*).
In Section 5, we describe experiments that manipulate both the listener's relevant prior knowledge and the speaker's state of knowledge in communicating generalizations about causes (*causal language*). 

Across all of our experimental settings, we find a very strong agreement of our model's predictions to the full spectrum of human elicited endorsements (cumulative $r^2(N) = 0.XX$), where simpler models fall short.
In the final section of the paper, we discuss how our model paves a path to unifying the extant psychological, linguistic, and philosophical literatures on *genericity*.
Our theory also opens up a number of new directions for understanding generalizations in language and building models that learn from language in the way that people learn from language.
...
If an agent were to encounter some number of individual dogs and note how many of them bark ($d$), using Bayes' rule, she could form a posterior belief distribution about the probability that the next dog she encounters will bark (in other words, a belief about the barkiness of dogs).
...
If generalization from observations can be described using probability, then it seems  natural to consider the semantic content of an utterance that conveys generalization also to be relating to probability. 
One very simple semantic theory is a *threshold semantics*: an utterance is true is some quantity or *degree* is above a threshold.
...
When there is a latent parameter the Bayesian scientist knows little about, she puts a prior distribution over the parameter and infers the parameter's likely value by collecting data.
...
Given the semantic likelihood function defined in Eq. \ref{eq:delta} and the prior distribution over thresholds $\theta$, we can formalize a **listener model** by specifying a prior distribution over feature-probabilities $P(h)$:
...
This component of the model provides a potential bridge to conceptual accounts of generics [e.g., @Leslie2007; @Prasada2006; @Prasada2013].
Contrary to these accounts, in our model, conceptual structure is represented in listeners' knowledge of the world $P(h)$ and not in the semantic content of the utterance $u_{gen}$.
...
Following the Rational Speech Act (RSA) framework [@Goodman2016], we describe a **speaker model** as a Bayesian decision of whether or not to say the generalization to the listener $L$ (Eq. \ref{eq:L0}).^[A more general version of this model can relax the assumption that the speaker has access to a specific feature-probability $h$ that he is trying to communicate.
...
The speaker's decision to say an utterance (e.g., the generalization) is determined by how well it would communicate the **referent feature-probability** $h$ to the listener $L$, in comparison to alternative utterances.
We specify a single alternative utterance corresponding to *not saying the generalization*:
...
To our knowledge there is no unifying computational account of linguistic genericity and as such, the primary theoretical interest of this paper is to outline such an account, rather than discuss possible variations within it. 
...
Participants first were shown six animal categories randomly sampled from a set corresponding to the generic sentences used in Expt. 1b (e.g., \textsc{robins, mosquitos}) and were asked to generate five of their own.
...
```{r generics-endorsement-figure, fig.width=12, fig.height=9.5, fig.cap="Endorsing generalizations about categories. Top left: Human elicited endorsements for thirty generic sentences reveal a continuuum of endorsements. Top right: Model fits for the uncertain semantics speaker model (upper right), a fixed semantics speaker model (upper left), and regression models based on referent feature-probability alone (lower left) and feature-probability + cue validity (lower right). Bottom: Five example feature-probability priors, listener posteriors upon hearing the generalization, and speaker belief distributions about the referent feature-probability. These distributions are inferred using all three data sources from Expt. 1 (see Figure \\ref{fig:genericsModelDiagram} for an overview of the Bayesian data analytic approach)."}
...
```{r habituals-endorsement-figure, fig.width=11, fig.height=8, fig.cap="Endorsing generalizations about events Top left: Human elicited endorsements for ninety-three habitual sentences as a function of the referent (log) frequency of action. Top right: Model fits for the uncertain semantics speaker model (upper right), a fixed semantics speaker model (upper left), and regression models based on referent (log) frequency alone (lower left) and referent log frequency + distinctiveness (lower right). Bottom: Five example frequency priors and listener posteriors upon hearing the generalization. These distributions are inferred using both data sources from Expts. 2a & 2b.  (analagous to the Bayesian data analytic approach outlined in Figure \\ref{fig:genericsModelDiagram})"}
