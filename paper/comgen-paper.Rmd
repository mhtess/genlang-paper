...
Knowing that an object tends to have a property, an entity tends to exhibit a behavior, or a cause tends to produce an effect can be crucial to trhiving in our open-ended world.
Yet such knowledge can be difficult to acquire: Relevant observations may be costly (e.g., learning that a plant is poisonous) or rare (e.g., understanding the properties of lightning).
It thus is crucial that language allow us to communicate such *generalizations* to each other.
By sharing generalizations we thrive collectively without individually needing to taste each potentialy-poisonous plant or witness enough lightning strikes.
Furthermore, sharing generalizations from one generation to the next may be a crucial mechanism behind the cultural ratchet \ndg{cite. or remove if you don't like it here}.
...
Generalizations in language are often studied in *generic language* or generics.
Generic language is ubiquitous in everyday conversation as well as in child-directed speech [@Gelman2008]. 
Around the age of two and a half, children begin to understand that generic language refers to categories and conveys information that extends beyond the present context [@Cimpian2008].
It is believed that every language can express generic meaning [@Behrens2005; @Carlson1995], and such generalizations in language are thought to be central to the growth of conceptual knowledge [@Gelman2004].
...
Individual entities can also be talked about in generalization, using so-called *habitual language* (e.g., "Mary swims after work.").
...
Communicating about causality can also take the form of communicating a generalization about a causal event (*causal language*; e.g., "Drinking moonshine makes you go blind.").
...
\ndg{prev three pars are floppy. tighten.
There haven't been many formal models of generic language, none have made quantitative predictions. Say why this is important.
...
So, the generalization should signal something weaker like *Most* dogs bark.
<!-- Indeed, many generalizations seem to have the quantificational force of *most*. -->
However, *Mosquitos carry malaria* is intuitively a true statement, despite less than 1\% of moquitos actually carrying malaria.
...
That same half of robins, however, also lay eggs, and *Robins lay eggs* is again intuively reasonable. 
...
Rational models of communication [@Frank2012; @Goodman2013; @Franke2015; @Goodman2016] have had a lot of recent success in formalizing complex linguistic phenomena ranging from politeness [@Yoon2016] and hyperbole [@Kao2014] to metaphor [@Kao2014] and vagueness [@Lassiter2013; @Qing2014]. 
Such models of rational communication describe the inferences listeners can draw from language, and the utterances speakers should say given their communicative goals. 
Heretofore, however, the fragments of language under investigation have been limited to utterances that convey information about a specific entity or situation, about the here and now.
Meanwhile, the language of generalizations don't make reference to entities in the world or concrete situations. 
They refers to concepts in the head and have proved tricky to formalize.

In this paper, we explore the hypothesis that the core meaning of a generic statement and other language for conveying generalizations is *simple but underspecified*.
...
In Section 1, we describe the Bayesian framework for generalization and define the scales along which generalization is measured in our three case studies. 
