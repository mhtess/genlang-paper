...
abstract: > 
    Generalizations are central to human understanding and language provides simple ways to convey them (e.g., "Dogs bark").
    ...
    The major issue in formalizing generalization in language is in determining what which such statements are true or false.
    ...
    To test this theory, we examine endorsements of generalizations about three different domains: generalizations about categories (*generic language* e.g., "Dogs bark"), events (*habitual language* e.g., "John runs"), and causes (*causal language* e.g., "Staring at the sun makes you go blind"). 
    Across these diverse domains, we find that our model explains almost all of the variance in human endorsements of generalizations in language, while simpler models fall short.
    This is the first formal theory that makes precise, quantitative predictions about human understanding of generalizations in language.
    The results suggest that central phenomena emerge from the interaction of diverse prior beliefs about properties with general communicative principles. 
...
Generalizations in language are often studied in *generic language* (or, generics): simple statements that convey generalizations about categories [e.g., "Dogs are friendly"; @Carlson1977; @Cohen1999; @Leslie2007; @Nickel2008].
Generics are ubiquitous in everyday conversation as well as in child-directed speech [@Gelman2008]. 
In contrast to *particular statements* (e.g., "Rufus is friendly") which describe individual entities or situations and can be verified through observation, generic statements refer to inherently unobservable categories (e.g., the category of \textsc{dogs}) and convey information that extends beyond the present context, a fact which 2-and-a-half-year-olds already appreciate [@Cimpian2008].
...
Generics are just one case of generalization in language, however;  events can be described in generalizations (*habitual language*; e.g., "Mary swims after work.") as can causality (*causal language*; e.g., "Staring at the sun makes you go blind.").
...
Despite their ubiquity and relative simplicity, there have been few formal models of generic language and none that have made precise, quantitative predictions to match human intuitions. 
The major challenge in formalizing the language of generalizations is in determining what makes such statements true or false.
When we hear "Dogs bark", it sounds like a universal statement, like *All* dogs bark.
But the property doesn't apply to all dogs: Basenji dogs are barkless^[
...
However, only a tiny fraction of moquitos carry malaria, while the statement "Mosquitos carry malaria" is intuitively true.
...
But weakening the generalization to *some* goes too far: Half of robins are female, but saying "Robins are female" seems odd.
That same half of robins (i.e., the females), however, also lay eggs, and "Robins lay eggs" is again intuively true. 

These observations have led to proposals that the prevalence or probability of a member of a category having the property (e.g., $P(x \text{ barks} \mid x \text{ is a dog})$) is only tangentially related to generic meaning. 
...
From this perspective, the statement "Bishops move diagonally" is true not because bishops tend to move diagonally, but rather because those are the rules of chess.
With this in mind, @Leslie2007 proposes *cognitively primitive* principles that dictate how generics become true or false.
*Characteristic* features are one such principle; "Robins lay eggs" is thus true because it is the mode of reproduction, and reproduction is a characteristic dimension for animal kinds.
*Horrifying* or *striking* information also leads to true generics, which is why "Mosquitos carry malaria" is true despite its low prevalence.

Yet others have observed that generics are not limited to conveying rich, conceptual relations [@Nickel2008; @Nickel2016; @Sterken2015]. 
<!-- Arbitrary relationships are also expressed with generics:  -->
"Barns are red" is true because that is how barn-owners tend to paint their barns; if barn-owners decided to switch to blue, the things they would be painting would still be considered barns.
"Ravens are bigger than toasters" because of the size of ravens and the size of toasters, not because of a special relationship between ravens and toasters.
*Conceptual accounts* must then be supplemented with the proviso that, in certain situations, the statistics of the world do influence human intuitions about generics.

Needless to say, generic statements are a diverse class, and the conceptual view highlights this diversity [e.g., @Prasada2013].
But insofar as generics are a single class in language---indeed, insofar as they are a subclass of generalization in language---there should be a general, unified principle that explains what makes the statements true or false.
The same logic should apply to all generalizations in language, including habitual and causal language.
...
In this paper, we take a critical look at the notion that prevalence---or its more general formulation, probability---cannot serve as the core of a semantic theory of generalization in language (including generics).
We strive for a probability-based semantics for its generality: Probability is the universal currency of belief and serves as a useful representation for human generalization from observations [@Shepard1987; @Tenenbaum2001].
Given that certain probabilities are the *result* of inductive generalization, it would make sense that these are the *object* of communication for generalizations. 
We formalize such a semantics in a general probabilistic framework for modeling cognition, and thus incorporate generalizations in language into models designed to formalize concepts and intuitive theories [@Goodmanconcepts].
Formalizing such a theory is scientifically useful as well to make explicit the assumptions of the psycho--linguistic hypothesis meant to match human intuitions; this, in turn, makes the theory more testable. 

We take the primary desideratum for a theory of the language of generalization to be adult human intuitions into the truth or felicity of statements conveying generalization, which we measure empirically. 
To test the theory that probability can serve as the core semantic theory, we build a computational model of endorsement around this core semantic theory and compare our model to human judgments.
Though concepts such as *striking properties* are not explicitly formalized in our model, there is a natural avenue for future integration as our theory is couched in a general cognitive modeling framework.

Specifically, we formalize an account where the core meaning of a generalization in language is a *simple but underspecified* function of probability.
...
We show that the decision to endorse depends in systematic ways on the prior knowledge of the agent who hears the generalizations, the listener.
We test this theory in three distinct case studies, by both measuring and manipulating listener prior knowledge, showing that it is strongly and causally related to the endorsement of generalizations in the ways predicted by our model.
Our three case studies correspond to generalizations about categories, events, and causes.
...
In Section 1, we describe the Bayesian framework for generalization from observations, serving as the foundation for our semantic theory.
In Section 2, we formalize the hypothesis that the core of meaning of a generalization is *simple but underspecified* and incorporate this *uncertain semantics* into a computational model of endorsement.
<!-- To test our model, we examine endorsements of generalizations in three domains: categories, events, and causes. -->
<!-- Our model makes predictions about how endorsements should vary according to (i) the listener's prior world knowledge and (ii) the speaker's state of knowledge (or, belief distribution) that she seeks to communicate.  -->
In Section 3, we describe experiments that measure both listener prior knowledge and the prevalence of the properties in question, and relate them via our model to the endorsements of generalizations about categories (*generic language*). 
In Section 4, we describe experiments that measure listener prior knowledge and manipulate the frequency of various events to explain endorsements of generalizations about events (*habitual language*).
Finally, in Section 5, we describe experiments that manipulate both the listener's prior knowledge and the prevalence of the properties in question in endorsing generalizations about causes (*causal language*). 

Across all of our experimental settings, we find a very strong agreement of our model's predictions to the full spectrum of human elicited endorsements (cumulative $r^2(N) = 0.XX$).
...
<!-- (2) It also allows us to consider sources of gradience in behavior, determining whether or not we should allocate it to *noise* in the observed data or to *uncertainty* in the head of the language user. -->
<!-- *Causal language* (e.g., "The block *makes* it [the machine] go.") faciliates more abstract forms of causal reasoning [@Bonawitz2010]. -->



<!-- \ndg{There haven't been many formal models of generic language, none have made quantitative predictions. Say why this is important. -->
<!-- To build one, we need to firsts say what a generalization is. We use the Bayesian idea that an inductive generalization can be understood in terms if probbilities. -->
<!-- Then we need to say how these probabilities are communicated. Use RSA...} -->




<!-- Rational models of communication have had a lot of recent success in formalizing complex linguistic phenomena ranging from politeness [@Yoon2016] and hyperbole [@Kao2014] to metaphor [@Kao2014] and vagueness [@Lassiter2013; @Qing2014].  -->
...
<!-- Generics roughly convey that the properties is widespread in the category (e.g., *Most dogs are friendly*), however they don't always imply the same degree of prevalence. -->
<!-- *Birds are female* sounds strange even though half of birds of female.  -->
...
People readily predict that a new dog will have fur, drinking a new cup of coffee will cause jitters, and a new day will find me typing.
In each case, we assign a specific exemplar $x$ to a category $k$, and make a prediction that it will have property $f$. 
The entities can be objects (a dog), events (a coffee drink), or more fine grained types (me on a given day).
The properties can vary greatly (e.g., having fur, causing jitters, typing). 
Yet the prediction for a new entity can be described as a *predictive probability*, which we will denote throughout the paper as $h$ and refer to as the **feature-probability**: 
$h = P(x \in f \mid x \in k)$, the probability that $x$ will have $f$ given that it is in $k$.^[
We denote feature-probability as $h$ to emphasize that it is a latent *hypothesis*, and not necessarily an observable frequency or prevalence.
]
...
<!-- Particular statements can be made about objects, events, and causes. For example: -->

<!-- 1. My dog is friendly. (object) -->
<!-- 2. John ran this afternoon. (event) -->
<!-- 3. My grandfather drank moonshine and it made him go blind. (cause) -->

<!-- Particular statements refer to individuals of a category of which generalizatons can be made. For example, -->

<!-- 4. Dogs are friendly -->
<!-- 5. John runs -->
<!-- 6. Drinking moonshines makes you go blind -->

<!-- That is, *dog* is an instance of the category \textsc{dogs}, *John at a particular time* is an instance of the category of \textsc{john}, and *drinking moonshine at a particular time* is an instance of the category of \textsc{drinking moonshine} (see Table \ref{tab:genpart}). -->
...
The Bayesian probability calculus provides a computational description of generalization [e.g., @Tenenbaum2006; @Tenenbaum2011]: Observing some number of instances of $k$ with $f$ will update what an observer believes *a priori* (formally: a prior belief distribution) in order to make predictions about unobserved instances, via a posterior belief distribution.
As a concrete example, consider the feature \textsc{barks} ($f$) for the category \textsc{dog} ($k$): $h = P(x \in \{\text{barks}\} \mid x \in \{\text{is a dog}\})$.
If an agent were to encounter some number of individual dogs and note how many of them bark ($d$), using Bayes' rule, she could form a posterior belief distribution about the probability that the next dog she encounters will bark (in other words, a belief about the barking capacity of dogs).
...
\ndg{i'm not sure that generalization from examples should have it's own section, as opposed to be a subsection or couple pars of the next...}

# Generalization from Language

If generalization from observations can be described using probability, then it seems  natural to consider the semantic content of an utterance that conveys a generalization to be related to probability. 
If we wish to connect to the semantic tools of *truth values* \ndg{cite montague?}, then we will need to connect the scalar quantity of probability to a boolean truth value.
Perhaps the simplest way to connect a scalar quantity to a truth value is with a *threshold semantics*: an utterance is true if the underlying quantity is above a threshold.
For generalizations, the threshold would apply to the *feature-probability* $h$:^[
We emphasize that our account is based on an agent's subjective probability $h$. The literature on generic language has often examined *prevalence*, or the proportion of members of a category with the property, as the primary quantitative metric that could relate to generic meaning (in addition to *cue validity*, described below). A Bayesian agent, however, would only use prevalence as a way to update her existing state of knowldege to form a subjective probability for future use. Thus, we consider the semantic content of a generalization to be related to probability, not necessarily prevalence.
]

\ndg{why are there any deltas yet? so far we only want a truth functional semantics, so something like $\denote{u_{gen}} = h>\theta$. then we use the delta in eq:L0.}

\begin{eqnarray}
\delta_{\denote{u_{gen}}(h, \theta)} & = & P(u_{gen} \mid h, \theta) \label{eq:delta} \\ 
&\propto  &
\begin{cases}
1 & \text{if } h > \theta \\
0 & \text{otherwise}
...
Following standard treatment from formal semantics, we represent the semantic content of an utterance by the Kronecker delta function  $\delta_{\denote{u}(h, \theta)}$, which here returns probabilities proportional to $1$ for feature-probabilities $h$ greater than threshold $\theta$, and $0$ otherwise. 
Logical quantifiers can be described using such a semantics (i.e., *some* is $h > 0$, all is $h = 1$).

What threshold $\theta$ should be used for the generic generalization?
In Bayesian statistics, when there is a latent parameter that little is known about, one puts a prior distribution over the parameter and infers the parameter's likely value given any data.
In a similar vein, we posit that the meaning of a generalization in language is *underspecified*; that is, there is not a fixed $\theta$ that corresponds to the generalization, but rather the listener has uncertainty over $\theta$---in the form of a prior distribution $P(\theta)$---and infers $\theta$ after hearing an utterance.
Though it is possible that different $P(\theta)$s could be learned for different contexts, we assume this prior is identical across contexts and is completely uninformed by the context: $P(\theta) = \text{Uniform}(0, 1)$.

Given the semantic function defined in Eq. \ref{eq:delta} and the prior distribution over thresholds $\theta$, we can formalize a **listener model** by specifying a prior distribution over feature-probabilities $P(h)$:

\begin{eqnarray}
L(h, \theta \mid u) &\propto& {\delta_{\denote{u}(h, \theta)} \cdot P(h) \cdot P(\theta)} \label{eq:L0}
\end{eqnarray}

Equation \ref{eq:L0} is a model of a Bayesian listener $L$ who updates her beliefs about the feature-probability $h$ according to the meaning of an utterance $u$. \ndg{i don't like describing L0 as a bayesian agent, because this is not an instance of Bayes' rule (because the semantics doesn't provide a likelihood, but only a weight)}
$\delta_{\denote{u}(h, \theta)}$ is the likelihood term in Bayes' Rule and represents the literal semantic content of an utterance.
$P(h)$ represents the listener's prior world knowledge concerning the relevant feature-probabilities $h$.
The particular form of the **feature-probability prior** $P(h)$ plausibly depends upon the kind of property or event in question and could be structured as a result of deeper conceptual knowledge about properties and events [@Griffiths2005; @Tenenbaum2006; @Kemp2008].
In our model, conceptual structure is represented in listeners' knowledge of the world $P(h)$ and not in the semantic content of the utterance $u_{gen}$.
This is a different approach than in conceptual accounts of generics [e.g., @Leslie2007; @Prasada2006; @Prasada2013], but provides a potential bridge, as explored later.

<!-- Our proposal that generic language and other forms of communicating generalizations convey information about probabilities that underly generalization and prediction implies that the conceptual factors of the kind discussed by Prasada, Leslie, et al. influence the meaning of a generic statement via their influence on subjective probability. -->
<!-- Flexible, structured models of the world have been shown to do just that in the case of generalizations from observations, merging structured representations with probabilisitic inference [e.g., @Kemp2008; @Goodmanconcepts]. -->
...
In this paper, we are interested in explaining the variable *endorsements* of generalizations (e.g., "Robins lay eggs" is good; "Robins are female" is not) based on some known or believed feature-probability $h$ in a referent category (e.g., only female robins lay eggs).
Following the Rational Speech Act (RSA) framework [@Goodman2016], we describe a **speaker model** as a Bayesian decision of whether or not to say the generalization to the listener $L$ (Eq. \ref{eq:L0}). \ndg{prabably should say something more about modeling endorsements as a speaker model...}
...
The speaker $S$ doesn't know the threshold $\theta$; rather, he integrates over the likely values that the listener will believe. 
Additionaly, following RSA, the speaker model is assumed to be an approximately rational Bayesian agent with rationality governed by parameter $\lambda$, which acts as a soft-max function.

\ndg{i wonder if we shoud start with the speaker / endorsement idea? introducing the listener and semantics might be more natural then.}

The speaker's decision to say an utterance (e.g., the generalization) is determined by how well it would communicate the **feature-probability** $h$ to the listener $L$, in comparison to alternative utterances.^[A more general version of this model can relax the assumption that the speaker has access to a specific feature-probability $h$ that he is trying to communicate.
Rather, the speaker may have a belief distribution over $h$ for the category $k$, corresponding to the speaker's beliefs about the feature-probability of the property in the category. 
...
We specify a single alternative utterance corresponding to *staying silent*^[
...
With just these two alternatives (say the generalization vs. not), the model can be interpreted as a model of felicity judgments or sentence *endorsement* [@Degen2014]. \ndg{why?}
Because staying silent carries no information content to the listener, it can be modeled as a uniform utterance:
\begin{equation*}
\denote{u_{silence}(h, \theta)} \propto \text{True}
\end{equation*}
\ndg{i simplified the eq... ok?}


The pair of speaker-listener models presented above correspond to the simplest communicative models with an underspecified threshold-meaning for a generalization. 
...
The following simulations were implemented using the R package RWebPPL in the RMarkdown document that generated this paper text and figures.
...
The RSA model defined in the previous section predicts that the endorsement of a generalization depends upon the probability that utterance vs. silence accurate conveys to a naive listener a feature-probability $h$ in a referent-category (e.g., the \% of robins that lay eggs).
...
For these simulations, we had specify the feature-probability priors $P(h)$.
...
Generalizations about categories (i.e., *generic language*; e.g., "Dogs bark.") have received substantial interest in psychology for their role in concept and theory formation [e.g., @Gelman2004], stereotype propogation [@Rhodes2012], and motivation [@Cimpian2007], as well as many other facets of everyday cognition.
It is the premier case study of generalizations in language because of its close relation to quantified statements (e.g., "Most dogs bark") and has received the most attention from philosophers, linguists, and psychologists.

Any theory of generic language must explain its puzzling flexibility of usage with respect to feature-probability.
That is, "Mosquitos carry malaria" and "Robins lay eggs" are reasonable things to say, but "Robins are female" is not.
As a first test of our theory, we wish to explain the flexibility in endorsements of generics in terms of the relevant prior knowledge about properties, as explored through simulation in the **model simulations** section.
We validate these simulations empirically by measuring the prior belief distribution over the feature-probabilities $P(h)$ used in Eq. \ref{eq:L0} and the referent-category feature-probability that the speaker model seeks to convey (the *referent feature-probability*) $h$ in Eq. \ref{eq:S1} (Expt. 1a).

<!-- The pragmatic speaker model $S_2$, Eq.~\ref{eq:S2}, is a model of truth judgments.  -->
We test our model on thirty generic sentences that cover a range of conceptual distinctions discussed in the literature [@Prasada2013]: *characteristic* (e.g., "Ducks have wings."), *minority* (e.g., "Robins lay eggs."), *striking* (e.g. "Mosquitos carry malaria."), *false generalization* (e.g., "Robins are female."), and *absent features* (e.g. "Lions lay eggs.").
In additional to the canonical cases from the linguistics literature, we select sentences to elicit the full range of acceptability judgments (intuitively: "acceptable", "unacceptable", and "indeterminate") with low, medium, and high feature-probability properties. 

To compare the model to empirical truth judgments, we measure the prior distribution over feature-probabilities of these properties (Expt. 1a).
We hypothesize that these prior distributions encode probabilities associated with conceptual distinctions [@Prasada2013].
In particular, we hypothesize that different kind--property relations [e.g., *principled* v. *accidental relations*^[This is sometimes referred to as "statistical" relations.]; @Prasada2013] can be observed in the feature-probability prior. 
If, in fact, the feature-probability prior is a manifestation of deeper, conceptual knowledge, we would expect the feature-probability prior data to follow a multi-modal, mixture distribution where there exist qualitatively distinct modes or components to the prior, as probabilistic traces of the different types of kinds with respect to the property.
A prior that is structured would be one way in which our formal theory connects to other psychological accounts of generic language [cf., @Prasada2013; @Gelman2003; @Leslie2007].
As a simple test of this hypothesis---that the feature-probability prior encodes conceptual knowledge---we compare a structured (i.e., mixture) model of the prior data to an unstructured one.

After having validated a measure of the feature-probability prior, we consider the issue of primary theoretical interest: How well our *uncertain threshold* hypothesis accounts for endorsements of generic sentences (Expt. 1b).
...
## Experiment 1a: Measuring the feature-probability prior for categories
...
We measured prior distributions empirically for the set of properties (e.g., \textsc{lays eggs, carries malaria}; 21 in total) used in our generic sentences.
After measurement, we show that assuming a structured model of the prior data is substantially preferred to an unstructured model.
...
Three participants were accidentally allowed to complete the experiment for a second time, so we excluded their second responses (resulting in $n=57$).
Two participants self-reported a native language other than English; removing their data ($n=55$) has no effect on the results reported. 
...
Participants first were shown six animal categories randomly sampled from a set corresponding to the generic sentences used in Expt. 1b (e.g., \textsc{robins, mosquitos}) and were asked to generate five of their own (Figure \ref{fig:generics-prior-task} left).
...
Eight property--columns appeared in the table, and this whole procedure was repeated two times.
...
\centering
    ...
    \caption{Design of Experiment 1a for eliciting prior knowledge about feature-probabilities. Left: Participants first generated animal names after seeing six example categories. Right: One feature at a time, participants estimated the percentage of the category with the feature, for each category.}
...
In analyzing the feature-probability prior data, we are primarily interested in exploring the hypothesis that the prior is structured as a result of deeper conceptual knowledge.
We formalize this hypothesis using a simple Bayesian mixture model. 
We test the simplest "structural" hypothesis, where there are two, qualitatively different components to the prior [e.g., if participants believe that some kinds have a causal mechanism that *could* give rise to the property, while other kinds do not; cf., @Griffiths2005].
The "unstructured" alternative hypothesis is that there is just a single distribution that gives rise to the feature-probability prior data.

For the structured hypothesis, we assume the data was generated from one of two distributions: a distribution corresponding to those kinds with a stable causal mechanism that *could* give rise to the property ($\mathcal{D}_{stable}$) and a "transient cause" distribution corresponding to those kinds without a stable mechanism ($\mathcal{D}_{transient}$).
...
Each property (e.g., *lays eggs*, *carries malaria*) is modeled independently (i.e., each property's feature-probability prior has its own parameters). 
...
To see how well the structured model fits the feature-probability prior data, we use Bayesian inference to infer the *a posteriori* likely values of the parameters and then use the inferred parameters to generate new data.
The generated data from the model's posterior is called the *posterior predictive distribution* and is an important step in model validation. 
...
```{r generic-endorsement-priors-figure, fig.width = 7, fig.height = 5, fig.cap="Cumulative density plots for prior distributions representing the feature-probability of eight different features. Distributions are the posterior predictive distributions for the Structured and Unstructured prior models, as well as the raw empirical distribution. A completely uniform distribution would be represented as the y = x line."}
...
This possibility has been investigated empirically in one of few examples of statistical modeling of generics, and in fact, cue validity is highly correlated with generic endorsement [@Khemlani2012].
...
If there is a finite number of kinds in the feature-probability prior for a feature, and one knows the probability of the feature for each kind, then the cue validity of that feature for a particular kind is encoded in the feature-probability prior, in that it can be derived via Bayes' Rule:
...
<!-- Properties like \textsc{doesn't attack swimmers} are very right-skewed; with this prior, not many thresholds lead to a substantial update in beliefs, and so the generic is unlikely to be used by speaker $S_1$ unless the property is practically-universal within the referent-category.  -->
...
In Expt. 1a, we measured and modeled the feature-probability prior distribution (e.g., the probability of laying eggs for different kinds of animals) for different features.
This data also includes information about the referent-category feature-probability (e.g., how many robins lay eggs).
Using the referent-category feature-probability as $h$ in our speaker model (Eq. \ref{eq:S1}), the speaker model will predict endorsements for the corresponding generic statements (e.g., "Robins lay eggs").
...
As a secondary alternative hypothesis, we explore whether referent feature-probability and *cue validity* (the probability of a kind given the feature; described in more detail below) predicts generic endorsement of these items.
...
Our underspecified threshold model has access to the full prior distribution on feature-probabilities $P(h)$, from which it infers likely thresholds for the generic. 
...
##### Referent feature-probability baseline

From the feature-probability prior data (Expt. 1a), we estimate participants' beliefs about the probability of a feature for the referent categories (e.g., the percentage of \textsc{robins} that \textsc{lay eggs}).
...
This is because our stimulus set includes generics that are true with high feature-probability properties (e.g., *Leopards have spots.*) and generics that are false with low feature-probability properties (e.g., *Leopards have wings.*). 
...
If feature-probability is the "forward probability" (e.g., one's predictions about whether or not an entity will lay eggs after learning that the entity is a robin), *cue validity* is the inverse probability: $P(k \mid f)$ (e.g., one's predictions about whether or not the entity is a robin, upon learning that it lays eggs).
...
For a feature-probability distribution with a fixed set of categories, the cue validity of a kind given a feature can be calculated from the feature-probability prior distribution.
In empirical studies of generic language, however, it is often empirically measured [@Khemlani2012].
For this analysis, we measured cue validity using a free production paradigm, following @Cree2006.
Participants ($n = 50$) were supplied with a feature (e.g., "X lays eggs") and asked to generate a kind (e.g., "what do you think X is ?").
For a detailed discussion of the results of this experiment and comparison to other measurements of cue validity, see Appendix B.
...
Unlike the simple regression models based on point-estimates of referent-category feature-probability and cue validity, this cognitive model has access to the full prior distribution on feature-probabilities $P(h)$.
This model has two other main architectural differences: an information-theoretic decision between producing the generic vs. an information-less alternative (formalized using RSA) and uncertainty about the semantic threshold that corresponds to the theoretical, core meaning of a generic statement.

We first test a model that has two of the the above three main architectural features.
This model has access to the full prior distribution on feature-probabilities as well as models an information-theoretic decision of producing the generic vs. an alternative.
...
Other fixed-threshold models are possible (e.g., generic means "Most" $\theta = 0.5$) but require the further specification of a noise-model for participants' responses in the cases where participants endorse the statement when it is literally false (e.g., "Mosquitos carry malaria" would be literally false under this semantics).
]
This is a strong alternative model to our uncertain-threshold model; the only difference concerns the threshold.


<!-- Our pragmatic speaker model, $S_1$ in Eq. \ref{eq:S1}, reasons about whether it would be useful to say to the generic to this kind of listener so that the listener may acquire the same beliefs as the speaker (represented as the feature-probability of the property within the referent-category $h'$). -->
We use the empirically estimated referent-category feature-probability as the $h$ that the speaker $S_1$ is trying to communicate, and use the empirically measured priors from Expt. 1a as the listener's feature-probability prior $P(h)$.^[
...
To fit this model, we incorporate the RSA model into the Bayesian data analysis model of the feature-probability prior data (Expt. 1a) to create a single, joint-inference model where the speaker optimality parameter $\lambda_1$ is inferred jointly with the referent feature-probability $h_{k, f}$ for each category $k$ and property $f$ and the parameters of the feature-probability priors $P_f(h)$ for each property $f$ using data from Expt. 1a \& b (see Figure \ref{fig:genericsModelDiagram}).
For the parameters of the feature-probability distributions, we use the same priors described in Expt. 1a; for the speaker optimality parameter, we use a prior with a range consistent with previous literature using the same model class: $\lambda_1 \sim \text{Uniform}(0,5)$.
...
The posterior distribution of this joint inference model tells us what we should believe about the latent parameters of the model (referent feature-probabilities, feature-probability priors, and the single speaker optimality parameter) given the observed data, as well as what predictions the model makes given these parameter values.
Importantly, the model recapitulates the feature-probability prior data (e.g., the probability of laying eggs among various species) and the referent-category feature-probability data (e.g., the probability of laying eggs among robins; see Appendix A for full posteriors) as it did when the Bayesian data analysis model only included these data and their associated model (results section of Expt. 1a).
This means the theoretically-interesting predictionso of this model --- predictions of generic endorsement --- are based on intuitively meaningful model components (i.e., the details of the feature-probability distributions). 
...
This is because almost all items get endorsed by some participants; the model then infers that the referent feature-probability must be at least a little greater than 0\%; but, if the referent feature-probability is greater than 0\%, the statement is 100\% true. The speaker model then must decide is it better to say the 0\%-threshold meaning or stay silent. 
...
<!-- Generics that received definitive agreement or disagreement are predicted to be judged as such by the model (corners of Figure \ref{fig:commongenerics}b), including items for which referent-category prevalence is not a good indicator of the acceptability (e.g. \emph{Mosquitos carry malaria}, for prevalence quartiles 2 and 3, $r_{Q2,3}^2=0.955$; MSE=0.005; Figure \ref{fig:commongenerics}b, intermediate shades). -->
...
<!--     Intervals on the top of insets show human judgments about the prevalence of the property within a referent-category. -->
<!--     (b) -->
<!--     Human acceptability judgments compared with model predictions (left) and the referent-category prevalence (right) for thirty generic utterances about familiar animals and properties.  -->
<!--     Color denotes referent-category prevalence of the property, with lighter colors indicating higher prevalence.  -->
...
By measuring listeners' prior beliefs about the probability of the property across kinds and the feature-probability of the referent kind (e.g., the percentage of birds that lay eggs), we were able to show how a semantics based on probability is tenable inspite of these seeming counterexamples.
...
In our second case study, we extend our theory to a new domain and provide the first causal evidence for our theory by manipulating the referent feature-probability that the speaker aims to communicate.
...
First, we measure the prior distribution over the feature-probabilities for different kinds of events (Expt. 2a).
...
Then, we test endorsements of habitual sentences, manipulating the feature-probability of a particular person doing an action using rate information (Expt. 2b).
...
Up to this point, we have said little as to the interpretation of $h$, the referent feature-probability  that the speaker aims to convey.
...
We do this by measuring particpants predictions about future rates where there are causal forces enabling or preventing the action (Expt. 2c). 
We separately measure endorsements under the same contexts (Expt. 2d) and use the prediction measurements (Expt. 2c) as the referent feature-probabilities $h$ that the speaker model is trying to communicate, in order to predict habitual endorsements.
...
In this experiment we elicit the feature-probability prior $P(h)$ for different events in order to generate model predictions for corresponding habitual statements (Expt. 2b \& 2d).
For the case of statements about the behaviors of people, $P(h)$ can be conceived as a distribution over *different people*, each of whom do the behavior with different rates or probabilities.
To better understand what frequencies should be expected for different actions, we took advantage of the structured representation of the feature-probability prior discovered in Expt. 1.
...
For the case of background knowledge over the feature-probabilities for people doing actions, we consider again two components.^[
...
Question 2 elicits the rate or frequency with which a person does the action.
...
We next explore the endorsements of habituals from the items whose feature-probability priors were measured the prior elicitation task. 
...
#### Referent frequency baseline

The simplest alternative hypothesis for habitual language endorsement is that the degree to which the habitual statement is acceptable is given by the referent frequency. 
...
In addition, for actions that are done on the time scale of years or longer (lower median of frequency), referent frequency no longer explains the endorsements ($r^2(`r r2.hab.infrequent.n`) = `r r2.habituls.infrequent.regression.freq`$; MSE =$`r mse.habituls.infrequent.regression.freq`$)

In addition, we observe that none of our items receive less than 25\% endorsement (i.e., a maximum of about 75\% of participants disagree with the felicity of the utterance).
...
With generics, we constructed a regression model that used empirical *cue validity* as a predictor of generic endorsement. 
Here, we articulate a regression model that uses participants' responses to the question about the mixture parameter $\phi$ (i.e., the proportion of people who have done the action before) to predict habitual endorsement. 
This is analagous to the regression model based on cue validity and referent feature-probability in the domain of generics.
...
We used the speaker model $S_1$ (Eq. \ref{eq:S1}) with the priors elicited above (Expt. 2a) to predict felicity judgments in Expt. 2b, assuming the referent feature-probability $h$ being conveyed by $S_1$ is the provided frequency (e.g., 3 times in the past year).
...
In Experiments 2c \& 2d, we further investigated the nature of the underlying feature-probability scale by introducing enabling and disabling causal evidence, measuring the predicted future frequency (Expt. 2c) and using that, with the model, to predict the felicity of habitual sentences (Expt. 2d). 
...
In our final case study, we extend our theory to communicating generlizations about causes, and experimentally test whether the feature-probability priors are indeed causally related to endorsing generalizations in language.
...
Indeed, causal language faciliates causal learning for nonobvious causal relations in children as young as 2-years-old [@Bonawitz2010].
...
In this last set of experiments, we test the causal relationship between the feature-probability priors and endorsements of generalizations about causes.
...
## Experiment 3a: Manipulating the feature-probability priors for causes
...
This prediction task serves as a manipulation check for our feature-probability prior manipulation.
...
In the second phase, participants are asked to make a judgment corresponding to either the prior task or the endorsement task (described below). 
...
Generalizations in language (e.g., "John runs.", "Dogs are friendly.", "Staring at the sun makes you go blind.") are a premier example of how simple statements --- statements understood by even the youngest language users --- can display complex sensitivity to context.
We have argued that the core meaning of such linguistic expressions is, in fact, simple, but underspecified. 
To our knowledge, this is the first formal theory of generalizations in language that makes precise quantitative predictions about human endorsement data. 

As a formal theory of generalizations in language, we aim to unify significant swaths of language that are seemingly quite different from one another: generalizations about categories, events, and causes.
...
Indeed we have been able to capture the context-sensivity of these statements by positing a single underlying scale defined by the *feature-probability* (Expt. 1, 2a \& 2b, 3).
...
Finally, our framework naturally accounts for a number of well-documented examples from the linguistics of generalizations about categories, or *generic language* (Expt. 1).
...
At the same time, this paves the way for articulating formal theories about conceptual structure and provides a route for precise, quantitative influence on generic language [c.f., @Leslie2008].
...
We constructed the prior belief distribution for the probability of a property, event, or cause by considering other possible categories having the property, people doing the action, or causes producing the effect.
These other kinds, people, and causes form *comparison classes* against which the referent-category is evaluated. 
Thus, the feature-probability prior $P(h)$ is always relative to a class of other entities or events, a comparison class $C$: $P_C(h)$.
...
In this work, we made the *a priori* decision to construct the comparison class with respect to other kinds, people, or causes.
...
Statements of genericity can be used to address either QUD, and this difference can even be observed in the same sentence.
The statement "Lawyers care about the law." can be interpreted with a category-wise comparison class (i.e., *Lawyers (as opposed to doctors, firefighters, ...) care about the law.*; one can imagine this interpretation in a pedagogical context) or a feature-wise comparison class (i.e., *Lawyers care about the law (as opposed to justice, ethics, ...)* e.g., when said sardonically).
...
Future work should address the feature-wise reading of generalizations, and explore what cues lead listeners to one or the other interpretation.
...
## Structured prior knowledge and implications for conceptual accounts
...
Previous psychological and philosophical work on generics has looked beyond feature-probability and focused on conceptual distinctions and relations [@Gelman2003; @Prasada2013; @Leslie2007; @Leslie2008].^[
In these accounts, feature-probability is normally treated as synonymous with the current *prevalence* of the feature in the category. 
As we have shown, feature-probability in our account is more general notion, reflecting speaker's subjective, predictive probabilities.
] 
...
Our approach makes the strong claim that beliefs about predicted feature-probability is the connective tissue between conceptual knowledge and the semantics of generic language.
...
We found in Expts. 1 \& 2 that conceptually different kinds of properties and events gave rise to different feature-probability priors. 
...
We found that empirical feature-probability distributions are structured in a way that reflects intuitions about causal mechanisms underlying different properties; the differences in shape of these distributions in turn led to variable endorsements of generalizations. 
...
The focus on subjective, predictive probability casts new light on classic puzzles surrounding accidentally-true situations.
Consider a thought experiment from the linguistics and philosophical literature: Imagine that currently every Supreme Court Justice's social security number is an even number. 
...
Thus, our model would not endorse the statement \emph{Supreme Court Justices have even social security numbers}, because the predictive feature-probability would not be any different for Supreme Court Justices than any other profession.
...
In fact, quantified statements, whose formal meaning is much more straight-forward (e.g., "Some" means more than zero), emerge *later* in development [@Brandone2014; @Gelman2015].
...
Statistical accounts take the feature-probability to be fundamental: \emph{Birds lay eggs} means \emph{Birds, in general, lay eggs}. 
Our probabilistic account has antecedents in @Cohen1999 's theory of generics as a frequency adverb (e.g., "generally").
Similar to our formulation, Cohen looks to feature-probability as a metric against which generics get evaluated.
For Cohen, there are two kinds of generic statements: "Absolute" and "Relative".
"Absolute generics" have a meaning with a fixed threshold on feature-probability and that threshold is taken to be 0.5: $P(F\mid K)>0.5$. 
...
It should be noted that another mechanism used in Cohen's theory is to contextually restrict the entities that go into the computation of feature-probability (i.e., which robins do we look at to compute the probability of *laying eggs* among *robins*?).
...
Feature-probability in our theory is a predictive probability, which incorporates background knowledge in order to make predictions about the future. 
...
This prediction was borne out experimentally when participants are supplied information about the dangerousness of a feature, even when the actual feature-probability is quite low [e.g., "30\% of lorches have purple feathers that are as sharp as needles and can easily get lodged in you, causing massive bleeding."; @Cimpian2010].
In pilot work, using paradigms similar to the manipulation check paradigm employed in the causal language experiments (Expt. 4a) and the structured elictation used in habituals (Expt. 2a), we have observed that the prior distribution over the probability of the feature $P(h)$ also changes when supplied information about the dangerousness of the feature.
...
Thus, our model would also predict these effects, but by way of the feature-probability prior rather than something special about dangerous features per se.
...
In this paper, we take a minimalist approach in modeling conceptual structure in our prior on feature-probabilities $P(h)$, and incorporate only what is necessary in our model of world knowledge. 
...
(1) is often used to demonstrate that the feature-probability is not sufficient for a generic to be true.
...
<!-- Specifically, the referent-category K in a generic statement "Ks F" is not expected to be in the comparison class.  -->
...
From the taxonomic relation of the subordinate categories (e.g., types of books) to the referent-category (i.e., books), the true probability of the feature among the referent-category must be the mean of the probabilities of all subcategories, assuming an equal partitioning of the prior probability of subcategories.
Thus, if we were to construct a speaker model of the statement "Books are paperbacks", the model would predict a 0.5 endorsement: neither true nor untrue, in the same way as "Robins are female". 
...
The current formulation of the model is relatively ambivalent about the deviations of the listener's beliefs from the speaker's belief (i.e., the speaker doesn't mind that the feature-probability that she believes to be true will be exaggerated from the generic statement).
...
If our speaker model believed that states of the world where the probability of the feature is high (e.g., a world where many women were submissive) were undesirable (either in absolute terms or as a belief), then a speaker who considered both the informational content of the utterance and the utility in the states of the world implied by that utterance would too find it reprehensible to make such generic claims. 
...
Disagreements are also predicted when interlocutors fail to share background assumptions, which in framework could be differences in the referent feature-probability, the prior distributions on feature-probabilities, or the comparison class.
...
The theory presented here gives one explanation of how we do so, providing a computational perspective on how we communicate generalizations and how beliefs play a central role in understanding language.
