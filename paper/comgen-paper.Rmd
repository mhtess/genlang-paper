...
This kind of model formalizes the basic aspects of communicative reasoning and is called a Rational Speech Act model [@Frank2012; @Goodman2016].

We interpret the endorsement task (e.g., "true" vs "false"; "agree" vs. "disagree") as supplying two alternatives to the endorsement model: produce the generalization vs. some null alternative [@Franke2014cogsci; @Degen2014].
For simplicity, we take the null alternative to be an information-less "silent" utterance: 
...
The utterance encoding a generalization updates the interpreter model's beliefs about the prevalence of a feature in the category being referred to, which we refer to as the *referent prevalence*. 
The endorsement model has a particular referent prevalence in mind and decides whether the generalization  (vs. the null alternative) is a good way to convey that to the interpreter. ^[
...
This endorsement model (called $S$ for speaker) does this by reasoning about the interpreter model $Lit$: 
\begin{equation}
S(u \mid p) \propto (\int_{\theta} Lit(p, \theta \mid u)  \diff \theta)^\lambda
\label{eq:S1}
\end{equation}

The informational content of the generalization is about the referent prevalence $p$.
