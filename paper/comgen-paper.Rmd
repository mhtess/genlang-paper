...
```{r causals-conditions, fig.caption = "Four background distributions participants were exposed to in Expt. 4 (stimuli) and priors reconstructed from participants' responses (model). "}
...
bind_rows(
  ...
  ggplot(., aes(x = x, fill = src, color = src, alpha = src, lty = src))+ 
...
```{r causals-endorsement, fig.width = 4, fig.height = 3, eval = F}
...
bind_rows(
  ...
  ggplot(., aes( x = dist, y = MAP, ymin = cred_lower, ymax = cred_upper, fill = dist,
                        group = dist))+
  geom_bar(stat='identity', position = position_dodge(), width = 0.3, color = 'black')+
  geom_linerange(width = 0.2, position = position_dodge(0.3))+
