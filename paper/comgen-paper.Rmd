...
Throughout our experiments, we compare the uncertain threshold model to an alternative, lesioned model by removing the vagueness in meaning, assigning a fixed semantics to the generalization (i.e., analogous to the quantified statements) but keeping the prior and the decision-theoretic architecture in place. 
...
Furthermore, for comparison to standard methods in the literature on generics [e.g., @Khemlani2012], we constructed alternative regression models based on empirically-elicited quantities. 
In addition to serving as alternatives, these baseline regression models help us understand the statistical properties of our items. 
...
Though generics are vague that does not preclude them from being treated by formal models. 
...
We tested this theory by exploring its implications for the simplest judgments a person could make: truth judgments.
...
<!-- In the rest of this discussion, we elaborate how our model relates to language understanding more generally, situate our theory with respect to extant theories of generic language, and sketch applications to a few philosophical puzzles in genericity. -->
...
## Prevalence priors and cue validity
...
For comparison to standard methods in the literature on generics [e.g., @Khemlani2012], we constructed alternative models based on empirically-elicited cue validity.
...
Thus, we bring precision and clarity to the *output* of these cognitive processes and conceptual understanding: intuitions about prevalence.
...
The assumed complexity of a formal theory of generics and the simplicity with which young children acquire generics has led some to conclude that the normal tools for describing the semantics of quantified utterances (e.g., a truth-functional threshold) are inappropriate for generics [@Leslie2008].
...
Thus, under our framework, generics should not only be learned before quantifiers but also facilitate the acquisition of quantifiers.
