...
Model predictions depend upon the uncertain threshold model's interpretations, which are highly sensitive to background knowledge.
...
The prevalence prior can be thought of a distribution over the prevalence of a feature among alternative categories (Figure\ \@ref(fig:simulations)).
...
Figure\ \@ref(fig:simulations) (row 1) shows intuitive prevalence priors for six different properties (we introduce methods for empirically eliciting these priors in the experimental section).
The *lay eggs* distribution is bimodal, with substantial probability mass near 0 (since many animal categories do not have egg-layers) and a secondary component peaked around 0.5, because among the animals with egg-layers, only the female members of the category have the property.
...
worked.examples <- data.frame(
  ...
  `referent prevalence` = c("95%", "5%", "85%", "50%", "50%", "5%", "0 times", "100%", "50%/50%"),
  ...
  `potential issue` = c(
    ...
    "True, even though only female robins (50%) lay eggs",
    "Not true, but same number that lay eggs (50%)",
    "Very few mosquitos actually carry malaria",
    "True even if there has never been any mail from Antarcita",
    "False even if 100% have even social security numbers",
...
Often the first observation with generics is that they appear to behave like universal quantifiers (e.g., "All") that permit exceptions.
That is, not all dogs bark, but still "Dogs bark" is true.
The uncertain threshold account immediately accomodates this example because there is no single, fixed threshold beyond which the generalization becomes true.
Rather, listeners have uncertainty about the threshold *a priori* which leads to graded interpretations (e.g., "Dogs bark" meaning *almost all* dogs bark; Figure\ \@ref(fig:simulations); column 1, row 2).
The endorsement model then predicts this is a rather good generic sentence (Figure\ \@ref(fig:simulations), bottom row) even though not all dogs bark (referent prevalence of barking among dogs shown with arrow in Figure\ \@ref(fig:simulations) row 2).

**2. Kangaroos have spots.**

A second observation regarding generics is that their truth conditions cannot be so lenient so to always convey *existential* quantification (e.g., "Some").
Indeed, it's intuitively plausible that some kangaroos do have spots but a rational language user might feel the generalization "Kangaroos have spots" is too strong to assert.
The model exhibits this same restraint, predicting a very low endorsement probability for this generic statement (Figure\ \@ref(fig:simulations); column 2).
...
The endorsement model, which believes that very few kangaroos have spots, then would rather not endorse the statement because it would mislead the listener.

**3. Sharks don't eat people.**

A similar logic can be applied to this example, which is the converse of the affirmative generalization "Sharks eat people", which is assumed true despite the low actual number of incidents involving sharks eating people [@Leslie2007]. 
The referent prevalence---the number of sharks that don't eat people---is thus very high: Almost all sharks don't eat people.
Yet, intuitively, the generalization is again misleading. 
Because for almost all animal categories, almost members of those categories don't eat people (i.e., the prevalence of the feature is almost 100% for all categories because very few things eat people), interpreting the statement "Sharks don't eat people" would lead one to believe that no sharks eat people, which is too strong.
It is interesting to note that because of the high referent-prevalence, the model is less certain of its decision, predicting an endorsement probability around 40%.
We will test this quantitative prediction in one of our experiments.

**4. and 5. Robins lay eggs vs. Robins are female**
...
Interestingly, the generic interpretation model returns roughly the same interpretation distribution for each hypothetical utterance: In each case, the model believes 50% have the property.^[
...
However, only in the case of "Robins lay eggs" does the endorsement model actually assert the generic; it does so because the listener interpretation would more aligned with the referent prevalence than if the endorsement model produced the null utterance, which would instead return the prior.
In the case of "Robins lay eggs", the hypothetical generic utterance does not carry any information (the listener posterior is not different from the prior); in this case, however, the generic would not be misleading only uninformative, and hence the model predicts an endorsement probability of 0.5 (i.e., *neither true nor false*).
...
The uncertain-threshold model comes to the same conclusion---endorsing a generic when the referent prevalence is very low---because the listener interpretation distribution would be more aligned with the referent prevalence than the prior. 
Intuitively, this is because not that many animals have malaria-carriers among them and so in a sense, having malaria is diagnostic for being a mosquito, an intuition that is often arrived at with this example.
This example also highlights that the construct of *cue validity*---the probability of the feature given the category e.g., $P({is a mosquito} \mid \text{carries malaria})$---plays a role in our model. 
...
Sentence (7) is still thought to be intuitively true despite zero actual instances of the event.
This highlights an important ambiguity in the theoretical commitments of the uncertain threshold model: The endorsement model aims to communicate some referent prevalence $p$, but does the prevalence represent the actual, objective frequency in the world or a subjective, predictive degree of belief in the head?

We posit that Sentence (7) is an extreme case of generalizations expressing *predictive* probabilities.
...
Sentence (8) is still considered false, even though the property holds for exactly 100\% of the category [@Cohen1999].
We predict the rejection of Sentence (8) is the result of abstract, intuitive theories guiding our subjective predictive probabilities.
...
Then, if we constructed the comparison class by considering other professionals, Sentence (8) would be rated by our model as similar to "Birds are female", because all professions have roughly the same probability of having employees with even social security numbers.
...
@Nickel2008 suggests Sentence (9) is semantically equivalent to: "Elephants live in Africa and elephants live in Asia".
...
In fact, with prior knowledge suggesting against the existence of international elephants and without any further semantic assumptions, our model interprets Sentence (9) as meaning some elephants live in Africa and that different ones live in Asia.^[
...
Interestingly, an incremental parsing of Sentence (9) (i.e., upon hearing only the first part of the sentence that "Elephants live in Africa") leads our model to believe that *most* elephants live in Africa.
