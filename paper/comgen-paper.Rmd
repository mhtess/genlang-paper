...
abstract: > 
    ...
    The major issue in formalizing *genericity* is in determining the conditions under which such statements are true or false; these conditions are extremely flexible in communicating generalizations. 
    ...
    To test this theory, we examine endorsements of generalizations about three different domains: generalizations about categories (i.e., *generic language*, e.g., *Birds fly*), events (i.e., *habitual language*, e.g., *John runs*), and causes (i.e., *causal language*, e.g., *The block makes the machine play music.*). Across these divesre domains, we find that our model, and not simpler models, explains the full spectra of endorsements of generalizations in language.
...
We acquire such *generalizations* early in life, simply by observing instances and drawing inductive inferences [@Baldwin1993; @Xu2008; @Dewar2010].
Once learners have the capacity to infer the intention of others, social learning can dramatically faciliate inductive reasoning [@Csibra2009; @Tomasello1999; @Butler2012].
At around the first birthday, a child's mind gains access to a new, human-unique source of data: language.
...
Learning from language is helpful because many properties are costly to directly observe (e.g., being poisonous) or events statistically unlikely (e.g., observing lightning).
Minimally, language helps foster generalization by bringing attention to non-obvious shared relations through the use of labeling [@Gelman1986; @Gopnik2000; @Graham2004; @Markman1989; @Schulz2008].
...
<!-- *Causal language*, expressing generalization beyond an individual causal event (e.g., "The block *makes* it [the machine] go."), faciliates more abstract forms of causal reasoning [@Bonawitz2010]. -->
...
Generic meaning, or *genericity*, can be observed in seemingly disparate kinds of linguistic devices [@Carlson1977; @Carlson1995].
Speakers can talk about individual people in generalizations (*habitual language*; e.g., "Mary swims after work."), individual causal events in generalizations (*causal language*; e.g., "Drinking moonshine makes you go blind."), and people in general [e.g., "You never know what will happen on a blind date."; @Orvell2017].
It is believed that every language can express generic meaning [@Behrens2005; @Carlson1995], and that such languge is central to the growth of conceptual knowledge [@Gelman2004] 
...
Computational models of learning formalize the inferences derived from observations, explaining how children and adults generalize from sparse data [@Tenenbaum2011]. 
...
The major issue in formalizing genericity is determining what makes statements that convey generalizations true or false. 
When we hear "Dogs bark", we may feel it's a universal statment as in *All* dogs bark.
Yet, exquisitely well-trained dogs, among others, do not bark; the statement should signal something weaker like *Most* dogs bark.
...
The only quantifier that would permit such a generalization would be "Some" (e.g., *some mosquitos carry malaria*).
But equating the generalization with *some* is too lenient: Some robins are female, but saying *Robins are female* seems odd.
Perhaps we should make exceptions for properties that only apply to one gender, but *Robins lay eggs* is intuitively true, despite the property being very similarly distributed among the population of robins as being female.
...
By formalizing such a hypothesis in a probabilistic language understanding model, we can derive predictions for endorsements of such statements by specifying relevant prior knowledge in the model.
...
In Section 3, we describe experiments the measure both (i) and (ii) and relate them to the endorsements of generalizations about categories, or *generic language*. 
...
Our theory also opens up a number of new directions for understanding generic language and building models that learn from language like people.
...
Particular statements can be made about objects, events, and causes.
...
For an action exhibited by an agent, the inference concerns a rate --- how frequently (in time) does $x \in f$ occur --- which can be described as a probability integral: $h = \int_{t} P(x \in f_t \mid x \in k_t) \diff t$
<!-- In this case, $x$ is a instance of an agent. -->
Causal learning can be described with the same formalism as for categories, a probability referring to the probability of an effect $e$ given a cause $c$, known as causal power: $h = P(x \in e \mid x \in c)$.
Throughout, we refer to the degree of probability as $h$ because it represents a hypothesis about the underlying causal power or frequency that generated the observed data.
...
Confounded causal evidence can be suddenly become unconfounded when you know the evidence was generated by an intentional agent, which is predicted assuming the data-generating process (i.e., the likelihood function) is another agent [@Goodman2009].
...
Arbitrary relations can also be expressed such as *Barns are red* or *Ravens are bigger than toasters*, which seem only to be true because those are the statistics of the world [@Nickel2008; @Nickel2016; @Sterken2015].
...
Bayesian cognitive models are used to represent flexible, structured knowledge of the world and formalize the inferences that people draw using such conceptual representations [e.g., @Kemp2008; @Goodmanconcepts].
...
If that's true, a simple, semantic theory (e.g., a threshold on subjective probability) may be enough to formalize the core meaning of a generic statement $u_{gen}$:
...
We assume this prior is uninformed: $P(\theta) = \text{Uniform}(0, 1)$.
...
This *simple but underspecified* hypothesis defines a class of models with potentially more levels of recursion (e.g., a listener who thinks about a speaker who thinks about a listener).
We discuss these alternative formulations (all of which are instances of the *simple but underspecified* hypothesis) and provide formal model comparisons between these RSA models in Appendix A.
...
For purposes of illustration, we present present the listener $L$ interpretations of the generalization for the underspecified threshol model as well as a fixed threshold model where the threhsold is set to the lowest possible value, corresponding intuitively to the quantifier *some* (Figure \ref{fig:simulations}).
...
Note that our model of endorsements is a model of a speaker who decides whether or not to say the generic or stay silent. 
Qualitatively speaking, model predictions for endorsement can be gleaned by comparing the posterior distribution over the probability $h$ upon hearing the generalization with the prior distribution over $h$ (which is the same as the listener's posterior upon hearing "silence").
For different degrees of target probability $h$ that the speaker wants to communicate, the generalization will be more or less good at communicating it.

In Figure \ref{fig:simulations} (row 2), we show a schematic prior distribution over a feature "barks".
Here, we posit the prior is bimodal with peaks near 0\% and some high prevalence values, corresponding intuitively to the fact that many kinds creatures do not bark (i.e., have $h \sim 0$) and for those that do have members that bark, many of them bark (i.e., have some high $h$).
<!--  -->
...
As a first test of our theory, we wish to explain the flexibility in endorsements of generics (e.g., "Birds lay eggs" vs. "Birds are female"; "Mosquitos carry malaria") in terms of the relevant prior knowledge about properties.
We do this by measuring the prior belief distribution over the relevant probabilities $P(h)$ used in Eq. \ref{eq:L0} as well as the target probabilitiy that the speaker model seeks to convey $h$ in Eq. \ref{eq:S1}.
We show how these measurements systematically predict, via our model, endorsements of generic statements about familiar categories (Expt. 1b).
...
This would be one way in which our theory touches upon other psychological accounts of generic language [cf., @Prasada2013; @Gelman2003; @Leslie2007].
...
As a simple test of this hypothesis, we compare a structured (i.e., mixture) model of the prior data to an unstructured one.
...
Finally, we assume that these two components combine with some mixture weighting $\phi$ such that the data we observe is $$P(d) = \phi\cdot \text{Beta} (d \mid \gamma, \xi) + (1 -  \phi) \cdot \text{Beta}(d \mid \alpha = 1, \beta = 100) $$.^[
We use the mean / concentration parameterization of the Beta distribution rather than the canonical shape (or pseudocount) parameterization for ease of posterior inference. The shape parameterization can be recovered using: $\alpha = \gamma \cdot \delta; \beta = (1 - \gamma) \cdot \delta$.
]
The unstructured hypothesis only represents a single component in the prior distribution $P(d) = \text{Beta} (d \mid \gamma, \xi)$.

We test which hypothesis better accounts for the data by computing a Bayes Factor, which compares the relative explanatory power of each model while taking into account the flexibility of each model [@LW2014].
We put the following priors over the latent parameters of each model (note that the unstructured model does not use $\theta$):
...
Each property (e.g., "lays eggs", "carries malaria") is modeled independently (i.e., each property has its own parameters). 
...
It is possible that the data is, in fact, more structured than our simple two-component model. 
...
We could fit a more complex model to this data (i.e., one with three mixture components); however, we retain the imperfect simpler model for data-analytic ease going forward. 
...
```{r generic-endorsement-priors-figure, fig.width = 7, fig.height = 5, fig.cap="Cumulative density functions for prior distributions representing the prevalence of eight different features. Distributions are the posterior predictive distributions for the Structured and Unstructured prior models, as well as the raw empirical distribution. A completely uniform distribution would be represented as the y = x line."}
...
In addition to these categorical differences between our thirty generic statements, we find a contiuum of endorsement values (Figure \ref{fig:generics-endorsement-figure}).
...
However, large deviations from an account based purely on target-category prevalence remain: Generics in which the target-category has intermediate prevalence (prevalence quartiles 2 and 3: $20\% < \text{prevalence} < 64\%$), are not at all explained by prevalence within those categories ($r_{Q2,3}^2 = 0.029$; MSE = 0.110).
...
This corresponds to the more general form of the model described in **Alternative Formulations** section. 
...
We combine the RSA model with the Bayesian data analysis model of the prevalence prior data (Expt. 1a) to create a single, joint-inference model where the speaker optimality parameter $\lambda_1$, the target prevalence $h$, and parameters of the prevalence priors $P(h)$ are inferred simultaneously via both data sources (Expt. 1a \& b; see Figure).
...
From a methodological standpoint, habituals are a particularly amenable domain to manipulating the target propensity that a speaker aims to communicate.
...
```{r habituals-prior-fig, fig.width = 9, fig.height = 6, fig.cap="Reconstructed priors on frequency from the structured, prior elicitation task (Expt. 2a). Priors are reconstructed by bootstrapping and forward sampling using the structured model (data, blue) or using Bayesian data analysis on the structured model (model, red)."}
...
All participants responded correctly to both questions in the attention check trial, so all data was used in the analysis.
...
Our prior distributions $P(h)$ are with respect to an alternative class of entities (e.g., other individuals). 
It's possible that when evaluating habitual statements, $P(h)$ is with respect to either *all people* or *people of the same gender*. 
For example, are the frequency conditions by which a man would qualify to "watch professional football" (habitually) different than those by which a woman would qualify to "watch professional football"?
We explore the possibility of different truth conditions for habituals of different gendered characters in Experiment 2, for select items with priors that differ substantially by gender.
...
Participants completed 37 trials, which were composed of the 31 items from the prior elicitation task randomly paired with either a male or female character name.
...
d.hab.endorse.regression <- left_join(
  ...
  unique(select(
    d.hab.endorsement, habitual, logAnnualRate, time_period, mixture
  )) %>%
...
for (i in seq(1, n_chains)){
  ...
  m.samp.i <- mi %>% mutate(chain = i)
  m.hab.samp <- bind_rows(m.hab.samp, m.samp.i)
}
...
Figure \ref{fig:habituals-predictive-figure} (left) shows the predicted future frequency as a function of the past frequency given to the participant and the type of causal information given. 
...
To account for participants and item variability in this analysis, we also include random effects of intercept and condition for both participants and items.
...
d.hab.predictive.endorsment.summary <- left_join(
  ...
  d.hab.predictive.summary %>% select(condition, item, logAnnualPastRate, mean, ci_lower, ci_upper) %>%
...
```{r habituals-predictive-figure, fig.cap="Left: Results of Expt 3a. Right: Results of Expt. 3b as they relate to the predicted frequencies from Expt. 3a"}
grid.arrange(fig.hab.freq.predictive.vs.past, fig.hab.endorse.vs.predfreq, nrow = 1)
```
...
There is a clear and consistent negative effect of preventative information on endorsements for the habitual sentence (Figure \ref{fig:habituals-predictive-figure}, middle; blue points).
...
`r round(d.hab.predictive.endorsment.baseline.summary[[1,"MAP_h"]], 2)` [
`r round(d.hab.predictive.endorsment.baseline.summary[[1,"low"]], 2)`,
`r round(d.hab.predictive.endorsment.baseline.summary[[1,"high"]], 2)`
], enabling = 
`r round(d.hab.predictive.endorsment.enabling.summary[[1,"MAP_h"]], 2)` [
`r round(d.hab.predictive.endorsment.enabling.summary[[1,"low"]], 2)`,
`r round(d.hab.predictive.endorsment.enabling.summary[[1,"high"]], 2)`
], preventative = `r round(d.hab.predictive.endorsment.preventative.summary[[1,"MAP_h"]], 2)` [
`r round(d.hab.predictive.endorsment.preventative.summary[[1,"low"]], 2)`,
...
<!-- [Gerstenberg] -->
Determining whether or not a singular event itself is causal is challenging, plausibly having to do with counterfactual reasoning [e.g., @Gerstenberg2015how]. 
...
Indeed, causal language has been shown to faciliate causal learning for nonobvious causal relations [@Bonawitz2010].
We design this last experiment with the goal of testing the causal relationship between the prevalence priors that we posit as a mediating representation between abstract conecptual structure and endorsements of generalizations about causes.
...
The distributions that resulted from participants predicting the causal efficacy of the new herbs / fertilizers are shown in Figure \ref{fig:empirical-causal-prior-densities}.
...
As in our other analyses of endorsement responses, we computed the Bayesian Maximum A-Posteriori (MAP) estimate and 95\% highest probability density interval of the true population probability of endorsing the statement, assuming a uniform prior (i.e., a Beta-Binomial model). 
...
```{r causal-model-insets, fig.width = 8, fig.height = 3.5, cache = F}
...
```{r figure-causals, fig.width=11, fig.height=8, cache = F}
...
In this domain, we succesfully manipulated participants' beliefs about the expected causal power in a domain (Expt. 4a).
...
In Expt. 4b, we showed that these manipulated priors influenced endorsements of the corresponding causal statements.
In addition to further demonstrating the generality of this theory, these experiments show that the *prevalence prior* $P(h)$ construct that we posit is causally related to endorsement.
...
It's likely that there exist domains for which abstract, intuitive theories might interact with the experimentally-supplied "experimental data" to form a hybrid belief distribution.^[
If this were happening in our domains, we would expect this to show up in the results of Expt. 4a. Participants' predictions about the likely causal power of new causes would be expected to show a mixture of their abstract, intuitive theories and the experimentally supplied data.
]
For example, physical causal systems (e.g., billiard balls hitting each other) could strongly induce near-deterministic notions of causal power, analagous to our "strong" priors conditions. 
...
In fact, quantified statements, whose formal meaning is much more straight-forward, emerge much *later* in development [@Brandone2014; @Gelman2015].
This has led some to conclude that the normal tools for describing the semantics of quantified utterances (i.e., a truth-functional threshold) will not work for generic language [@Leslie2008].
...
This *soft semantics* (corresponding intuitively to something like "more is better") is perhaps the simplest semantics one could imagine.
...
Thus, if we were to construct a speaker model of the statement "Books are paperbacks", the model would predict a 0.5 endorsement: neither true nor untrue. 

(2) is often used to argue that no actual instances need have the property to make a generalization true. 
Part (or perhaps all) of Mary's job is to handle the mail from Antarctica; in this thought experiment, to date, there has not been any mail from Antarctica [@Cohen1999]. 
Thus, it is not the case that Mary *has handled* any mail from Antarctica before.
This example is an extreme case of the phenomenon highlighted by Expt. 3.
In Expt. 3, we manipulated predictive probability, while keeping fixed past frequency, and saw the endorsements of habitual statements indeed tracked predictive probability (with the linking function being the RSA speaker model). 
...
That is, even if it were the case that, because of a variety socio-cultural reasons, women tend to be submissive (more so than men, and many other social categories), we might still not want to say *Women are submissive* [@Haslanger2011]. 
...
If our model believed that states of the world where the prevalence of the feature is high (e.g., a world where many women were submissive) were undesirable (either in absolute terms or as a belief), then a speaker who considered both the informational content of the utterance and the utility in the states of the world implied by that utterance would too find it reprehensible to make such generic claims. 
...
```{r child = 'appendix-fullModelPosteriors.Rmd'}
```
