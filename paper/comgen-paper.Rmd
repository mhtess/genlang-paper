...
Probability is the universal currency of belief and this prediction can be described by a conditional probability: $P(x \in f \mid x \in k)$, the probability that $x$ will have $f$ given that it is in $k$ (formally, an instance of $k$ will be in the set of things that have $f$), which we will refer to as the *prevalence* and for convenience write as $p = P(f \mid k)$.
...
Bayes Rule provides the way to update one's prior beliefs with some obesrvational data $d$: $P(p \mid d) \propto P(d \mid p) \cdot P(p)$, assuming some generative process that resulted in the observed data---summarized in the likelihood $P(d \mid p)$ [@Tenenbaum2001].
...
Thus, the simplest semantics for a generalization is a threshold on the prevalence: $\denote{gen}(p, \theta) = \{p >\theta\}$.
...
By doing so, we thus provide an account of how context influences generic understanding and are able to make precise, quantitative predictions about human judgments. /ndg{this paragraph belongs later, after the model has been finished.}
...
The model also has prior knowledge about the property $P(p)$ (described in more detail below) and literal interpretation is given by updating these prior beliefs according the truth-functional meaning of the utterance $\delta_{\denote{u}(p, \theta)}$:
...
We consider the notion of felicity or endorsement from the information-theoretic perspective: Would a generalization statement convey the appropriate information to a naive interpreter? 
We formalize this by considering how well the generalization would convey the target prevalence $p$ to the $Lit$ model (Eq. \ref{eq:L0}).^[
...
This model can be seen as a special case of a Rational Speech Act model [@Frank2012; @Goodman2016] with only a single level of recursion and the space of possible utterances maximally constrained; the endorsement model is a "speaker", while the interpretation model is a "listener", and we are interested in whether a reasonable speaker would bother to utter the generalization. \ndg{i wonder if the content of this paragrph should be earlier, to motivate the endorsement model?}
...
However, the interpretation model's prior beliefs $P(p)$ will be constrain the interpretation and thus the endorsement. 
...
\ndg{this paragraph should get moved elswhere. it is distracting here....}
