...
Extracting generalizable knowledge that extends beyond the present context is crucial to thrive in our open-ended, dynamic world. 
Yet such knowledge can be difficult to acquire: The relevant observations may be costly (e.g., learning that a plant is poisonous) or rare (e.g., understanding that lightning strikes tall objects).
Fortunately, we are not bound to acquiring generalizations on our own; language allow us to communicate generalizations between each other.
By sharing generalizable knowledge, we flourish collectively without individually needing to taste potentially-poisonous plants or personally witness many lightning strikes.
Furthermore, flexibly communicating generalizations from one generation to the next may be a mechanism by which culture can cumulatively evolve [@Tomasello1999; @Henrich2015].
...
The *language of generalization* is most commonly studied with *generic language* (or, *generics*), generalizations about categories [e.g., "Dogs are friendly"; @Carlson1977; @Cohen1999; @Leslie2007; @Nickel2008].
<!-- Generics are ubiquitous in everyday conversation as well as in child-directed speech [@Gelman2008].  -->
In contrast to statements about concrete individuals or events (e.g., "Rufus is friendly"), generic statements refer to inherently unobservable categories (e.g., the category of \textsc{dog}) and convey information that extends beyond the here and now, a fact which children as young as 2 appreciate [@Cimpian2008].
...
It is believed every language can express generalization [@Behrens2005; @Carlson1995] and that such language is central to the growth of conceptual knowledge [@Gelman2004].
...
Despite its ubiquity and relative morphosyntactic simplicity, there have been few formal models of the language of generalization and none that have made precise, quantitative predictions to match human intuitions. 
The major challenge in formalizing generalization in language is determining *what makes the statements true or false*.
...
But weakening the generalization to mean "some" is too permissive: Some robins are female (in fact, half of them are), but asserting "Robins are female" seems odd.
Even more perplexing: Those very same female robins lay eggs, and "Robins lay eggs" is a perfectly fine thing to say.

These observations have led to the conclusion that the prevalence of the feature (or the probability of a member of the category having the property e.g.,  $P(x \text{ barks} \mid x \text{ is a dog})$) is not the only factor that goes into making a generalization true or false.
The predominantly studied generic statements are thought to refer to *kinds* [@Leslie2008] and thus may the linguistic manifestation of special, kind--property relations [@Prasada2000; @Prasada2012]. 
...
"Bishops move diagonally" not because bishops tend to move diagonally but rather because those are the rules of the game.
...
Yet, purely arbitrary relationships can result in true generalizations [@Nickel2008; @Nickel2016; @Sterken2015].
Ravens and toasters have no priviledged relationship to each other, but "Ravens are bigger than toasters" is still true.
An understanding of generalizations just in terms of conceptual relations must be supplemented with the proviso that, in some situations, the actual prevalence of the feature matters for making generalizations true.

<!-- Furthermore, how conceptual accounts of the language of generalizations should extend beyond category generalizations (i.e., generics) is less clear.  -->
...
What we are left with is hybrid theories, which try to classify different kinds of generalizations and the various inferences they support [@Prasada2013].
But insofar as there is a single class of linguistic expressions that convey generalization, there should be something common to them all: a general, unified semantic core.
In this paper, we propose such a semantic core for the language of generalization.
...
To formalize a model of endorsement (i.e., what makes the statements true or false), we consider the information-theoretic function of a generalization by explicitly modeling the background knowledge a language interpreter would bring to bear when understanding generalizations.
We create an endorsement model based on this interpretation hypothesis, providing the first formal theory that makes quantitative predictions about what makes generalizations in language true or false.
The assumptions of the theory, and in turn, the conditions under which the theory may be meaningfully tested are made explicit by its instantiation in a formal model.
Though conceptual structure of kind discussed of the sort discussed by Leslie and Prasada are not explicitly formalized in our model, our general cognitive modeling framework provides natural avenues for future integration.
...
In Section 1, we formalize the hypothesis that the core of meaning of a generalization in language is simple but *underspecified*, articulate a thory of relevant background knowledge used to understand a generalization in language, and develop a computational model of endorsement around these components.
The endorsement model's predictions depend upon the background knowledge and the prevalence communicated, which we demonstrate via simulation.
...
Across these three case studies, we both measure and manipulate background knowledge and the prevalence communicated, measuring their effects on endorsement. 
We find a very strong agreement of our model's predictions to the full spectrum of human elicited endorsements, where alterantive models fall short.
This work opens the door to understanding generalizations in richer, pragmatic language understanding contexts and building models that learn abstract knowledge from language.
...
Generalizations are used to make predictions about properties of instances or events that an agent has yet to experience [@HumeTHN].
People readily predict that the next dog will have fur, drinking another cup of coffee will cause jitters, and a new day will find people that we know doing what they habitually do. 
In each case, we assign a specific exemplar $x$ to a category $k$, and make a prediction that it will have feature $f$.
This prediction can be described by a conditional probability: $p(f \mid k) = P(x \in f \mid x \in k)$, the probability that $x$ will have $f$ given that it is in $k$ (formally, an instance of $k$ will be in the set of things that have $f$), which we will refer to as the *prevalence*.
In particular, we define *prevalence* as a latent sense that a future instance of a category would have a particular property; others might call this projectibility [@Goodman1955].
...
If you *observe* wugs that have two legs, you might infer that all wugs have two legs.
But not all properties have such strong projectibility (prevalence).
Seeing a wug with broken wings tells you comparatively less about other wugs having broken wings [@Nisbett1983].
Abstract, potentially domain-specific, beliefs about properties can be represented by a probability distribution over the prevalence $P(p_{fk})$ [@Kemp2008].^[
  We adopt the following notational conventions: $p_{fk} = p(f \mid k)$ as prevalence of feature $f$ for a category $k$. 
]
Learning, then, can proceed via Bayes' Rule: $P(p_{fk} \mid d) \propto P(d \mid p_{fk}) \cdot P(p_{fk})$ given one's observational data $d$ and the observer's beliefs about the generative process that resulted in the observed data (the likelihood: $P(d \mid p_{fk})$).

But we do not always have access to observational data $d$.
Instead, we must listen to others to learn about properties that are costly to observe (e.g., *staring at the sun makes you go blind*) or events that are statistically unlikely (e.g., *lightning strikes tall objects*).
...
The language of generalization is easy to spot when a property is predicated of a category: "Ks F" (e.g., "Dogs bark").
This is often called *generic language* or *generics* [@Carlson1977].
But generalization can also manifest when describing an individual (*habitual language*; e.g., "John smokes").
Verbs like *causes* or *makes* also seem to convey generalization, in this case, about an intance of a causal event (*causal language*; e.g., "Staring at the sun makes you go blind"). ^[
  We adopt this somewhat idiosyncratic categorization in order to emphasize the different semantic types predicated. 
  Some authors refer to all of these types as *generics*.
  Still, however, the primary "generics" investigated in the literature have been generalizations about categories ("Ks F").
]
Psychologists, linguists, and philosphers have long studied the language of generalization, as it appears very simple (e.g., syntactically) yet its meaning is difficult to formalize.^[
  We take up the problem of formalizing a *meaning* for the language of generalization as opposed to the problem of *identifying* generalizations. 
...
If generalization from observations can be described by a probability $p_{fk}$, it is natural to posit that same construct will be at the heart of a semantic theory of the language of generalization.
The semantic tools of *truth values* provide a way to update a state of knowledge by relating the semantic quantity (probability) to  Boolean values [@Montague1973].
The simplest way to relate a scalar quantity like probability is via a *threshold semantics*: The utterance is true if the relevant scalar value is above a threshold.
Thus, the simplest semantics for a generalization about category K with property F is a threshold on the *prevalence*: $\denote{K \text{ } F}(p_{fk}, \theta) = \{p_{fk} >\theta\}$.

Attempts of this kind have been made before in formal semantics [e.g., by positing that $\theta = 0.5$; @Cohen1999], but heretofore no model has made precise, quantitative predictions about adult judgments. 
In addition to articulating an explicit semantic hypothesis, we present a Bayesian cognitive model that uses this semantics in order to update its beliefs from language.
We then ask: When would a rational speaker choose to produce a generalization (vs. staying silent) to update the Bayesian model's state of knowledge?

<!-- We distinguish our proposal from previous ones by emphasizing the predictive nature of $p_{fk}$, as well as the formalization into a model that makes precise, quantitative predictions about human endorsements. -->
<!-- $p_{fk}$ describes a latent belief and should not be confused with *frequency*, which can inform a belief but is not itself the belief. -->

<!-- We emphasize that this account is based on a probability $h$. -->
<!-- The literature on generic language has often focused on *prevalence*---either the actual or believed proportion of members of a category with the property---which we distinguish from the prevalence $h$.^[ -->
...
The simplest semantics for a generalization is a threshold function: $\denote{gen}(p_{fk}, \theta) = \{p_{fk} >\theta\}$, but what threshold is appropriate?
Quantifier statements can be described using particular values of $\theta$: $\denote{some}(p_{fk}) = \{p_{fk} > 0\}; \denote{all} = \{p_{fk} = 1\}$.
The extreme flexiblity of generalizations (e.g., "Mosquitos carry malaria"; "Birds lay eggs" vs. "Birds are female") suggests that no fixed threshold would suffice. 
Indeed, no particular threshold can suffice: The semantics of the language of generalizations is *underspecified*.
To resolve this underspecification, we hypothesize that $\theta$ is determined by context, in a way analagous to how gradable adjectives like "tall" have contextually-determined thresholds [e.g., what counts as "tall" for a four-year-old is different than what counts as "tall" for an adult; @Kennedy2007; @Lassiter2013].
...
We can formalize this underspecified semantics in a Bayesian cognitive model, where the threshold $\theta$ is sampled from a uniform prior distribution over possible thresholds $P(\theta)$ [@Lassiter2013; @Lassiter2015; cf., @Qing2014].
The model also has prior knowledge about the property $P(p_f)$ and so literal interpretation is given by updating these prior beliefs according the truth-functional meaning of utterance $u$:

\begin{eqnarray}
Lit(p_{fk}, \theta \mid u) &\propto& {\delta_{\denote{u}(p_{fk}, \theta)} \cdot P(p_{fk}) \cdot P(\theta)} \label{eq:L0}
\end{eqnarray}

Formally, the truth-functional meaning is represented by the Kronecker delta function  $\delta_{\denote{u}(p_{fk}, \theta)}$ that returns probabilities proportional to $1$ when the utterance is true (i.e., when $p_{fk} > \theta$) and $0$ otherwise.
Following @Lassiter2015, we defined $P(\theta)$ to be a uniform distribution over the support of $P(p_{fk})$.
...
Given the above model of literal interpretation, how do we decide when a statement conveying a generalization is true or not?
We consider the notion of felicity or endorsement from the information-theoretic perspective: Would a generalization convey the appropriate information to the naive interpreter? 
We formalize this by considering how well the generalization would convey the prevalence $p_{fk}$ to the $Lit$ model (Eq. \ref{eq:L0})^[
  A more general version of this model can relax the assumption that the endorsement model has access to a specific prevalence $p_{fk}$ that it wants to communicate.
  Rather, the endorser may have a distribution over $p_{f}$ for the category $k$, corresponding to the endorser's uncertain beliefs about the prevalence. 
...
The endorsement model simulates what the interpretation model would believe upon hearing an utterance $u$: $Lit(p_{fk}, \theta \mid u)$.
$S$ does not know $\theta$ and integrates over the interpreter's posterior beliefs about $\theta$.
Following Bayesian decision theory, we assume the decision to endorse is approximately rational, with degree of rationality given by $\lambda$.
...
Our endorsement model (Eq. \ref{eq:S1}) provides a mapping from a *referent-category prevalence* $p_{fk}$  (e.g., $P(x \text{ lays eggs} \mid x \text{ is a robin})$) to an endorsement probability for the corresponding generalization (e.g., "Robins lay eggs"): $S(u = u_{KF} \mid p_{fk})$. 
...
This model can be seen as a special case of a Rational Speech Act model [@Frank2012; @Goodman2016] with only a single level of recursion and the space of possible utterances maximally constrained; the endorsement model is a "speaker", while the interpretation model is a "listener". 

### Prevalence priors

The uncertain, truth-conditional threshold $\theta$ is resolved by the literal interpretation model $Lit$ (Eq. \ref{eq:L0}) by integrating the model's prior beliefs about the feature in question---the *prevalence prior* $P(p_fk)$---with the underspecified semantics.
Because $P(\theta)$ is assumed to be a uniform distribution across all contexts, a generalization could in principle be felicitious with any referent-prevalence. 
However, the interpretation model's *a priori* beliefs $P(p_fk)$ must be considered before the endorsement model can say whether or not would it want to update those beliefs by endorsing the statement. 

<!-- Our model makes context-sensitive predictions about endorsing generalizations based on two model components: The prevalence priors $P(p_fk)$ in Eq. \ref{eq:L0} and the referent-prevalence $p_{fk}$ in Eqs. \ref{eq:S1} and \ref{eq:delta}. -->
...
Beliefs about properties result from internal conceptual models of the world. 
Without appealing to domain specific knowledge, one can construct a prior distributions over prevalence by considering the prevalence of the feature among *different* categories.
For example, many different kinds of animals *do not bark* (more precisely: have close to 0 exemplars who bark); thus, the prevalence prior for the feature *barks* $P(p_{barks})$ should have substantial probability mass near 0, because one would a randomly-sampled category to have 0 or close to 0 exemplars that bark.
Furthermore, among the categories that do have exemplars which bark (e.g., *dogs*, *seals*, ...), the probability is quite high that an exemplar will bark; thus, the corresponding prevalence prior would have substantial mass at relatively high prevalences, potentially with some uncertainty; Figure\ \@ref(fig:simulations); row 1, column 1).
Abstract beliefs between different kinds of categories would manifest in these prevalence priors in the form of multi-modal distributions; for example, the prevalence prior for *lays eggs* should similiarly have substantial probability mass near 0 (since many animal categories do not have egg-layers) but should have a secondary component peaked around 0.5, because among the animals with egg-layers, only the female members of the category have the property (Figure\ \@ref(fig:simulations)A; row 1, column 4).

By encoding knowledge about other categories, the prevalence prior distributions are deeply connected to the construct of *cue validity*, or the probability that an entity with a certain property $f$ is a member of kind $k$: $P(x \in k \mid x \in f)$ (e.g., one's predictions about whether or not an entity is a mosquito, upon learning that it carries malaria). 
This statistic has been shown to be highly correlated with endorsements of generalizations about categories, or *generics* [@Khemlani2012].
*Cue validity* implicitly encodes knowledge of other categories, by averaging over those categories $k'$ via the normalizing constant in Bayes' Rule:
$P( k \mid  f) = \frac{P( f \mid  k) \cdot P( k)}{\sum\limits_{k' \in K} P( f \mid k') \cdot P( k')}$.
Since cue validity is defined for a particular category--property pair, it can be seen as a point estimate of the prevalence prior distribution.
In particular: The normalizing constant for calculating cue validity ($\sum_{k' \in K} p_{fk'} \cdot P(p_{fk'} )$) is the expected value (i.e., the mean) of the prevalence prior distribution: $\mathbb{E}[P(p_{f})]$.
For a slower derivation of this relationship, see Appendix A.
...
<!-- In the prevalence prior $P(p_f)$, a category is represented only by its corresponding prevalence (i.e., a sample from the prevalence prior can be thought of as a category $k$, with the only information available about the category being the prevalence of the feature in that category $p_{fk}$).  -->
<!-- Thus, $P(p_{fk}) = P(k_{f})$. -->
...
The endorsement model defined in the previous section explicitly predicts that the variability in endorsements for generalizations in language can be explained by two factors: (i) the prevalence communicated in the category under discussion $p_{fk}$ in Eq. \ref{eq:S1} (e.g., $P(x \text{ lays eggs} \mid x \text{ is a robin})$) and (ii) the background knowledge about prevalences $P(p_f)$ (Eq. \ref{eq:L0}).
...
```{r uniformPriorSimulation, fig.cap = "Computational model behavior. A: Interpretation model posteriors assuming different fixed thresholds. High thresholds rule out more world-states (prevalence levels), leaving fewer world-states among which to distribute the full probability mass. B: Full interpretation model marginalizes over possible thresholds to return a posterior distribution that favors higher prevalence levels in a graded manner. C: Endorsement model predicts higher rates of endorsemens as prevalence levels increase.", fig.width = 13, fig.asp = 0.3, out.width = "100%", fig.align = "center", cache = F}
...
Figure\ \@ref(fig:uniformPriorSimulation) shows different components of the model behavior assuming a uniform prevalence prior. 
...
As the threshold decreases, more and more prevalence levels would be true  (Figure\ \@ref(fig:simulations)A).
The uncertain-threshold interpretation model averages over these possibilities, resulting in a posterior distribution that favors higher prevalence levels (Figure\ \@ref(fig:uniformPriorSimulation)B).
The endorsement model thus predicts higher rates of endorsement as the speaker's belief about the prevalence (referent-prevealence) increases (Figure\ \@ref(fig:uniformPriorSimulation)C). 

<!-- This reasoning process favors lower thresholds because they are consistent with more prevalences. -->
<!-- Low thresholds (e.g., $\theta = 0.25$) rule out very few possibilities (only $p_{fk} \leq 0.25$), and thus an interpretation with a low-threshold still has a lot of uncertainty about the prevalence (Figure\ \@ref(fig:simulations)A, left).  -->
...
Hearing "Ks bark" tells the literal interpretor that *almost all* Ks bark, though there is a good deal of uncertainty in the precise quantity (column 1).
...
The endorsement model (Eq. \ref{eq:S1}) predicts endorsement probabilities based on the literal interpretation of the utterance and the referent-category prevalence (e.g., how many robins lay eggs; shown in arrows in Figure\ \@ref(fig:simulations)A).
...
Intruigingly, the model predicts "Robins are female" (which has the same prevalence as "Robins lay eggs") should receive intermediate endorsement: It is neither true nor false.
...
These simulations show how the model predicted endorsement depends upon the background knowledge about the property $P(p_f)$ as well as the referent-prevalence $p_{fk}$ communicated. 
...
We do this by both measurning and manipulating background knowledge and referent-prevalence and relate it to human elicited endorsements of generalizations.
...
We have argued that the core meaning of such linguistic expressions is simple, but underspecified. 
...
As a formal theory of the language of generalization, our framework aims to unify large swaths of language that are, on their surface, quite distinct from one another: "Birds lay eggs", "Sally smokes", "Herb X makes animals sleepy".
<!-- What these expressions have in common is not surface features but generalization (also referred to as *genericity*). -->
We have formalized a unified, core meaning of generalization in language using the underlying scale of predictive probability or *prevalence*.
We showed that endorsements of all three types of generalizations exhibit the same sensitivity to a background distribution comprised of other categories (Expt. 1, 2a \& 2b, 3).
As well, our framework naturally accounts for a number of logically puzzling category generalizations (Expt. 1) and provides an avenue for top-down moderators on language use, by elucidating the semantic scale as defined by a subjective, predictive probability, not mere frequency (Expt. 2c).
Finally, we have shown that listeners' prior knowledge about the prevalence of the feature in a category is directly related to their endorsement of statements of genericity (Expt. 3).
...
In this paper, we proposed a model for interpreting and endorsing generalizations in language by formalizing a simple semantic theory in a communicative model with shared background knowledge.
...
The endorsement model, which we use to generate predictions for our three case studies, is a special case of a speaker model who can only produce one of two utterances: the generalization or silence. 
The interpreter model is a "literal listener", who updates its beliefs by averaging over plausibly thresholds given prior knowledge and the simple truth-functional meaning.
...
The formal Bayesian model we articulate provides a clean separation of the semantics of a generalization from background, world knowledge.
This formal separation is a key advancement beyond previous accounts, which have either aimed to account for context-sensitivity by positing distinct semantic constructs [e.g., *relative* vs *absolute* generics, described below; @Cohen1999] or positing a semantics that cannot be separated from world knowledge [e.g., @Leslie2008].
...
That is, we provide a route for precise quantitative influence of conceptual knowledge on generic language by way of shared background knowledge between speakers and listeners.
In the rest of this discussion, we elaborate some key features of the theory that warrant further investigation, sketch out arguments for other known philosophical puzzles in genericity, and situate our theory with respect to extant theories of genericity.
...
## Prior knowledge and kind representations

Our theory assumes that listeners and speakers share relevant background knowledge (in the form of a *prevalence prior*) against which generalizations are interpreted.
Given this representation of background knowledge (*prevalence prior*), language interfaces with conceptual knowledge via *prevalence*. 
Previous psychological and philosophical work on generics has looked beyond prevalence and focused kinds and their conceptual relations to properties [@Leslie2007; @Leslie2008; @Prasada2013].
Indeed, we see generics as describing kinds, but argue that the reason why particular generics are licensed in a context is about the information conveyed to a listener relative to their background knowledge, formalized with by prevalence prior.
In the right situations, this shared background knowledge can presuppose beliefs about natural kinds and assume richly structured relations between categories and the properties [a la @Prasada2013].
In other situations, relevant background knowledge may be merely correlational in nature.
...
We believe such probabilistic concept representations can be adopted as a formal analog to an intuitive theory of natural kinds [@Gelman2003; @Keil1992].
We do not assume a particular view of natural kinds in our model, but note that viewing natural kinds as richly structured categories with clusters of non-obvious properties is compatible with our information-theoretic communicative theory. 
The important commonality among different domain-specific background theories is that they each have a statistical layer to them (i.e., they each give rise to beliefs about *prevalence*), which language interfaces with via the tools of truth-functional semantics.
Thus, our theory provides a separation between background knowledge and the semantics of generalizations; the interface between the two is predictive probability or *prevalence*. 
...
<!-- Our approach makes the strong claim that predictive probabilities are the connective tissue between conceptual knowledge and the semantics of generalizations. -->
<!-- That is, the effect of conceptually meaningful differences on generic language is predicted to be mediated by differences in corresponding speaker beliefs or listener prevalence distributions, which can occur for multiple reasons (e.g., by changing the comparison class). -->
<!-- We found in Expts. 1 \& 2 that conceptually different kinds of properties and events gave rise to quantitatively different prevalence priors, even while keeping the comparison class fixed.  -->
<!-- In Expt. 3 we showed that these prevalence distributions are causally related to endorsement. -->


In our modeling work, we found that the empirical data which underlie the prevalence prior distributions are best modeled by a mixture-model with two mixture components.
Such a prior distribution draws qualitative distinctions between different kinds of categories by virtue of the prevalence prior model representing multiple distinct components. 
Actual conceptual knowledge against which generalizations are interpreted is most likely far-richer than simply a two-component mixture-model.
We note, however, that a richer model of the prior is not necessary to account for the context-sensitive endorsement patterns we observe in our empirical data. 
...
Our simple endorsement model assumes that the goal of communication is the prevalence (Eq.\ref{eq:S1}).
In other words, the endorsement decision is computed with respect to the prevalence information conveyed to the listener.
...
We note, however, that the endorsement model makes almost identical predictions if the QUD in the model is assumed to be about the abstract parameters of the background knowledge (e.g., which mixture component the referent is part of).
...
We distinguish this construct from other uses of *prevalence* in the literature, namely the usage as synonymous with frequency.
<!-- the distinction that what is communicated is probability not frequency. -->
<!-- In previous accounts, prevalence is treated as synonymous with the frequency of the feature in the category.  -->
...
Predictions about the presence of a feature can be the result of beliefs about internal causes of the feature [e.g., in an essentialist view of natural kinds; @Gelman2003] or merely correlational happenstance.
...
Rather than conveying *explicit probabilities*, generalizations we argue convey *implicit probabilities*.
...
Indeed, young children are actually quite good with reasoning about probabilities, but in ways that are not explicit or tied to the actual language of probability [@Xu2008; @xu2009statistical].
...
The probabilistic communicative model we introduce in this paper assumes shared background 
<!-- Our model of an interpreter, or naive listener, has access to implicit  -->
statistics of the event or property in question, represented by the prior belief distribution over the prevalence $P(h)$.
...
Thus, the prevalence prior $P(h)$ is actually a conditional distribution constructed with respect some comparison class $C$: $P(h \mid C)$.
...
An important problem for comparison classes for both gradable adjectives and generalizations is that the comparison class often goes unsaid (e.g., "John is tall for a person" is often produced as just "John is tall").

Uncertainty about the comparison class adds yet more opportunity for miscommunication, deceit, or just faultless disagreement to the language of generalization.
Even if interlocutors roughly coordinate on a particular comparison class (e.g., agreeing that they are talking *relative to other animals*), they may disagree about the prevalences for other categories in the comparison class.
That is, our model makes the prediction that the acceptance or rejection of controversial statements (e.g., "Muslims are terrorists") depends upon a speaker's beliefs about how often other categories exhibit the property (e.g., neo-nazis commiting acts of terrorism).
In Expt. 3, we empirically demonstrated the influence of other categories on endorsement of a generalization.
...
Finally, interlocturs may not only fail to coordinate on what categories are in the comparison class, the prevalences that may be assumed for the constituent categories of the comparison class, but the very nature of *how* the comparison class should be constructed.
The comparison classes used in our case studies were with always respect to the *category* (*other animals*, *other people*, *other possible causes*); thus, our model addresses the implicit QUD of "*what* has feature?".
...
It's long been noted that the same generalization can be used to address multiple QUDs [@KrifkaGenericBookFocus], and we posit that differences in interpretation are the result of multiple distinct comparison classes competing for influence.
...
Future work should investigate the factors that give rise to feature-wise interpretations of generalizations, and how the relevant comparison classes are constructed. 
...
Generalizations are often contrasted with quantifier language (e.g., "some", "most", "all", ...), which is in fact *easier* to formalize, but more difficult for young children to acquire [@Brandone2014; @Gelman2015].
...
This asymmetry between the simplicity of the formal theory and the age of acquisition has led some to conclude that the normal tools for describing the semantics of quantified utterances (i.e., a truth-functional threshold) is inapprorpriate for generalizations  [@Leslie2008].

Rejecting the tools of truth-functional semantics for generalizations would be throwing the baby out with the bathwater.
The acquisition of a threshold-based truth-functional meaning $\denote{u}(h, \theta) = \{h :h > \theta\}$ requires learning at least three distinct aspects of meaning: 
(1) the *dimension* being described (i.e., probability $h$),
...
Upon repeated cross-situational data, this prior could be updated towards acquiring a fixed meaning.
However, for generalizations, no fixed threshold is required; the language of generalization is learned once aspects (1) and (2) are understood.
...
Uniform uncertainty over values in the unit interval $[0, 1]$ for threshold $\theta$ in a threshold-function (as we have assumed in our model) is mathematically equivalent to a *soft semantics* wherein the degree to which the utterance is true is proportional to the degree itself, in this case prevalence $h$: $\int_{0}^{1} \delta_{h > \theta} \diff \theta =  h$
...
In this paper, we aimed to balance two competing goals in designing our experimental stimuli: (1) including some familiar, philosophically puzzling examples that have been discussed in the vast literature on generic language and (2) introducing novel stimuli that provide targeted tests of our model vis-a-vis alternative models.
In this section, we discuss how a few other previously-discussed troubling examples of generalizations could be addressed in our communicative framework.
We consider the five following examples:

1. Mary handles the mail from Antarctica.
...
In our framework, we treat Sentence (1) is a boundary case of generalizations expressing predictive probabilities.
In Expt. 2c., we experimentally dissociated predictive probability from past frequency, and saw that only a model that took into account predictive probability could explain participants' endorsements of generalizations.
We expect the same to be true with Mary, elucidated by the intuition that: were there to be any mail coming from Antarctica, Mary would handle it. 
...
Our focus on predictive probability makes even novel predictions about some classic puzzles.
Sentence (2) is the conceptual opposite of sentence (1).
Imagine that currently every Supreme Court Justice has a social security number which is an *even number*. 
Sentence (2) is still considered false, every though the property holds in exactly 100\% of the category [@Cohen1999].
We predict the rejection of Sentence (2) is the result of abstract, intuitive theories guiding our subjective probabilities.
That is, because observers strongly believe one's social security number does not infleunce selection for the Supreme Court, speakers would assign a roughly 50\% subjective probability that the *next* justice would have an even social security number.
...
That is, we predict there are situations where statements like (2) could be considered felicitious generalizations.
...
In fact, with prior knowledge pushing against the existence of *international elephants* and without any further semantic assumptions, our model can interpret Sentence (3) as meaning some elephants live in Africa and that different ones live in Asia.^[
...
Importantly, an incremental parsing of Sentence (3) leads our model to first believe that *most* elephants live in Africa (when it only hears that "Elephants live in Africa"), and then later believe that *some* elephants live in Africa (while others live in Asia) after hearing the end of the utterance ("...and Asia").
Our model flexibly accomodates new evidence that might otherwise contradict previous utterances because it maintains uncertainty about the precise meaning of the utterance.
...
Why?
We first observe that the sentence is constructed by pairing a category ("Books") with a property that describes a strict subset (i.e., "paperbacks", or "paperback books"). 
We hypothesize that the strangeness of this example has to do an inflexible relationship between the comparison class and the referent-prevalence.
Assume that the comparison class is constructed by considering the *predicability of the property* [in the sense of @Keil1979, originally from @Sommers1963].^[
...
Then, "is a paperback" can only be informatively said of individual books, because these are the only objects that can plausibly have the property; that is, world knowledge gives us that an elephant cannot be predicated (either true or falsely) by the property "is a paperback".^[
  Note that this idea is related to @Cohen1999's notion of "Relative generics" (described more fully below), which have truth conditions that are determined by a comparison to an alternative set of Ks (his   Alt(K), our comparison class) which is composed of Ks that could satisfy some F in an alternative set of Fs.
]
...
Finally, a property can be widespread relative to other kinds or groups as well as being so systemic that we would predict it to be the case in the future, and speakers still might find it aversive to actually endorse the generalization.
...
We propose adopting such a value-based utility structure for states of the world for language about social categories: If the speaker found states of the world where the probability of the feature is high as undesirable (e.g., a world where many women were submissive), then a speaker who considered both the informational content of the utterance and the utility in the states of the world implied by that utterance would too find it reprehensible to make such a generalization.

In sum, we've tried to argue that our information-theoretic communicative model couched in our general cognitive framework can be used to explain a number of interesting and heretofore puzzling phenomena in genericity as well as make novel predictions about a wider range of language.
...
To our knowledge, the present paper presents the first formal theory of genericity in language that makes accurate and precise quantitative predictions about human behavioral data. 
A number of other theories of genericity have been proposed in the formal semantics  literatures.
...
These theories fall into two broad camps: Those that appeal to the statistics of the world (e.g., how many Ks have F) and those that appeal to conceptual representations (e.g., *there is something about being K which causes it to F*).
...
The relation of our account to the conceptual accounts of @Leslie2008 and @Prasada2012 has been described above.
We reiterate that the key point of divergence between the conceptual accounts and our view concerns the core, semantic meaning of the language of generalization.
...
We believe this provides a more unifying view of generics, giving a single metric as the basic currency for the language of generalizations: the metric of probability. 
...
In our model, we do not draw a *relative* vs. *absolute* distinction, instead treating all generics as relative. 
Attested differences in endorsement and interpretation, then, emerge through the interplay of prior knowledge.^[
  For a related theory argument against the "relative" / "absolute" distinction for gradable adjectives, see @Lassiter2015.
]
...
In contrast, we posit an uncertain threshold which is resolved through a Bayesian interpretation model; our theory is a joint semantic--pragmatic theory. 
...
We note additional machinery that Cohen's theory relies upon (and that is not currently required in our account) to explain the context-sensitivity of generics: contextually restricting the entities that go into the computation of prevalence (i.e., which robins do we look at to compute the probability of *laying eggs* among *robins*?).
His theory posits that prevalence is calculuated by only considering entities that *could have some feature* in a contextually-specified alternative set of features (so called $Alt(F)$).
...
"Robins lay eggs" is evaluated as an *absolute generic* as only individuals that are female members of kinds are under consideration, because only the female members can plausibly satisfy one of the other aternatives having to do with reproduction (i.e., the alternative features in $Alt(F)$).
The inferential machinery behind Cohen's domain restriction (i.e., what comprises $Alt(F)$) relies upon conceptual information, but the details remain obscure [@Carlson1995essay].^[
...
However, we note that this mechanism may be implicit in our own theory: the prior distribution over the prevalence of *lays eggs* presumably includes aspects of people's theories about reproduction (i.e., that only females lay eggs).
...
@Sterken2015 has proposed analyzing generic statements as inherently context-sensitive, analagous to *indexicals* (e.g., "this" or "I").
Variability of endorsements is attributed by @Sterken2015 to context-sensitive "quantificational force" as well as domain-restrictor [of the kind used by @Cohen1999, and many others].
...
By leveraging this common ground, the language of generalizations provide a powerful way to communicate and learn generalizations, which would otherwise be difficult or costly information to learn through direct experience.
