...
Furthermore, flexibly communicating generalizations from one generation to the next is a mechanism by which culture can cumulatively evolve [@Tomasello1999; @Henrich2015].
...
Despite its ubiquity and relative morphosyntactic simplicity, the language of generalization has been addressed by few formal models and none that have made precise, quantitative predictions that match human intuitions. 
...
These observations have led some to conclude that the prevalence of the feature (or the probability of a member of the category having the property e.g.,  $P(x \text{ barks} \mid x \text{ is a dog})$) is not what makes generalization true or false.
...
"Bishops move diagonally" or "The Speaker of the House succeeds the Vice President" are true statements not because of a tendency on behalf of the category to uphold the property.
Rather, the utterances are believed to point out the existence of a conceptual structure (e.g., that bishops have a certain role in the game of chess).
...
Instead, we posit that the cut-off is *underspecified* in the literal meaning of a generalization, and that listeners' background knowledge and the communicative context can be used to derive more precise interpretations in the moment. 
...
With these ideas, we create an endorsement model, providing the first formal theory that makes quantitative predictions about what makes generalizations in language true or false.
...
In Section 1, we formalize the hypothesis that the core meaning of a generalization in language is a simple but underspecified threshold, articulate a theory of relevant background knowledge used to understand a generalization in context, and develop a computational model of endorsement around these components.
...
Sections 2-4 present three empirical case studies: generalizations about categories (*generic language*), events (*habitual language*), and causes (*causal language*). 
...
While alternative models fall short, we find a very strong agreement of our model's predictions to the full spectrum of human elicited endorsements.
