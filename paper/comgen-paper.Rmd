...
These predictive probabilities are fundementally mental representations, as opposed to observed frequency, which is often taken to be the object of statistical approaches to genericity.
...
Our theory suggests that the problems with understanding probabilities observed in classic, cognitive psychology paradigms are a problem in understanding the numerical language used to convey explicit probabilities [cf., @Levinson1995]. 
Rather than conveying probabilities *explicitly*, generalizations we argue convey them *implicitly*.
...
Indeed, young children are actually quite good with reasoning about probabilities, but in ways that are not explicit or tied to the (historically quite recent) numerical language of probability [@Xu2008; @xu2009statistical].
...
Such a prior distribution implies a qualitative distinctions between different kinds of categories, perhaps by virtue of having (or lacking) certain causal mechanisms that can brings about the target feature.
Actual conceptual knowledge against which generalizations are interpreted is most likely richer than this.
...
Finally, the comparison classes used in our case studies were with always respect to the *category* (*other animals*, *other people*, *other possible causes*); thus, our model addresses the implicit QUD of "*what* has feature?".
\ndg{Note QUD might not have been introduced, depending on where we move above floating paragraph..}
