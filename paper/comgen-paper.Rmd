...
Our model makes predictions about endorsing generalizations based on two externally-determined model components: The prevalence priors $P(p_f)$ in Eq. \ref{eq:L0} and the referent-prevalence $p_{fk}$ in Eqs. \ref{eq:S1} and \ref{eq:delta}.
...
For now, the referent-prevalence is simply what determines the truth-conditional semantics. 
Our hypothesis does not provide a mapping from the truth-conditional semantics directly to endorsement judgments, because $\theta$ is underspecified.

The truth-conditional threshold $\theta$ is resolved by the literal interpretation model $Lit$ (Eq. \ref{eq:L0}) by integrating prior belief about the feature in question---the *prevalence prior* $P(p_f)$---and the uncertain threshold $P(\theta)$ with the truth-functional semantics.
Again, $P(\theta)$ is set to be a uniform distribution across all contexts, and thus, the generalization could in principle be felicitious with potentially any prevalence. 
However, what the interpretation model believes *a priori* $P(p_f)$ must be considered before the endorsement model can say whether or not it would endorse the generalization. 

Beliefs about properties result from internal conceptual models of the world. 
These internal models have domain-specific features but should all give to the domain-general currency of probability.
One way of constructing prior distributions over prevalence, without appealing directly to domain-specific conceptual knowledge, is by considering the prevalence of the feature among different categories.^[
  An alternative way is to consider the prevalence of different features among the same category. 
  We return to this point in the general discussion.
]
For example, many different kinds of animals *do not bark* (more precisely: have close to 0 exemplars who bark); thus, the prevalence prior for the feature *barks* $P(p_{barks})$ should have substantial probability mass near 0 .
...
Indeed, previous studies have found cue validity to be highly correlated with endorsing generalizations about categories, or *generic language* [@Khemlani2012].
Cue validity is defined for a particular category--feature pair, and relates to referent-prevalence by Bayes' Rule:
$$ P(x \in k \mid x \in f) = \frac{P(x \in f \mid x \in k) \cdot P(x \in k)}{\sum_{k' \in K} P(x \in f \mid x \in k') \cdot P(x\in k')} $$
...
Thus, $P(x \in k) = P(k) = P(p_{fk})$).
The denominator of the cue validity expression above (the normalizing constant), then, is the expectation (i.e., the mean) of the prevalence prior distribution: $\mathbb{E}[P(p_{f})] = \sum_{k' \in K} p_{fk'} \cdot P(p_{fk'} )$.
This relationship reveals a potential shortcoming of a point estimate like *cue validity*; the prevalence prior potentially encodes structured, conceptual knowledge of the world, which would be lost by reducing it to a single value.
...
We see the model correctly predicts "Dogs bark", "Robins lay eggs", and "Mosquitos carry malaria" are all true, despite the referent-prevalences varying substantially between these three examples.
...
These simulations show how the model predicted endorsement depends upon the background knowledge about the property $P(h)$ as well as the prevalence $h'$ communicated. 
