...
Acquiring understanding that extends beyond the present context is crucial to thrive in our open-ended, dynamic world. 
...
The *language of generalization* is most commonly studied with *generic language* (*generics*) or generalizations about categories [e.g., "Dogs are friendly"; @Carlson1977; @Cohen1999; @Leslie2007; @Nickel2008].
...
Despite its ubiquity and relative morphosyntactic simplicity, the language of generalization has been addressed by few formal models of and none that have made precise, quantitative predictions that match human intuitions. 
...
These observations have led some to conclude that the prevalence of the feature (or the probability of a member of the category having the property e.g.,  $P(x \text{ barks} \mid x \text{ is a dog})$) is not the only factor that goes into making a generalization true or false.
...
Ravens and toasters have no privileged relationship to each other, but "Ravens are bigger than toasters" is still true.
An understanding of generalizations just in terms of conceptual relations must be supplemented with the proviso that, in some situations, the actual statistics of the feature matters for making generalizations true.
...
In Section 1, we formalize the hypothesis that the core of meaning of a generalization in language is simple but *underspecified*, articulate a theory of relevant background knowledge used to understand a generalization in language, and develop a computational model of endorsement around these components.
...
We find a very strong agreement of our model's predictions to the full spectrum of human elicited endorsements, where alternative models fall short.
...
Abstract, potentially domain-specific, beliefs about properties can be represented by a probability distribution over the prevalence $P(p_{fk})$ [@Kemp2008].^[
  We adopt the following notational simplification: $p_{fk} = p(f \mid k)$. 
]
...
Observational data $d$ is not always available, however.
Instead, we must listen to others to learn about properties that are costly to observe (e.g., *staring at the sun makes you go blind*), events that are statistically unlikely (e.g., *lightning strikes tall objects*), or just aspects of the world that we have yet to experience.
...
The language of generalization is easy to spot when a property, that could project onto an individual, is predicated of a category (e.g., "Dogs are friendly"; *barks* could apply to an individual as in "This dog is friendly").
This is often called *generic language* or *generics* [@Carlson1977].
Generic statements can be contrasted with kind-denoting statements which use properties that can only apply to the category as a whole (e.g., "Dinosaurs are extinct"; *extinct* is not a property that could apply to an individual).
Generalization can also manifest when describing an individual (*habitual language*; e.g., "John smokes").
Verbs like *causes* or *makes* also seem to convey generalization, in this case, about an instance of a causal event (*causal language*; e.g., "Staring at the sun makes you go blind"). ^[
...
Rather than throw out the threshold-semantics, we posit this semantics is *underspecified*.
We hypothesize that $\theta$ is determined by context in a way analogous to how gradable adjectives like "tall" have contextually-determined thresholds [e.g., what counts as "tall" for a four-year-old is different than what counts as "tall" for an adult; @Kennedy2007; @Lassiter2013].
...
We formalize this underspecified semantics in a Bayesian cognitive model, where the threshold $\theta$ is sampled from a uniform prior distribution over possible thresholds $P(\theta)$ [@Lassiter2013; @Lassiter2015; cf., @Qing2014].
The model also has prior knowledge about the property $P(p_fk)$ and literal interpretation is given by updating these prior beliefs according the truth-functional meaning of utterance $u$:
...
The uncertain, truth-conditional threshold $\theta$ is resolved by the literal interpretation model $Lit$ (Eq. \ref{eq:L0}) by integrating the model's prior beliefs about the feature in question---the *prevalence prior* $P(p_{fk})$---with the underspecified semantics.
...
However, the interpretation model's prior beliefs $P(p_{fk})$ must be considered before the endorsement model can say whether or not would it want to update those beliefs by endorsing the statement. 
...
In particular: The normalizing constant for calculating cue validity ($\sum_{k' \in K} p_{fk'} \cdot P(p_{fk'} )$) is the expected value (i.e., the mean) of the prevalence prior distribution: $\mathbb{E}[P(p_{fk})]$.
For a more detailed derivation of this relationship, see Appendix A.
...
We do this by both measuring and manipulating background knowledge and referent-prevalence and relate it to human elicited endorsements of generalizations.
...
Finally, we have shown that listeners' prior knowledge about the prevalence of the feature in other categories is directly influences their endorsement of generalizations for the same category (Expt. 3).
...
This formal separation is a key advancement beyond previous accounts, which have either aimed to account for context-sensitivity by positing distinct semantic constructs [e.g., *relative* vs *absolute* generics, described below; @Cohen1999] or positing a semantics that cannot be separated from world knowledge [@Leslie2008].
...
Previous psychological and philosophical work on generics has looked beyond prevalence and focused on kinds and their conceptual relations to properties [@Leslie2007; @Leslie2008; @Prasada2013].
Indeed, we see generics as describing kinds, but argue that the reason why particular generics are licensed in a context concerns the information conveyed to a listener relative to their background knowledge, formalized with by prevalence prior.
In the right situations, this shared background knowledge can presuppose beliefs about natural kinds and richly structured relations between categories and the properties [a la @Prasada2013].
...
We note, however, that the endorsement model makes almost identical predictions for the data presented in this paper if the QUD is assumed to be about the abstract parameters of the background knowledge (e.g., which mixture component the referent is part of) as opposed to the prevalence itself.
...
Our theory suggests that the problems with understanding probabilities observed in classic, cognitive psychology paradigms are a problem in understanding the language used to convey explicit probabilities [cf., @Levinson1995]. 
...
Adult judgments of the felicity of gradable adjectives like *tall* or *dark* depend upon fine-grained details of the statistics of the comparison class [@Qing2014; @Schmidt2009; @Solt2012] in ways directly analogous to the context-sensitivity of the language of generalizations explored in this paper.
...
Finally, interlocutors may not only fail to coordinate on what categories are in the comparison class, the prevalences that may be assumed for the constituent categories of the comparison class, but the very nature of *how* the comparison class should be constructed.
...
This asymmetry between the simplicity of the formal theory and the age of acquisition has led some to conclude that the normal tools for describing the semantics of quantified utterances (i.e., a truth-functional threshold) is inappropriate for generalizations  [@Leslie2008].
...
However, were we to learn much more surprising information---for instance, if every Supreme Court Justice in U.S. history had a social security number which was a *prime number*---the shear auspiciousness could compel an observer to revise their theory of the domain (appealing perhaps to a conspiracy), update their subjective probability of future instances, and then accept the generic. 
That is, we predict there are situations where statements like (2) could be considered felicitous generalizations.
...
Our model flexibly accommodates new evidence that might otherwise contradict previous utterances because it maintains uncertainty about the precise meaning of the utterance.
...
A number of other theories of genericity have been proposed in the formal semantics  literature.
...
His theory posits that prevalence is calculated by only considering entities that *could have some feature* in a contextually-specified alternative set of features (so called $Alt(F)$).
...
@Sterken2015 has proposed analyzing generic statements as inherently context-sensitive, analogous to *indexicals* (e.g., "this" or "I").
Variability of endorsements is attributed by @Sterken2015 to context-sensitive "quantificational force" as well as domain-restriction [of the kind used by @Cohen1999, and many others].
