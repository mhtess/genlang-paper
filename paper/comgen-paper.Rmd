...
It has been the premier case study of generalization in language because of its similarity to quantified statements (e.g., "Most dogs bark").
Any theory of generic language must explain its puzzling flexibility of endorsements with respect to feature-probability (e.g., the number of dogs that bark; the number of mosquitos that carry malaria).

We first measure endorsement for thirty generic sentences that cover a range of conceptual distinctions discussed in the literature [@Prasada2013]: *characteristic* (e.g., "Ducks have wings."), *minority* (e.g., "Robins lay eggs."), *striking* (e.g. "Mosquitos carry malaria."), *false generalization* (e.g., "Robins are female."), and *absent features* (e.g. "Lions lay eggs.").
In additional to the canonical cases from the linguistics literature, we craft sentences to elicit the full range of acceptability judgments (intuitively: "true", "false", and "indeterminte") for generics with properties of low, medium, and high feature-probabilities (Expt. 1a).

We restrict our items to generics about animal categories so we may more easily measure the prior belief distribution over the feature-probabilities $P(h)$ (Expt. 1b).
Our measurement of the feature-probability priors includes measurements of the referent-category feature-probabilties $h'$ (e.g.,$P(x \text{ lays eggs} \mid x \text{ is a robin})$), allowing us to both generate predictions for the endorsement model (Eq. \ref{eq:S1}) as well as for simpler, alternative models (e.g., endorsement as a linear function of referent feature-probability). 
To preview our results, we find that only an endorsement model based on an uncertain-threshold semantics is able to account for the flexibility observed in generic endorsement.
...
<!-- We validate these simulations empirically by measuring t and the referent-category feature-probability that the speaker model seeks to convey (the *referent feature-probability*) $h$ in Eq. \ref{eq:S1} (Expt. 1a). -->
...
In this experiment, we elicit human endorsements for thirty generic statements taken from the linguistic and psychological literature on generics [@Prasada2013].
...
In order to better understand the process that generates this endorsement data, we measure the prior distribution over feature-probabilities and articulate a set of formal models that try to predict these endorsement data. 
...
One of these distributions is fixed to represent kinds of animals who *do not have* an internal mechanism that could give rise to the property (e.g., \textsc{lions} and \textsc{lay eggs}).
The second distribution is flexible and represents kinds of animals who *do have* such an internal mechanism; this distribution is flexible and can represent different shapes for different properties (e.g., the differences in feature-probability judgments for \textsc{lay eggs} vs. \text{has wings}).

Formally, we assume the prior data (analzyed independently for each property) was generated from one of two distributions: a distribution corresponding to those kinds with a stable causal mechanism that *could* give rise to the property ($\mathcal{D}_{stable}$) and a "transient cause" distribution corresponding to those kinds without a stable mechanism ($\mathcal{D}_{transient}$).
...
's data; however, we retain the imperfect simpler model for data-analytic ease going forward. 
...
We identify three, theoretically-substantial components of this model: (a) the prior distribution $P(h)$, (b) the endorsement deciding whether or not to produce the generic, and (c) the uncertain threshold.
