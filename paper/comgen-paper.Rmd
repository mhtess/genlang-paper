...
The properties can also vary widely (e.g., having fur, causing jitters, typing). 
Yet the mathematical description of the inductive inference is always given by the conditional feature-probability $h$.

How does one come to learn $h$, the uncertainty that underlies an inductive inference about a category having a property?
...
If an addition you observe that it has broken wings, the corresponding prediction of another instance having broken wings is not nearly as strong [@Nisbett1983].
This kind of knowledge can be represented formally in a probability distribution over the feature-probabilities $h$: $P(h)$ [@Kemp2008], and observational data $d$ can update one's beliefs via Bayes' Rule:
...
If generalization from observations can be described using probability, then it seems natural to consider probability to be central to the semantic content of an utterance that conveys a generalization.
To connect to the semantic tools of *truth values* [@Montague1973], we will need to relate the scalar quantity of probability to a Boolean truth value.
