...
abstract: > 
    ...
    Across these diverse domains, we find that our model explains almost all of the variance in human endorsements of language conveying generalization, while simpler models fall short.
...
But insofar as there is a single class of linguistic expressions that we can describe as communicating generalizations, there should be something common to them all: a general, unified semantic core.
...
To do so, we look to probability, a useful representation for human generalization from observations [@Shepard1987; @Tenenbaum2001].
...
Sections 2 - 4 presents our three empirical case studies: generalizations about categories (*generic language*), events (*habitual language*), and causes (*causal language*). 
...
How does one come to have an $h$ in their head?
...
Relying upon observational evidence can only take you so far, though. 
To learn about properties that are costly to observe (e.g., *staring at the sun makes you go blind*) or events that are statistically rare (e.g., *lightning strikes tall objects*), it is better to learn from others.
...
Generalization from observations can be described as a predictive probability; it is intuitive then to posit that same quantity to be at the core of a semantic theory of the language of generalization.
...
A threshold semantics for the language of generlaizaion would then apply to the *feature-probability* $h$:
...
### Semantics and background knowledge
...
Generics have been the case study of choice for generalizations in language because of theiry vexing similarity to quantified statements (e.g., "*Most* dogs bark").
...
Here, we take our first look at the 
