...
The prevalence prior $P(p)$ is a distribution over probabilities $p$, which is be the output of a generative model of particular domain.
...
The goal of the endorsement model is to align the listener's beliefs about prevalence (given by the *prevalence prior*) with the speaker's beliefs about the prevalence of the feature in the category being referred to, which we call the *referent prevalence* ($p$; e.g., that 50% of robins lay eggs). ^[
...
Given that the speaker only has two options (i.e., produce the generalization utterance or stay silent), the endorsement decision comes down to whether or not the referent prevalence is more likely under the prevalence prior distribution $P(p)$ (i.e., the listener's posterior upon hearing silence) or the listener generic interpretation distribution given by Eq. \ref{eq:L0} (Figure\ \@ref(fig:uniformPriorSimulation)C).
Thus, the communication-relevant content of the generalization is the prevalence in the referent category $p$.
The semantic threshold $\theta$ is necessary for establishing the precise truth conditions (used in Eq. \ref{eq:L0}) but is otherwise a nuisance parameter, which the endorsement model integrates out (note that there is no $\theta$ in the left-hand side of Eq. \ref{eq:S1}).
Finally, because Eq. \ref{eq:S1} models an information-theoretic speaker's decision, $S_1$ computes a belief distribution and then produces utterances which are approximately rational according to that belief distribution, via a soft-max function with degree of rationality $\lambda$.
\mht{better?}
