...
The human species is remarkable not only because we can extract useful generalizations from observations of the world, but because we can convey these generalizations to each other succinctly in language.
...
In each case, our model provided strong quantitative fit to human judgements while other models fall short.

Ours is a psychological model that leverages the insights of truth-functional semantics to articulate how an agent should update their beliefs based on a generic sentence. 
...
Here, we demonstrate how the context-sensitivity of generics can emerges via the interplay of a stable (but vague) semantics and diverse background knowledge. 
...
This result is promising for constructing models of speakers with the goal of teaching about the abstract relations between kinds and properties.
...
Recent empirical and computational modeling work suggests that pragmatic reasoning combined with world knowledge can flexibly adjust the comparison class to appropriately suit the utterance in context [@Tessler2017].
In this paper, we do not include such a mechanism in our model; instead, when possible, we make the comparison class clear to participants in order to study the remaning context-sensitivity with generics.
...
Finally, the comparison classes used in our case studies were constructed with respect to the category (*other animals*, *other people*, *other possible causes*).
...
This asymmetry between the simplicity of the formal theory and the age of acquisition has led some to conclude that the normal tools for describing the semantics of quantified utterances (i.e., a truth-functional threshold) are inappropriate for generics [@Leslie2008].
...
We leave for future work the precise implementation of the acquisition model sketched out here.
...
A number of other theories of genericity have been proposed in formal semantics and in psychology, which posit different constructs or psychological mechanisms to account for the context-sensitivity of generics.
...
Incorporating a hierarchical model of kinds and properties into our model of generic language is a natural future direction towards understanding the computational underpinnings of principled connections. 
Furthermore, we find it an exciting possibility that different conceptual structures may give rise to roughly the same distributions on prevalence (and thus have the same implied prevalence interpretation from our model), while nonetheless human listeners draw different inferences from generic statements [a la the differences observed for "prevalence-matched items" in @Prasada2006].
Such a result would not contradict our model, but would require further elaboration of how different conceptual structures relate to beliefs about prevalence. 
...
Attested differences in endorsement and interpretation, then, emerge through the interplay of prior knowledge withour uncertain semantics.^[
...
We note an additional mechanisms that Cohen's and other statistical theories rely upon (and not currently required in our account) to explain the context-sensitivity of generics: contextually restricting the entities that go into the computation of prevalence (i.e., which robins do we look at to compute the probability of laying eggs among robins?).
...
The inferential machinery domain restriction (i.e., what comprises $Alt(F)$) relies upon conceptual information, but the details remain obscure [@Carlson1995essay].^[
...
Our focus on predictive probability even makes novel predictions about some classic puzzles.
...
However, such is the only interpretation available if the generalization is given a fixed, majority based quantificational semantics, as posited in most statistical views of generics (e.g., if the generic means *most*).
...
When the sentence is completed ("...and Asia"), the model non-monotonically updates its beliefs to something weaker: *some* elephants live in Africa (and others in Asia.
...
Finally, Sentence (5) suggests that a property can be widespread relative to other kinds or groups, as well as being so systemic that we would predict it to be the case in the future (high *predictive prevalence*), and speakers still might find it aversive to actually endorse the generalization [@Haslanger2011]. 
...
Such a value-based utility structure for states of the world for language about social categories may capture intuitions like those about Sentence (5): If the speaker found states of the world where the probability of the feature is high as undesirable (e.g., a world where many women were submissive), then a speaker who considered both the informational content of the utterance and the utility in the states of the world implied by that utterance would too find it reprehensible to make such a generalization.

An alternative information-theoretic analysis of (5) could be made concerning the conceptual structure communicated by generalizations (see earlier discussion of *Prevalence Priors and Conceptual Structure*). 
...
The decision to endorse (5) depends upon what the speaker believes her listener will infer. 
If she is in a group of close friends discussing the ways in which society shapes behavior, the speaker may opt to endorse (5) because with high probability, she will be understood as endorsing the external, society cause. 
However, if she is worried that some might take her to mean that there's something about women that causes them to be submissive, the speaker may refrain from uttering (5).
