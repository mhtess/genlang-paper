...
<!-- Later we will test these predictions in a quantitative way using empirical data. -->
...
In this paper, we aimed to balance two competing goals in our experimental case studies: (1) drawing upon familiar, philosophically interesting examples that have been discussed in the vast literature on generic language and (2) introducing novel stimuli that provide targeted quantitative tests of our model vis-a-vis alternative models.
...
In our framework, Sentence (1) is an extreme case of generalizations expressing *predictive* probabilities.
...
Then, if we constructed the comparison class by considering other professionals, Sentence (2) would be rated by our model as similar to "Birds are female", because all professions have roughly the same probability of having employees with even social security numbers.
...
The present paper presents the first formal theory of generalization in language that generates quantitative predictions about human behavioral data. 
...
The dominant approach from formal semantics tries to describe unified, objective criteria by which to establish if a generic and other generalizations are true or false (i.e., generic *truth conditions*).
These views are typically *statistical* and appeal to quantification or the statistics of the world (e.g., how many Ks have F) to define those unified, objective criteria.
Theorists in philosophy and psychology have argued that what matters for producing and understanding generics is not the statistics of the world, but to mental representations or abstract beliefs in the head (e.g., *there is something about being a K which causes it to F*).
...
Many of the key factors in this conceptual approach are compatible with our predictive probability approach: What is required is that the distinctions made by conceptual accounts influence predictive probability, which mediates linguistic effects.
...
The conceptual view of generics argues it is not desirable to define generics in the same terms as quantifiers (e.g., a truth-functional threshold on prevalence), because generics are in many ways *more fundamental* than quantifiers [@Leslie2008; @Gelman2009].
...
Similarly, adults who are told novel information about categories using quantified language (e.g., "Most spiders shed their skin") and later asked to recall that information will tend to recall the information using generics [e.g., "Spiders shed their skin"; @Leslie2012], but not visa versa.
...
This similar increase in endorsement for striking and distinctive properties suggests that the prevalence prior $P(p)$ distributions for the two are similar to each other, and distinct from that of the neutral properties.
...
A popular alternative view under the statistical banner draws on the intuition that generics express something normative in the world [@Asher1995; @Pelletier1997; @Nickel2008; @Nickel2016]. 
