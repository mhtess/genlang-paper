...
In this paper, we proposed a model for understanding generalizations in language that formalizes a simple semantic theory that interfaces with interlocutors' shared beliefs over the statistics of the event or property in question, represented by the prior belief distribution over the feature-probability.
...
Collectively, these other kinds, people, or causes form classes against which the referent-category is evaluated.
That is, the feature-probability prior $P(h)$ is constructed with respect a class of other entities or events, *the comparison class* $C$: $P_C(h)$.
...
## Structured conceptual prior knowledge
