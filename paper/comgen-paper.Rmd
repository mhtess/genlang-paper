...
abstract: > 
    ...
    Yet such knowledge can be difficult to extract from the environment. 
    Fortunately, we are not limited to acquiring generalizations on our own; language allows us to communicate generalizations to each other (e.g., “Birds fly”, “John hikes”, “Drinking milk makes your bones strong"). 
    ...
    Here we argue that three ingredients are necessary to formalize generalizations: Probability, vagueness, and context. 
    We formalize these ingredients in an information-theoretic probabilistic model that makes quantitative predictions about human understanding of generalizations.
    Across diverse domains, we find that the model explains the gradience in human endorsement, while simpler models fall short. 
    This is the first formal theory that makes accurate and precise predictions about human understanding of generalizations in language and is the first step towards building models that can learn abstract knowledge from language.
...
The *language of generalization* covers a diverse swath of natural language expressions.^[
  Some writers refer to the all of the *language of generalization* as "generic language".
  In the literature, however, analysis and experiments are often limited to generalizations about categories. 
...
Simple events (e.g., "John ran yesterday") can be generalized into habitual sentences (e.g., "John runs").
Even events of complex inferential types such as actual causal events (e.g., "My grandpa drank moonshine and it made him blind") can be described in generalization (e.g., "Drinking moonshine makes you go blind").^[
  Conceretely, we are generalizing over the causal relationship being realized in a particular instance, in this casea person.
]

The language of generalization is ubiquitous in everyday conversation and conveys rich meanings [@Orvell2017].
It is highly-prevalent in child-directed speech [@Gelman2008] and its ability to refer to abstractions beyond the present context has implicated it as central to the growth of conceptual knowledge [@Gelman2004].
It is believed every language can express generalization [@Behrens2005; @Carlson1995], and so understanding the language of generalization is a project with a far-reaching implications.
...
Even more perplexing: "Supreme Court Justices have even social security numbers" is thought to be intuitively a bad sentence even if it were the case that on the current bench, nine out of nine justices have even social securtity numbers (i.e., when the sentence "All Supreme Court Justices have even social security numberes" is true).
...
Yet, if John went for a run 3 times in the last year, we probably shouldn't say that "John runs".
...
These observations have led some to conclude that the literal meaning of a generic statement involves more than just the number of members of the category who have the property, referred to as the *prevalence* of the feature in the category.
...
True generics can be supported by different underlying types of category--property relations [e.g., principled vs. statistical connections; @Prasada2007], and thus support qualitatively different inferences [e.g., "Being striped is one aspect of being a tiger" is well supported while "Carrying malaria is one aspect of being a mosquito" is not not, still while "Tigers are striped" and "Mosquitos carry malaria" sound true; @Prasada2013].
This theory has been influential in psychology, because it predicts several qualitative effects. 
Still, the theory is not sufficiently precise to many any quanitative predictions.
...
In this paper, we propose such a semantic core.
To do so, we draw upon the tools of Bayesian models of cognition [@Tenenbaum2011] and truth-functional semantics [@Montague1973; @Cohen1999] to formalize the context-dependent aspects of language interpretation [@Frank2012] with the core semantic hypothesis: The meaning of an utterance conveying generalization is vague description of subjective, predictive probabilities.
...
In Section 1, we describe the computational model, which formalizes the hypothesis that the core meaning of a generalization in language is a vague probability and articulates it in a model of language understanding in which relevant background knowledge can be described formally.
Having defined a model of interpretation, we then describe an information-theoretic, communicative linking function from interpretation to judgements about the truth or acceptibility of a sentence.
...
Generalizations are used to make predictions about events or properties of instances events that an agent has yet to experience [@HumeTHN].
People readily predict that the next dog will have four legs, drinking another cup of coffee will cause jitters, and a new day will find people that we know doing what they habitually do. 
...
We identify *prevalence* as the latent sense that a future instance of a category would have a particular property; others might call this projectibility [@Goodman1955].
...
It is remarkable that so much is learned from ideas expressed vaguely in words.
...
We have argued that the core meaning of such linguistic expressions can be understood by a vague threshold function that operates over a speaker's inductive beliefs, formalizing by probability.
Being able to precisely articulate the conditions under which a sentence is true lets you describe how an agent should update its beliefs based on that sentence. 
Thus, in addition to providing a quantitative understanding of the phenomena, formalizing the language of generalizations opens up the possibility of building models that learn abstract, generalizable knowledge from language.


As a formal theory of the language of generalization, our framework unifies large swaths of language about distinct semantic types: "Robins lay eggs" (categories), "Sally smokes" (events), "Herb X makes animals sleepy" (causes).
Generalization underlies each of these, and we formalize their meaning by appealing to prevalence, which we take to mean *predictive probability* (as opposed to frequency).
We test this theory by exploring its implications for *endorsement*, operationalized as a speaker's decision about whether or not to produce the generalization to naive listener. 
Endorsement, we show, depends in systematic ways on the listener's prior beliefs about the prevalence.
...
This formal separation is a key advancement beyond previous theoretical accounts, which have either aimed to account for context-sensitivity by positing distinct semantic constructs [e.g., *relative* vs *absolute* generics, described below; @Cohen1999] or positing a semantics that cannot be separated from world knowledge [@Leslie2008].
...
In the rest of this discussion, we elaborate how our model relates to other problems in language understanding, sketch out arguments for other known philosophical puzzles in genericity, and situate our theory with respect to extant theories of genericity.
...
Indeed, we see a core communicative function of generics as teaching properties of kinds to naive listeners.
But we argue that the reason why particular generics are licensed in a context concerns the information conveyed to a listener relative to their background knowledge; to interface with a truth-functional semantics, such background knowledge is represented in some capactiy in the form of a prevalence prior.
There is nothing in our theory that rules out shared background knowledge presupposing beliefs about natural kinds and richly structured relations between categories and the properties [a la @Prasada2013].
Such knowledge is not necessary, however: Relevant background knowledge may be merely correlational in nature.
...
statistics of the event or property in question, represented by the prior belief distribution over the prevalence $P(p_{fk})$.
...
Thus, the prevalence prior $P(p_{fk})$ is actually a conditional distribution constructed with respect some comparison class $C$: $P(p_{fk} \mid C)$.
...
Thus, under our framework, generalizations should not only be learned before, but also faciliate in learning, quantifiers.
...
We close by discussing how a few other previously-discussed troubling examples of generalizations could be addressed in our communicative framework.
