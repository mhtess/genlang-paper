...
That document, as well as the other models, analysis, data and experiments used in this paper can be found at \url{https://github.com/mhtess/comgen-paper}.
...
#### Descriptive behavioral results
...
A simple null hypothesis is that habitual statements convey that the a person *has done this action before*, analagous to how the quantifier *Some* conveys that there exists at least one entity with the feature.
Under this hypothesis, all of the items used in our experiment would qualify as good habitual statements as the target character always did the action 3 times during some time window. 
...
Both of the simple alternative hypotheses can immediately be ruled by out by visual inspection of Figure \ref{fig:habituals-endorsement-figure} (top-left), which shows the correspondence between the frequency of the event (transformed to a log times-per-year scale) and the human-judged felicity of the corresponding habitual sentence. 
...
```{r habituals-endorsement-gender-exploration, results="asis"}
...
left_join(
  ...
  d.hab.bayes.gendered %>%
    ...
    #rename(female_MAP, female_low = low, female_high = high)
         ...
         caption = c("Exploratory ananlysis of endorsements of habitual statements by gender for six properties of interest, collapsed across frequencies."),
...
Differences in endorsements would suggest that the *comparison class* of entities in the prior distribution used in our model might be gender-specific (i.e., when evaluating "Mary drinks beer." participants implicitly consider the distribution of frequencies for other women and not other men).
Endorsement probabilities and 95\% credible intervals for these six items for each gender are showed in Table \ref{tab:habituals-endorsement-gender}.
Descriptively, we do not observe appreciate 
...
We used the pragmatic speaker model $S_2$ (Eq.~\ref{eq:S2}) with the priors elicited above (Expt. 2a) to predict felicity judgments in Expt.~2, assuming the target propensity (to be conveyed by $S_2$) is the provided frequency.
