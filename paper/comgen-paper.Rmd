...
Probability is the universal currency of belief and this prediction can be described by a conditional probability: $P(f \mid k) = P(x \in f \mid x \in k)$, the probability that $x$ will have $f$ given that it is in $k$ (formally, an instance of $k$ will be in the set of things that have $f$).
which we will refer to as the *prevalence* and provide the notational simplification $p = p(f \mid k)$.
Our *prevalence* $p$ is a *prevalence in the mind*, the latent sense that a future instance of a category would have a particular property; others might call this projectibility [@Goodman1955].
...
If you *observe* wugs (a novel category) that have two legs, you might infer that all wugs have two legs.
...
Abstract, potentially domain-specific, beliefs about the projectibility of different properties can be represented by a probability distribution over the prevalence $P(p)$ [@Kemp2008].^[
  We adopt the following notational simplification. 
]
Bayes Rule provides the mathematically correct way to update one's prior beliefs with some obesrvational data $d$: $P(p \mid d) \propto P(d \mid p) \cdot P(p)$, assuming some generative process that resulted in the observed data---the likelihood $P(d \mid p)$ [@Tenenbaum2001].
...
The language of generalization is easy to spot when a property, that could project onto an individual, is predicated of a category (e.g., "Dogs have four legs"; *has four legs* could apply to an individual as in "This dog has four legs").
In the semantics literature on generics, bare plural sentences of this kind are sometimes described as *characterizing sentences* in contrast with *kind-denoting* sentences where the property can only meaningfully apply to the category as a whole (e.g., "Dinosaurs are extinct"; *extinct* cannot apply to an individual).
<!-- Dinosaurs are exctinct ~ John is a smoker? -->
Generalization can also manifest when describing an instances of an individual [*habitual language*; e.g., "John smokes"; @Carlson1977, @Carlson2008].
...
If generalization from observations can be described by a probability $p$, it is natural to posit that same construct will be at the heart of a semantic theory of the language of generalization.
...
Thus, the simplest semantics for a generalization about category $k$ with property $f$ is a threshold on the *prevalence*: $\denote{generalization}(p, \theta) = \{p >\theta\}$.
...
<!-- We distinguish our proposal from previous ones by emphasizing the predictive nature of $p$, as well as the formalization into a model that makes precise, quantitative predictions about human endorsements. -->
<!-- $p$ describes a latent belief and should not be confused with *frequency*, which can inform a belief but is not itself the belief. -->
...
The simplest semantics for a generalization is a threshold function: $\denote{gen}(p, \theta) = \{p >\theta\}$, but what threshold is appropriate?
Quantifier statements can be described using particular values of $\theta$: $\denote{some}(p) = \{p > 0\}$; $\denote{all} = \{p = 1\}$.
...
The model also has prior knowledge about the property $P(p)$ and literal interpretation is given by updating these prior beliefs according the truth-functional meaning of utterance $u$:

\begin{eqnarray}
Lit(p, \theta \mid u) &\propto& {\delta_{\denote{u}(p, \theta)} \cdot P(p) \cdot P(\theta)} \label{eq:L0}
\end{eqnarray}
...
Following @Lassiter2015, we defined $P(\theta)$ to be a uniform distribution over the support of $P(p)$.

### Endorsement model

Given the above model of literal interpretation, how do we decide when a statement conveying a generalization is intuitively true or not?
...
We formalize this by considering how well the generalization would convey the prevalence $p$ to the $Lit$ model (Eq. \ref{eq:L0}).^[
  ...
  Rather, the endorser may have a distribution over $p$ for the category $k$, corresponding to the endorser's uncertain beliefs about the prevalence. 
  ...
  $S(u \mid k) \propto \exp{(\lambda \cdot {\mathbb E}_{p\sim P_{k}} \ln{ \int_{\theta} Lit(p, \theta \mid u)} \diff \theta )}$
...
Our endorsement model (Eq. \ref{eq:S1}) provides a mapping from a *referent-category prevalence* $p$  (e.g., $P(x \text{ lays eggs} \mid x \text{ is a robin})$) to an endorsement probability for the corresponding generalization (e.g., "Robins lay eggs"): $S(u = u_{gen} \mid p)$. 
...
The uncertain, truth-conditional threshold $\theta$ is resolved by the literal interpretation model $Lit$ (Eq. \ref{eq:L0}) by integrating the model's prior beliefs about the feature in question---the *prevalence prior* $P(p)$---with the underspecified semantics.
...
However, the interpretation model's prior beliefs $P(p)$ must be considered before the endorsement model can say whether or not would it want to update those beliefs by endorsing the statement. 
...
In particular: The normalizing constant for calculating cue validity ($\sum_{k' \in K} p' \cdot P(p')$) is the expected value (i.e., the mean) of the prevalence prior distribution: $\mathbb{E}[P(p)]$.
...
These simulations show how the model predicted endorsement depends upon the background knowledge about the property $P(p_f)$ as well as the referent-prevalence $p$ communicated. 
...
statistics of the event or property in question, represented by the prior belief distribution over the prevalence $P(p)$.
