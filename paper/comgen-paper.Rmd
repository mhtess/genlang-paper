...
Simple events (e.g., "John ran yesterday") can be generalized into habitual sentences (e.g., "John runs"), and even events of complex inferential types such as actual causal events (e.g., "My grandpa drank moonshine and it made him blind") can be described in generalization (e.g., "Drinking moonshine makes you go blind").

Understanding the language of generalization is a project with far-reaching implications.
The language of generalization is ubiquitous in everyday conversation and conveys rich meanings, impacting motivation [@Cimpian2007], transmitting stereotyped beliefs about social categories [@Rhodes2012], making meaning from experience [@Orvell2017].
...
Finally, it is believed every language can express generalization [@Behrens2005; @Carlson1995].
...
In Section 1, we describe the computational model, which formalizes the hypothesis that the core meaning of a generalization in language is a vague probability and articulates it in a model of language understanding in which relevant background knowledge is formally described.
...
Probability is the universal currency of belief and this prediction can be described by a conditional probability: $P(f \mid k) = P(x \in f \mid x \in k)$, the probability that $x$ will have $f$ given that it is in $k$ (formally, an instance of $k$ will be in the set of things that have $f$), which we will refer to as the *prevalence* and provide the notational simplification $p = P(f \mid k)$.
...
Generalization can also manifest when describing an instances of an individual [*habitual language*; e.g., "John smokes"; @Carlson1977, @Carlson2005].
...
Thus, the simplest semantics for a generalization is a threshold on the prevalence: $\denote{generalization}(p, \theta) = \{p >\theta\}$.
...
We go beyond previous semantic models of generics [e.g., @Cohen1999] by embedding this semantic hypothesis in a cognitive model in which relevant contextual knowledge can be formalized.
By doing so, we thus provide an account of how context influences generic understanding and are able to make precise, quantitative predictions about human judgments. 
...
We formalize this underspecified (or, *vague*) semantics in a Bayesian cognitive model, where the threshold $\theta$ is sampled from a uniform prior distribution over possible thresholds $P(\theta)$ [@Lassiter2013;   @Lassiter2015; cf., @Qing2014].^[
  Following @Lassiter2015, we defined $P(\theta)$ to be a uniform distribution over the support of $P(p)$.
]
The model also has prior knowledge about the property $P(p)$ (described in more detail below) and literal interpretation is given by updating these prior beliefs according the truth-functional meaning of utterance $\delta_{\denote{u}(p, \theta)}$:
...
We do not take up the task of formalizing particular conceptual models, but rather merely try to measure their implications on beliefs about prevalence. 
Without appealing to domain specific knowledge, one can construct a prior distributions over prevalence by considering the prevalence of the feature among alternative categories.
...
These simulations show how the model predicted endorsement depends upon the background knowledge of the interpreter about the property $P(p)$ as well as the referent-prevalence of the endorser $p$ communicated. 
...
We test this theory by exploring its implications for truth judgments, operationalized as a speaker's decision about whether or not to produce the generalization to naive listener. 
...
Rather than have beliefs about prevalence *per se*, listeners are likely to have conceptual knowledge about kinds and properties [@Leslie2007; @Leslie2008; @Prasada2013] that gives rise to beliefs about prevalence.
That is, the prevalence prior is a statistical layer that provides the glue between conceptual knowledge and natural language semantics.
The truth conditions of generalizations depend only on prevalence. 

<!-- Given this minimal representation of background knowledge, language interfaces with conceptual knowledge via prevalence.  -->

<!-- Previous psychological and philosophical work on generics has looked beyond prevalence and focused on kinds and their conceptual relations to properties  -->
<!-- Indeed, we see a core communicative function of generics as teaching properties of kinds to naive listeners. -->
<!-- But we argue that the reason why particular generics are licensed in a context concerns the information conveyed to a listener relative to their background knowledge; to interface with a truth-functional semantics, such background knowledge must have a representation in the form of a prevalence prior. -->
