...
d.hab.priors <- read.csv("/Users/mht/Documents/research/habituals/data/priors/priors-2.csv")
...
```{r habituals-prior-model-run}
items <- levels(d.hab.priors.filtered$item)
n_samples <- 500
...
d.hab.endorsement.bayes <- d.hab.endorsement %>% 
  group_by(habitual, logAnnualRate) %>%
  summarize(k = sum(response), n = n()) %>%
  ungroup() %>%
  mutate(a = 1 + k,
         b = 1 + n - k,
...
d.hab.endorse.regression <- left_join(
  ...
  unique(select(
    d.hab.endorsement, habitual, logAnnualRate, mixture
  )) %>%
