...
Despite their ubiquity and relative simplicity, there have been few formal models of generic language and none that have made precise, quantitative predictions about human understanding.
...
These observations have led to proposals that the statistics of the feature (e.g., the prevalence or probability of a member of a category having a property) is only incidentally related to generic meaning. 
...
This theory predicts that generics which convey striking properties would be true even if the property was relatively uncommon (e.g., "Sharks attack swimmers"), which has been borne out emprically [@Cimpian2010; @Prasada2013].
...
In addition, a unified theory should be formalized to make explicit the relevant assumptions of the psychological or linguistic hypothesis meant to predict human behavior; this, in turn, makes the theory more testable. 
\ndg{we go on to talk about informal models next... maybe this defense of formal models needs to be later? adjust flow somehow.}
\mht{moved it here}
...
The Bayesian probability calculus describes inductive generalizations in terms of probability, and this is has proven to be a good model of human generalization [@Shepard1987; @Tenenbaum2001].
...
We thus explore how these probabilities can be communicated in natural language. 
...
Specifically, we explore the hypothesis that the core meaning of a generic statement and other language for conveying generalization concerns subjective probability in a *simple but underspecified* way.
We combine this notion of generic semantics with RSA, and derive predictions for the endorsements of such statements by considering whether or not a speaker would bother to say the generalization to a listener.
...
\ndg{the next two paragraphs feel partly redundant with what just came...}
...
In Section 2, we formalize the hypothesis that the core of meaning of a generalization is *simple but underspecified* and incorporate this *uncertain semantics* into the RSA framework for pragmatic language understanding.
