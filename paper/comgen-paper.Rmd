...
abstract: > 
    Generalizations are central to human understanding and language provides simple ways to convey generalizations (e.g., "Birds fly").
...
Yet the prediction for a new entity can be described as a *predictive probability*, which we will denote throughout the paper as $h$ and refer to as the **prevalence**: 
$h = P(x \in f \mid x \in k)$, the probability that $x$ will have $f$ given that it is in $k$.^[
We denote prevalence as $h$ to emphasize that it is a latent *hypothesis*, and not necessarily an observable frequency.
]
...
For generalizations, the threshold would apply to the subjective probability or *prevalence* $h$:
...
Following standard treatment from formal semantics, we represent the semantic content of an utterance by the Kronecker delta function  $\delta_{\denote{u}(h, \theta)}$, which here returns probabilities proportional to $1$ for prevalence values $h$ greater than threshold $\theta$, and $0$ otherwise. 
...
Given the semantic likelihood function defined in Eq. \ref{eq:delta} and the prior distribution over thresholds $\theta$, we can formalize a **listener model** by specifying a prior distribution over the prevalence $P(h)$:

\begin{eqnarray}
L(h, \theta \mid u) &\propto& {\delta_{\denote{u}(h, \theta)} \cdot P(h) \cdot P(\theta)} \label{eq:L0}
\end{eqnarray}

Equation \ref{eq:L0} is a model of a Bayesian listener $L$ who updates her beliefs about the prevalence $h$ according to the meaning of an utterance $u$.
$\delta_{\denote{u}(h, \theta)}$ is the likelihood term in Bayes' Rule and represents the literal semantic content of an utterance.
$P(h)$ represents the listener's prior world knowledge concerning the relevant prevalences $h$.
The particular form of the **prevalence prior** $P(h)$ plausibly depends upon the kind of property or event in question and could be structured as a result of deeper conceptual knowledge about properties and events [@Griffiths2005; @Tenenbaum2006; @Kemp2008].
...
In this paper, we are interested in explaining the variable *endorsements* of generalizations (e.g., "Birds lay eggs" is good; "Birds are female" is not) based on some known or believed prevalence $h$ (e.g., only female birds lay eggs).
Following the Rational Speech Act (RSA) framework [@Goodman2016], we describe a **speaker model** as a Bayesian decision of whether or not to say the generalization to the listener $L$ (Eq. \ref{eq:L0}).^[A more general version of this model can relax the assumption that the speaker has access to a specific value of prevalence $h$ that he is trying to communicate.
...
The speaker's decision to say an utterance (e.g., the generalization) is determined by how well it would communicate $h$ to the listener $L$, in comparison to alternative utterances.
...
The RSA model defined in the previous section predicts that the endorsement of a generalization depends upon the probability that utterance vs. silence accurate conveys to a naive listener a prevalence $h$ in a referent-category (e.g., the \% of robins that lay eggs).
This probability is given by $L(h \mid u, \theta)$ (Eq. \ref{eq:S1}) and depends upon  the listener's *a priori* beliefs about the prevalence $h$ in question: $P(h)$ (Eq. \ref{eq:L0}), as well as the referent-category prevalence $h$ (Eq. \ref{eq:S1}).
...
The fixed-threshold model rules out only the lowest possible prevalence ($h \sim 0$) and returns a posterior very similar to the prior.
...
If the threshold $\theta$ is very high, only the highest prevalences $h$ would pass the threshold.
...
In each case, we must consider the prevalence prior for the feature:  $P(h_\text{lay eggs})$ and $P(h_\text{are female})$.
...
For these simulations, we had specify the prevalence prior $P(h)$.
...
That is, "Mosquitos carry malaria" and "Birds lay eggs" are reasonable things to say, but "Birds are female" is not.
...
We validate these simulations empirically by measuring the prior belief distribution over the relevant probabilities $P(h)$ used in Eq. \ref{eq:L0} and the prevalence of the category that the speaker model seeks to convey (the *referent-category* prevalence) $h$ in Eq. \ref{eq:S1} (Expt. 1a).
...
In additional to the canonical cases from the linguistics literature, we select sentences to elicit the full range of acceptability judgments (intuitively: "acceptable", "unacceptable", and "indeterminate") with low-, medium-, and high-prevalence properties. 
...
This thought experiment decomposes the prevalence prior $P(h)$ into a prior on kinds $P(k)$ and then a conditional probability of the prevalence of the feature given the kind $P(h \mid k)$.
We will use this decomposition to measure the prevalence prior $P(h) = \int_{k} P(h \mid k) \diff k$, and perform a structured Bayesian data-analysis to model the prior data.
...
To see how well the structured model fits the prevalence prior data, we use Bayesian inference to infer the *a posteriori* likely values of the parameters and then use the inferred parameters to generate new data.
...
Similar phenomena have been observed in other prevalence elicitation tasks when the referent-category is fixed [e.g., estimating only the percentage of *robins* that *lay eggs*; @Prasada2013]
...
The mixture parameter $\phi$ in the prevalence prior distribution can be seen as a generalization of the construct of cue validity. 
Recall that $\phi$ is the weighting between the two generative components of the prevalence prior (i.e., the stable cause distribution $\mathcal{D}_{stable}$ and the transient cause distribution $\mathcal{D}_{transient}$).
...
This data also includes information about the referent-category prevalence (e.g., how many robins lay eggs).
Using the referent-category prevalence as $h$ in our speaker model (Eq. \ref{eq:S1}), the speaker model will predict endorsements for the corresponding generic statements (e.g., "Robins lay eggs").
...
```{r generics-endorsement-figure, fig.width=12, fig.height=9.5, fig.cap="Endorsing generalizations about categories. Top left: Human elicited endorsements for thirty generic sentences reveal a continuuum of endorsements. Top right: Model fits for the uncertain semantics speaker model (upper right), a fixed semantics speaker model (upper left), and regression models based on prevalence alone (lower left) and prevalence and cue validity (lower right). Bottom: Five example prevalence priors, listener posteriors upon hearing the generalization, and speaker belief distributions about the prevalence of the feature within a referent category. These distributions are inferred using all three data sources from Expt. 1 (see Figure \\ref{fig:genericsModelDiagram} for an overview of the Bayesian data analytic approach)."}
...
Unlike the simple regression models based on point-estimates of referent-category prevalence and cue validity, this cognitive model has access to the full prior distribution on prevalence $P(h)$.
...
<!-- Our pragmatic speaker model, $S_1$ in Eq. \ref{eq:S1}, reasons about whether it would be useful to say to the generic to this kind of listener so that the listener may acquire the same beliefs as the speaker (represented as the prevalence of the property within the referent-category $h'$). -->
We use the empirically estimated referent-category prevalence as the $h$ that the speaker $S_1$ is trying to communicate, and use the empirically measured priors from Expt. 1a as the listener's prevalence prior $P(h)$.^[
...
Importantly, the model recapitulates the prevalence prior data (e.g., the prevalence of laying eggs among various species) and the referent-category prevalence data (e.g., the prevalence of laying eggs among robins; see Appendix A for full posteriors) as it did when the Bayesian data analysis model only included these data and their associated model (results section of Expt. 1a).
...
The blue, solid lined distribution corresponds to the speaker's beliefs about the prevalence of the feature for the referent-category (also in blue).
...
In explaining the variable endorsements of generics, we measured the prior distribution over the prevalence of features across categories in addition to the prevalence of the feature in the referent-category (e.g., the percentage of robins that lay eggs) to model the endorsement data.
