...
If generalization from observations can be described using probability, then it seems very natural to consider the semantic content of an utterance that conveys generalization also to be relating to probability. 
...
<!-- Bare plural statements (e.g., "Dogs are friendly") tend to yield a generic meaning, though other forms can express such a meaning [e.g., "A mongoose eats snakes."; @Carlson1977] . -->
...
<!-- "Robins lay eggs" is true, though, despite the fact that only female robins lay eggs. -->
...
$\delta_{\denote{u}(h, \theta)}$ is the Kronecker delta function returning $1$ for probabilities $h$ greater than threshold $\theta$ and $0$ otherwise. 
...
When there is a latent parameter the Bayesian scientist does not know much about, she puts a prior distribution over the parameter and infers the parameter's likely value by collecting data.
...
The particular form of the prior plausibly depends upon the kind of property or event in question and could be structured as a result of deeper conceptual knowledge about properties and events [@Griffiths2005; @Tenenbaum2006; @Kemp2008].
...
This is a Bayesian model of interpreting a statement conveying a generalization whose meaning is simple (i.e., a threshold) but underspecified.
Figure \ref{fig:simulations} shows model $L$'s posterior distribution on prevalence $h$ upon hearing utterances corresponding to the uncertain threshold (Eq. \ref{eq:L0}) as well as a comparison model using a fixed threshold at 0 (intuitively, corresponding to the quantifier "some").
The differences in the semantics are easily seen when considering a uniform prior distribution over $h$ (top left facet).
The fixed threshold model rules out only the lowest possible prevalence ($h=0$) and returns a posterior very similar to the prior.
The behavior of the generalization is distinct: The higher the $h$, the more likely it is after hearing the generalization.

The intuition behind the model's behavior can be gleaned by imagining what the listener $L$ would believe given different thresholds and averaging over those possibilities.
If the threshold $\theta$ is very high, only the highest prevalence $h$ would pass the threshold.
If the threshold $\theta$ is slightly lower but still high, the highest $h$ would still pass the threshold, as would some that are slightly lower.
...
When the listener averages over these possibilities, $h$'s with higher values pass more thresholds; thus, higher $h$'s are more likely *a posteriori*.
...
Following RSA, the speaker is assumed to be an approximately rational Bayesian agent with degree with rationality governed by parameter $\lambda$.
