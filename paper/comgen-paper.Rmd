...
For example, the sentence "Some dogs have four legs" is believed to be literally true if the chances that an individual dog will have four legs is greater than zero: $\denote{some}(p) := p > 0$.
"Most dogs have four legs" can also be described as a threshold on prevalence (e.g., the chance that a dog will have four legs is greater than 50%): $\denote{most}(p) := p > 0.5$.^[
  We note that we are describing the standard semantic truth conditions of quantifiers, not their pragmatic interpretations (e.g., that "some" often implies "not all").
]
...
We denote this probabilistic generic interpretation model by $L$ to indicate that it is modeling a listener updating their beliefs according to the truth-functional meaning of an utterance $u$.
...
As the threshold decreases, more and more prevalence levels become be compatible with the threshold (Figure\ \@ref(fig:uniformPriorSimulation)A).
...
The semantic threshold $\theta$ is necessary for establishing the precise truth conditions (used in Eq. \ref{eq:L0}) but is otherwise a nuisance parameter, which the endorsement model integrates out.
Finally, Eq. \ref{eq:S1} models an information-theoretic speaker's decision, which we assume is approximately rational, with degree of rationality given by $\lambda$.
...
Given that the speaker only has two options (i.e., produce the generalization utterance or stay silent), the endorsement decision comes down to whether or not the *referent prevalence* is more likely under the prevalence prior distribution $P(p)$ or the listener interpretation distribution given by Eq. \ref{eq:L0} (Figure\ \@ref(fig:uniformPriorSimulation)C).
...
We explore the predictions of this model by varying each of these factors in the context of a few worked examples.
These examples are taken from the linguistics literature on generics and are statements that any theory of generics should be able to correctly predict are either true, false, or somehow indeterminate. 
...
The uncertain threshold model's interpretations are highly sensitive to background knowledge, formalized in simplest terms in terms of a *prevalence prior*.
Prior distributions over prevalence can be revealed by considering the prevalence of the feature in alternative categories (Figure\ \@ref(fig:simulations)).
...
Figure\ \@ref(fig:simulations) (row 1) show hypothetical prevalence priors for six different properties, formalized as Beta distributions or mixtures of Beta distributions.^[
  The Beta distribution is a distribution over numbers between 0 and 1. 
  In other words, it is a probability distribution over probabilities or prevalences.
]
...
We explore the model's ability to account for intuitive truth judgments for different examples that have been historically challenging for prevalence-based approaches.
...
worked.examples.tab <- xtable::xtable(worked.examples, 
                       caption = "Example generalizations that a theory of generics should correctly predict.", 
