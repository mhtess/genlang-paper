...
What is the meaning of the utterance: "Drinking moonshine makes you go blind"?
...
Classically, this is cast as a problem of inducing an unobservable relation from observable data. 
For example, assuming a controlled experiment: observing E whenever C is present, and the absence of E whenever C is absent, is strong evidence for concluding that C causes E.
At the same time, individual events (e.g., observing $c_i$ and $e_i$) can be construed as *causal*, depending on whether $c_i$ is seen as a difference-maker for $e_i$ and how exactly $c_i$ made that difference [@Gerstenberg2015how].
Going from covariation to causation, then, implies that during the events observed (the covariation), causal attribution was withheld: The events were not seen as causal at the time, only in retrospect.
Alternatively, we may conclude there is no cause after first seeing events in-the-moment as causal.
