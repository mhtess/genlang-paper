  ...
  slides.prior_elicitation = slide({
   ...
   // present : _.shuffle(_.range(numTrials)),
    ...
    nextExperiment: function() {
      ...
      if (this.stim.data[i] == "?") {
        $("#evidenceDescription").html("Your team treated "+isAnother+"100 " + this.stim.category + " with " + this.stim.treatment + " " + this.experimentNames[i] + ". <br>The results of this experiment have been misplaced so <strong>we don't know</strong> how many " + this.stim.unit + this.stim.evidence +".");
      } else {
      ...
      if (this.count == (this.experimentNames.length - 1)) {
        ...
        if (this.waitFlag == 1) {
          ...
          if (exp.condition == "prior"){
            ...
            $(".question0").html("There were 4 other experiments conducted that day.<br>For each one, your team treated another 100 " + this.stim.category + " with a different " + this.stim.treatment + ".<br> How many " + this.stim.unit + " do you think will be " +this.stim.past + " by each " + this.stim.treatment + "?")
            ...
            prompt +=  "<br>" + replaceTerms(this.stim, "frequencyStatement") + "<br>As a result, "+"<strong>" +
            this.stim.frequency + "</strong> " +this.stim.category + this.stim.evidence + "<br>";
