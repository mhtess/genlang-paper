...
d.gen.endorse.bayes <- left_join(
  ...
  mutate(response = ifelse(response == "agree-key", 1, 0),
         sentence = gsub('&quotechar', '', sentence),
         sentence = gsub('lyme', 'Lyme', sentence)) %>%
  group_by(sentence, Property, Category) %>%
...
```{r generic-model-insets2, fig.width = 4.75, fig.height = 1.5}
load('../analysis/generics-priors-posteriorPredictives.Rdata')
lower_bins <-seq(0, 0.975, 0.025)
upper_bins <- seq(0.025, 1, 0.025)
...
m.gen.endorse.priors.binned.i <- m.gen.endorse.priors.posteriorPredictive %>% 
  filter(src == "Data") %>%
  #group_by(property) %>%
  #sample_n(28, replace = TRUE) %>%
  #gather(category, response, -workerid, -property) %>%
  rowwise() %>%
  mutate(binned_response = round_to_bin(prevalence)) %>%
  group_by(Property, binned_response) %>%
  count() %>%
  ungroup() %>%
  spread(binned_response, n)

m.gen.endorse.priors.binned.i[is.na(m.gen.endorse.priors.binned.i)] <- 0

m.gen.endorse.priors.binned <- m.gen.endorse.priors.binned.i %>%
...
var bins = _.map(data.prior, "state");

var thetaBins = map2(function(b1, b2){
  return roundTo3((b2-b1)/2 + b1);
}, bins.slice(0, bins.length-1), bins.slice(1))

var priorProbs =  _.map(data.prior, "prop");

var thetaPrior = Infer({model: function(){
 return uniformDraw(thetaBins)
}});

var uncertainThresholdInterpreter = Infer({model: function(){
   var state = categorical({vs: bins, ps: priorProbs})
   factor( Math.log(state) )
    return {
      state: state, 
  }
...
var returnVal = {uncertainThresholdInterpreter, prior}
returnVal
'

properties <- unique(m.gen.endorse.priors.binned$Property)
gen.listener.predictions <- data.frame()
for (p in unique(m.gen.endorse.priors.binned$Property)){
  
  priorProbs = m.gen.endorse.priors.binned %>% 
    filter(Property == p) %>%
    ungroup() %>%
    select(state, prop) %>%
  ...
  wp.rs <- webppl(
    program_code = rsaHelpers2, 
    data = list(prior = priorProbs, 
                utt = "generic") ,
    data_var = "data"
    )
  wp.rs.generic <- get_samples(data.frame(wp.rs$uncertainThresholdInterpreter) %>%
                                 rename(prob = probs), 10000)
  wp.rs.prior <- get_samples(data.frame(wp.rs$prior) %>%
                                 rename(prob = probs), 10000)
  
  
  gen.listener.predictions <- bind_rows(
   gen.listener.predictions, 
    wp.rs.prior  %>% mutate(property = p, Parameter = "state_Prior"),
    wp.rs.generic  %>% mutate(property = p, Parameter = "state_Posterior")
   )
}



example.generics.properties
...
gen.inset.distributions <- bind_rows(
  gen.listener.predictions %>%
    mutate(category = NA),
  d.gen.endorse.priors %>% 
    mutate(Sentence = paste(Category, Property)) %>%
    filter(Sentence %in% example.generics) %>%
    select(prevalence, Property, Category) %>%
    rename(state = prevalence, property = Property, category = Category) %>%
...
library(ggridges)
gen.inset.distributions.refactored <- gen.inset.distributions %>% filter(property %in% category.text.labels$property)
  
#save(gen.inset.distributions.refactored)
ggplot( gen.inset.distributions.refactored %>%
  ...
  geom_density_ridges(scale = 1, alpha = 0.3, adjust = 1) +
...
for (p in example.generics.properties){
 ...
 l0.rs.posterior <- webppl(paste(rsaHelpers, 
                                 uncertain.threshold.model.3comp, 
                                 uncertain.threshold.call, sep = '\n'), 
                           data = inputData, data_var = "data")
 
 l0.rs.prior <- webppl(paste(rsaHelpers, 
                             no.utterance.model.3comp, 
                             no.utterance.call, sep = '\n'), 
                       data = inputData, data_var = "data")
...
m.samp.i <- m.samp %>%
    ...
    filter(Sentence %in% example.generics) %>%
    mutate(parameter = paste(param, property, category, component, sep = "_")) %>%
    select(-param, -property, -category, -component, -chain, -type) %>%
    group_by(parameter) %>%
    mutate(iteration = ave(parameter==parameter, parameter, FUN=cumsum)) %>%
    ungroup() %>%
    separate(parameter, into = c("parameter", "property", "category", "component"), sep= "_") %>%
...
m.samp.i.mixture <- m.samp.i %>% 
    filter(component == 1) %>%
    rowwise() %>%
    mutate(c = rbinom(size = 1, n = 1, prob = mix))

m.samp.ij <- left_join(
  m.samp.i %>%
    select(Sentence, property, category, iteration, component, val) %>%
    spread(component, val),
  m.samp.i.mixture %>% select(Sentence, property, category, iteration, c)
) %>% rowwise() %>%
  mutate(prev = ifelse(c == 1, `1`, `2`))
...
```{r vizualize insets, fig.width = 8, fig.height =4}
...
#      file = "../paper/cached_results/case1_modelPosteriors_moreFlexiblePrior.RData")
...
generics.endorsement.insets <- ggplot(gen.inset.distributions.refactored, 
  ...
  geom_density(aes(y = ..scaled..), adjust = 1.5, size = 1)+
  facet_wrap(~property, nrow = 1)+
...
```{r}
load( file = '../analysis/generics-priors-posteriorParameters.Rdata')
m.gen.endorse.priors.parameters <- left_join(
  m.gen.endorse.priors.parameters,
...
ggplot(m.gen.endorse.priors.parameters, aes( x = MAP, xmin = cred_lower, xmax = cred_upper,
...
### Referent prevalence parameters

```{r generic-refprev-parameters, eval = F}
m.gen.endorse.refprev.posteriorPredictive <- data.frame()
n_samples <- 20000
lg = 0

m.gen.endorse.refprev.parameters <- data.frame()

for (p in levels(d.gen.endorse.priors$Property)) {
  categories <- d.gen.endorse.bayes %>% filter(Property == p) %>% distinct(Category)
  
  for (k in categories$Category) {

    priorData <- filter(d.gen.endorse.priors, Property == p, Category == k)$prevalence
  
    m.gen.refprev <- webppl(
      paste(genericEndorsementPriorModelHelpers, unstructuredPriorModel, sep = ""),
                                 data = priorData, data_var = "data",
                                 model_var = "model",
                                 chains = 2,
  ...
  m.gen.refprev.summary <- m.gen.refprev %>%
    group_by(Parameter) %>%
    summarize(MAP = estimate_mode(value),
              cred_upper = hdi_upper(value),
              cred_lower = hdi_lower(value))
    
    m.gen.endorse.refprev.parameters <- bind_rows(
      m.gen.endorse.refprev.parameters,
      m.gen.refprev.summary %>%
        mutate(Property = p, Category = k)
      )
    print(k)
  }
  print(p)
 ...
 #save(m.gen.endorse.refprev.parameters, file = '../analysis/generics-refprev-posteriorParameters.Rdata')
```


```{r}
load(file = '../analysis/generics-refprev-posteriorParameters.Rdata')

m.gen.endorse.refprev.parameters <- left_join(
  m.gen.endorse.refprev.parameters,
  m.gen.endorse.joint.prior.parameters %>%
    filter(type == "targetPrevalence") %>%
    rowwise() %>%
    mutate(Parameter = ifelse(param == "mean", "g", "d")) %>%
    ungroup() %>% 
    select(property, category, Parameter, MAP, cred_upper, cred_lower) %>%
    rename(Property = property,
           Category = category,
           MAP_joint = MAP,
           cred_upper_joint = cred_upper,
...
ggplot(m.gen.endorse.refprev.parameters, aes( x = MAP, xmin = cred_lower, xmax = cred_upper,
                 y = MAP_joint, ymin = cred_lower_joint, ymax = cred_upper_joint ))+
  geom_point()+
  geom_errorbar(alpha = 0.3)+geom_errorbarh(alpha = 0.3)+
  facet_wrap(~Parameter, scales = 'free', labeller = label_parsed)+
  xlab("Inferred parameters using only prevalence data")+
  ylab("Inferred parameters in joint-inference model")
```


```{r}
save(m.gen.endorse.priors.parameters, m.gen.endorse.refprev.parameters,
     file = "../paper/cached_results/case1_prevParamDistortion.RData")
```
