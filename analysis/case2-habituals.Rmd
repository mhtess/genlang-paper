...
d.hab.predictive.endorsement.filtered <- left_join(
...
d.hab.predictive.endorsment.condition.summary <- d.hab.predictive.endorsement.filtered %>%
  group_by(condition) %>%
  summarize(k = sum(response), n = n()) %>%
  ungroup() %>%
  mutate(a = 1 + k,
...
```{r}
 ...
 m.hab.predictive.endorse.summarized <- m.hab.predictive.endorse.rsa %>%
      select(-MAP, -cred_upper, -cred_lower, -src) %>% 
      unique()


d.hab.predictive.endorsement.filtered.logAnnualRates <- left_join(
  d.hab.predictive.endorsement.filtered %>% 
    select(-freqLevel, -past_logfreq, -future_logfreq),
  m.hab.predictive.endorse.summarized %>%
    select(item, condition, logAnnualPastRate, logAnnualFutureRate, future_lower, future_upper) 
)
...
glm.hab.pred.endorse.pastfreq <- glm(response ~ logAnnualPastRate, 
              data = d.hab.predictive.endorsement.filtered.logAnnualRates, family = 'binomial')

glm.hab.pred.endorse.predfreq <- glm(response ~ logAnnualFutureRate, 
              data = d.hab.predictive.endorsement.filtered.logAnnualRates, family = 'binomial')


d.hab.predictive.endorsement.filtered.logAnnualRates.uniq <- unique(select(
    d.hab.predictive.endorsement.filtered.logAnnualRates, item, logAnnualPastRate, logAnnualFutureRate, future_lower, future_upper, condition
  ))


m.hab.predictive.endorse.regPastFreq <- left_join(
    m.hab.predictive.endorse.summarized,
    m.hab.predictive.endorse.summarized %>%
    mutate(
      MAP = predict(glm.hab.pred.endorse.pastfreq, ., type = "response"), 
      cred_upper = MAP,
      cred_lower = MAP,
      src = "regression_pastfreq"
      )
)

m.hab.predictive.endorse.summarized.lower <- m.hab.predictive.endorse.summarized %>%
  select(-logAnnualFutureRate) %>%
  rename(logAnnualFutureRate = future_lower)


m.hab.predictive.endorse.summarized.upper <- m.hab.predictive.endorse.summarized %>%
  select(-logAnnualFutureRate) %>%
  rename(logAnnualFutureRate = future_upper)


m.hab.predictive.endorse.regPredFreq <- left_join(
    m.hab.predictive.endorse.summarized,
    m.hab.predictive.endorse.summarized %>%
    mutate(
      MAP = predict(glm.hab.pred.endorse.predfreq, ., type = "response"), 
      cred_lower = predict(glm.hab.pred.endorse.predfreq, m.hab.predictive.endorse.summarized.lower, type = "response"),
      cred_upper = predict(glm.hab.pred.endorse.predfreq, m.hab.predictive.endorse.summarized.upper, type = "response"),
      src = "regression_futurefreq"
      )
)



m.hab.predictive.endorse.allModels <- bind_rows(
  m.hab.predictive.endorse.rsa,
  m.hab.predictive.endorse.regPredFreq,
  m.hab.predictive.endorse.regPastFreq,
  m.hab.predictive.endorse.rsa %>%
    select(-MAP, -cred_upper, -cred_lower, -src) %>%
    unique() %>%
    mutate(MAP = MAP_h, cred_lower = low,
         cred_upper = high, src = "human data")
)
...
save(d.hab.predictive.endorsment.summary, d.hab.predictive.summary, 
     ...
     lmer.rs.hab.predictive.summary, m.hab.predictive.endorse.allModels,
     file = "../paper/cached_results/case2_predictive_data.RData")

#m.hab.predictive.endorse.allModels
```